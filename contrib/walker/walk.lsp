...
(define-walker-template COMPILER-LET         walk-compiler-let) ;; CLTL1
...
(define-walker-template si:lambda-block walk-named-lambda)	;Not really right, but
							        ;we don't hack block
						        	;names anyways.
#+mkcl
...
(defun walk-dotimes/dolist (form context old-env)
  (walker-environment-bind (new-env old-env)
    (let* ((dotimes/dolist (car form))
	   ...
	   (walked-bindings (walk-bindings-1 (list bindings) ;; this wrap is need by the hack. JCB
					     old-env
					     new-env
					     context
					     t))
      ...
      (relist* form
	       dotimes/dolist
	       (car ;; needed to undo the hack. JCB
		(walk-bindings-2 
		 (list bindings) ;; wrapped here too to be consistent with the hack. JCB
		 walked-bindings context new-env))
	       walked-body))))
