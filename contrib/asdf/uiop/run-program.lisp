...
It returns a process-info plist with possible keys:
    ...
    #-(or allegro clozure cmu (and lispworks os-unix) mkcl sbcl scl)
    ...
    (or (getf process-info :exit-code)
        (let ((process (getf process-info :process)))
          (when process
            ...
            ;; both combined
            #+mkcl (mk-ext:join-process process)))))
  ...
  (defun %redirected-system-command (command in out err directory) ;; helper for %USE-SYSTEM
    ;; This thing here below is using Bourne shell syntax for redirection. It has no
    ;; chance at all of working on MS-Windows. JCB
