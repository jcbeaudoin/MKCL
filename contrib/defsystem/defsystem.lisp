...
(terpri) (princ "About to define package MAKE.")
...
#+(or clisp cormanlisp mkcl ecl (and gcl defpackage) sbcl)
(defpackage "MAKE" (:use "COMMON-LISP") (:nicknames "MK"))

#-(or :sbcl :cltl2 :lispworks :mkcl :ecl :scl)
	  ...
	  (let ((*load-verbose* #-common-lisp-controller t
	    ...
	    (declare (special *compile-verbose* 
		     ...
		     #-openmcl (ignore *compile-verbose*
				       #-(or MCL CMU CLISP MKCL ECL :sbcl lispworks scl)
				       *compile-file-verbose*)
		     #-(or mkcl openmcl) (optimize (inhibit-warnings 3)))
...
(unless *old-require*
  ...
  (unless *dont-redefine-require*
    (let (#+(or :mcl (and :CCL (not :lispworks)))
      ...
      #-(or :mkcl (and allegro-version>= (version>= 4 1)) :lispworks)
...
(defun run-unix-program (program arguments)
  ...
  #+:allegro (excl:run-shell-command
	      (format nil "~A~@[ ~{~A~^ ~}~]"
		      program arguments))
  #+(or :kcl :mkcl :ecl) (system (format nil "~A~@[ ~{~A~^ ~}~]" program arguments))
...
output to *trace-output*.  Returns the shell's exit code."
  ...
  (let ((command (apply #'format nil command-control-string arguments)))
    ...
    #-(or openmcl clisp lispworks allegro scl cmu sbcl mkcl)
...
(define-language :c
  ...
  :loader #+:lucid #'load-foreign-files
	  ...
	  #+(and :lispworks :win32) #'fli:register-module
          #+(or :mkcl :ecl :gcl :kcl) #'load ; should be enough.
          #-(or :lucid
		:allegro
		:cmu
		:sbcl
