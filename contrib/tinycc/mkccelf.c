...
ST_FUNC addr_t get_elf_sym_addr(MKCCState *s, const char *name, int err)
{
    int sym_index;
    ElfW(Sym) *sym;
...
#if defined MKCC_IS_NATIVE || defined MKCC_TARGET_PE
/* return elf symbol value or error */
ST_FUNC void* mkcc_get_symbol_err(MKCCState *s, const char *name)
{
    ...
    if (sym_bind != STB_LOCAL) {
        ...
        if (esym->st_shndx != SHN_UNDEF) {
            ...
            if (sh_num == SHN_UNDEF) {
                ...
                /* gr: Happens with 'mkcc ... -static mkcctest.c' on e.g. Ubuntu 6.01
                   No idea if this is the correct solution ... */
                goto do_patch;
            } else if (s == mkcc_state->dynsymtab_section) {
...
#endif
                mkcc_error_noabort("'%s' defined twice... may be -fcommon is needed?", name);
            }
        } else {
        do_patch:
                            ...
                            int type, int symbol, addr_t addend)
{
    ...
    if (!sr) {
        ...
        /* if the symtab is allocated, then we consider the relocation
           are also */
        sr = new_section(mkcc_state, buf, SHT_RELX, symtab->sh_flags);
...
#if defined(MKCC_TARGET_ARM64) || defined(MKCC_TARGET_X86_64)
    rel->r_addend = addend;
#else
    if (addend)
        mkcc_error("non-zero addend on REL architecture");
#endif
}
...
/* In an ELF file symbol table, the local symbols must appear below
   the global and weak ones. Since MKCC cannot sort it while generating
   the code, we must do it after. All the relocation tables are also
   modified to take into account the symbol table sorting */
static void sort_syms(MKCCState *s1, Section *s)
{
    int *old_to_new_syms;
    ElfW(Sym) *new_syms;
    int nb_syms, i;
    ...
    nb_syms = s->data_offset / sizeof(ElfW(Sym));
    new_syms = mkcc_malloc(nb_syms * sizeof(ElfW(Sym)));
    old_to_new_syms = mkcc_malloc(nb_syms * sizeof(int));
...
ST_FUNC void relocate_syms(MKCCState *s1, int do_resolve)
{
    ElfW(Sym) *sym, *esym;
    ...
    for_each_elem(symtab_section, 1, sym, ElfW(Sym)) {
        ...
        if (sh_num == SHN_UNDEF) {
            ...
            /* Use ld.so to resolve symbol for us (for mkcc -run) */
            if (do_resolve) {
#if defined MKCC_IS_NATIVE && !defined _WIN32
                void *addr;
            ...
            if (sym_bind == STB_WEAK) {
                ...
                mkcc_error_noabort("undefined symbol '%s'", name);
            }
...
ST_FUNC void relocate_section(MKCCState *s1, Section *s)
{
    Section *sr = s->reloc;
    ElfW_Rel *rel;
    ElfW(Sym) *sym;
    ...
    for_each_elem(sr, 0, rel, ElfW_Rel) {
      /* section_ptr_add(s, 8); */ /* Someone suggested this to fix a malloc corruption. JCB */
        ...
        case R_386_32:
            if (s1->output_type == MKCC_OUTPUT_DLL) {
        ...
        case R_386_GOT32X:
            ...
            break;
        case R_386_16:
            if (s1->output_format != MKCC_OUTPUT_FORMAT_BINARY) {
            output_file:
                mkcc_error("can only produce 16-bit binary files");
            }
		...
		printf (" newx=0x%x name=%s\n", x,
			(char *) symtab_section->link->data + sym->st_name);
                ...
                if (th_ko || x >= 0x2000000 || x < -0x2000000)
                    mkcc_error("can't relocate value at %x,%d",addr, type);
                x >>= 2;
                x &= 0xffffff;
        ...
        case R_AARCH64_ADR_PREL_PG_HI21: {
            ...
            if ((off + ((uint64_t)1 << 20)) >> 21)
                mkcc_error("R_AARCH64_ADR_PREL_PG_HI21 relocation failed");
	    ...
	    printf ("reloc %d @ 0x%lx: val=0x%lx name=%s\n", type, addr, val,
	      ...
	      {
                mkcc_error("R_AARCH64_(JUMP|CALL)26 relocation failed (val=%lx, addr=%lx)", addr, val);
	      }
        ...
        case R_AARCH64_ADR_GOT_PAGE: {
            ...
            if ((off + ((uint64_t)1 << 20)) >> 21)
                mkcc_error("R_AARCH64_ADR_GOT_PAGE relocation failed");
            write32le(ptr, ((read32le(ptr) & 0x9f00001f) |
                            (off & 0x1ffffc) << 3 | (off & 3) << 29));
            break;
        }
        ...
        case R_X86_64_32S:
            if (s1->output_type == MKCC_OUTPUT_DLL) {
                /* XXX: this logic may depend on MKCC's codegen
                   now MKCC uses R_X86_64_32 even for a 64bit pointer */
                qrel->r_info = ELFW(R_INFO)(0, R_X86_64_RELATIVE);
		/* Use sign extension! */
	...
	plt32pc32:
	{
            ...
            if (diff < -2147483648LL || diff > 2147483647LL) {
                mkcc_error("internal error: relocation failed");
            }
...
static void relocate_rel(MKCCState *s1, Section *sr)
{
    Section *s;
    ElfW_Rel *rel;
...
static struct sym_attr *alloc_sym_attr(MKCCState *s1, int index)
{
    int n;
    struct sym_attr *tab;
    ...
    if (index >= s1->nb_sym_attrs) {
        ...
        while (index >= n)
            n *= 2;
        tab = mkcc_realloc(s1->sym_attrs, n * sizeof(*s1->sym_attrs));
        s1->sym_attrs = tab;
...
static void build_got(MKCCState *s1)
{
    unsigned char *ptr;
...
static unsigned long put_got_entry(MKCCState *s1,
...
#if defined(MKCC_OUTPUT_DLL_WITH_PLT)
            modrm = 0x25;
#else
...
#elif defined(MKCC_TARGET_C67)
    if (s1->dynsym) {
        mkcc_error("C67 got not implemented");
    }
#else
...
ST_FUNC void build_got_entries(MKCCState *s1)
{
    Section *s;
    ElfW_Rel *rel;
    ElfW(Sym) *sym;
...
ST_FUNC Section *new_symtab(MKCCState *s1,
...
static void add_init_array_defines(MKCCState *s1, const char *section_name)
{
    Section *s;
    long end_offset;
    char sym_start[1024];
...
#if 0
static int mkcc_add_support(MKCCState *s1, const char *filename)
{
    char buf[1024];
    snprintf(buf, sizeof(buf), "%s/%s/%s", s1->mkcc_lib_path,
    /* an cpu specific path inside mkcc_lib_path, mainly for keeping libmkcc1.a */
    ...
    return mkcc_add_file(s1, buf, MKCC_FILETYPE_BINARY);
}
#endif

ST_FUNC void mkcc_add_bcheck(MKCCState *s1)
{
    ...
    *ptr = 0;
    ...
    if (s1->output_type != MKCC_OUTPUT_MEMORY) {
        ...
        /* XXX not called on MSYS, reason is unknown. For this
           case a call to __mkcc_bound_init is performed in bcheck.c
           when __mkcc_bound_ptr_add, __mkcc_bound_new_region,
           __mkcc_bound_delete_region called */

	int sym_index = find_elf_sym(symtab_section, "__mkcc_bound_init");
	if (sym_index) {
    	    ...
    	    mkcc_warning("__mkcc_bound_init not defined");
    }
#endif
}

/* add mkcc runtime libraries */
ST_FUNC void mkcc_add_runtime(MKCCState *s1)
{
    mkcc_add_pragma_libs(s1);

    /* add libc */
    if (!s1->nostdlib) {
#if 1
        mkcc_add_library(s1, "mkcc1");             
#endif
        mkcc_add_library(s1, "c");
#ifdef CONFIG_USE_LIBGCC
        if (!s1->static_link) {
            mkcc_add_file(s1, MKCC_LIBGCC, MKCC_FILETYPE_BINARY);
        }
#endif
#if 0
        mkcc_add_support(s1, "libmkcc1.a");
#endif
    }

    /* mkcc_add_bcheck tries to relocate a call to __mkcc_bound_init in _init so
       libmkcc1.a must be loaded before for __mkcc_bound_init to be defined and
...
/* add various standard linker symbols (must be done after the
   ...
   symbols)) */
ST_FUNC void mkcc_add_linker_symbols(MKCCState *s1)
{
    char buf[1024];
    int i;
    Section *s;
...
static void mkcc_output_binary(MKCCState *s1, FILE *f,
                              const int *sec_order)
{
    Section *s;
    int i, offset, size;
...
static void patch_dynsym_undef(MKCCState *s1, Section *s)
{
...
ST_FUNC void fill_got_entry(MKCCState *s1, ElfW_Rel *rel)
{
...
ST_FUNC void fill_got(MKCCState *s1)
{
    Section *s;
    ElfW_Rel *rel;
    int i;
...
static void bind_exe_dynsyms(MKCCState *s1)
{
    const char *name;
    int sym_index, index;
    ElfW(Sym) *sym, *esym;
...
static void bind_libs_dynsyms(MKCCState *s1)
{
    const char *name;
    int sym_index;
    ElfW(Sym) *sym, *esym;
    ...
    for_each_elem(s1->dynsymtab_section, 1, esym, ElfW(Sym)) {
        ...
        if (sym_index) {
            ...
            if (ELFW(ST_BIND)(esym->st_info) != STB_WEAK)
                mkcc_warning("undefined dynamic symbol '%s'", name);
        }
    }
}

/* Export all non local symbols (for shared libraries) */
static void export_global_syms(MKCCState *s1)
{
    ...
    s1->symtab_to_dynsym = mkcc_mallocz(sizeof(int) * nb_syms);
    for_each_elem(symtab_section, 1, sym, ElfW(Sym)) {
        if (ELFW(ST_BIND)(sym->st_info) != STB_LOCAL) {
	    name = (char *) symtab_section->link->data + sym->st_name;
...
ST_FUNC void relocate_plt(MKCCState *s1)
{
    uint8_t *p, *p_end;
    ...
    if (p < p_end) {
        ...
        if ((off + ((uint32_t)1 << 20)) >> 21)
            mkcc_error("Failed relocating PLT (off=0x%lx, got=0x%lx, plt=0x%lx)", off, got, plt);
        ...
        while (p < p_end) {
            ...
            if ((off + ((uint32_t)1 << 20)) >> 21)
                mkcc_error("Failed relocating PLT (off=0x%lx, addr=0x%lx, pc=0x%lx)", off, addr, pc);
...
static void alloc_sec_names(MKCCState *s1, int file_type, Section *strsec)
{
    int i;
    Section *s;
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        if (file_type == MKCC_OUTPUT_DLL &&
            ...
            file_type == MKCC_OUTPUT_OBJ ||
            file_type == MKCC_OUTPUT_EXE ||
            (s->sh_flags & SHF_ALLOC) ||
            i == (s1->nb_sections - 1)) {
...
static int layout_sections(MKCCState *s1, ElfW(Phdr) *phdr, int phnum,
                           ...
                           struct dyn_inf *dyninf, int *sec_order)
{
    ...
    sh_order_index = 1;
    file_offset = 0;
    if (s1->output_format == MKCC_OUTPUT_FORMAT_ELF)
...
/* Fill the dynamic section with tags describing the address and size of
   sections */
static void fill_dynamic(MKCCState *s1, struct dyn_inf *dyninf)
{
    Section *dynamic;
...
/* Relocate remaining sections and symbols (that is those not related to
   dynamic linking) */
static int final_sections_reloc(MKCCState *s1)
{
    int i;
    Section *s;
...
static void mkcc_output_elf(MKCCState *s1, FILE *f, int phnum, ElfW(Phdr) *phdr,
                           int file_offset, int *sec_order)
{
    ...
    if (file_type == MKCC_OUTPUT_EXE || file_type == MKCC_OUTPUT_DLL)
    ...
    switch(file_type) {
    default:
    case MKCC_OUTPUT_EXE:
        ehdr.e_type = ET_EXEC;
        ehdr.e_entry = get_elf_sym_addr(s1, "_start", 1);
        break;
    case MKCC_OUTPUT_DLL:
        ehdr.e_type = ET_DYN;
        ehdr.e_entry = text_section->sh_addr; /* XXX: is it correct ? */
        break;
    case MKCC_OUTPUT_OBJ:
        ehdr.e_type = ET_REL;
        break;
    }
    ehdr.e_machine = EM_MKCC_TARGET;
...
static int mkcc_write_elf_file(MKCCState *s1, const char *filename, int phnum,
                              ElfW(Phdr) *phdr, int file_offset, int *sec_order)
{
    ...
    if (fd < 0) {
        mkcc_error_noabort("could not write '%s'", filename);
        return -1;
    }
    f = fdopen(fd, "wb");
    if (s1->verbose)
    ...
    if (s1->output_format == MKCC_OUTPUT_FORMAT_COFF)
        mkcc_output_coff(s1, f);
    else
#endif
    if (s1->output_format == MKCC_OUTPUT_FORMAT_ELF)
        mkcc_output_elf(s1, f, phnum, phdr, file_offset, sec_order);
    else
        mkcc_output_binary(s1, f, sec_order);
    fclose(f);
...
static int elf_output_file(MKCCState *s1, const char *filename)
{
    ...
    if (file_type != MKCC_OUTPUT_OBJ) {
        ...
        mkcc_add_linker_symbols(s1);

        if (!s1->static_link) {
            if (file_type == MKCC_OUTPUT_EXE) {
                char *ptr;
    ...
    sec_order = mkcc_malloc(sizeof(int) * shnum);
    sec_order[0] = 0;
    ...
    phdr = mkcc_mallocz(phnum * sizeof(ElfW(Phdr)));
    ...
    /* Perform relocation to GOT or PLT entries */
    if (file_type == MKCC_OUTPUT_EXE && s1->static_link)
        fill_got(s1);

    /* Create the ELF file with name 'filename' */
    ret = mkcc_write_elf_file(s1, filename, phnum, phdr, file_offset, sec_order);
    if (s1->do_strip) {
	int rc;
...
ST_FUNC int mkcc_load_object_file(MKCCState *s1,
                                int fd, unsigned long file_offset)
{
    ...
    if (ehdr.e_ident[5] != ELFDATA2LSB ||
        ehdr.e_machine != EM_MKCC_TARGET) {
    fail1:
        mkcc_error_noabort("invalid object file");
        return -1;
    }
    /* read sections */
    shdr = load_data(fd, file_offset + ehdr.e_shoff,
                     sizeof(ElfW(Shdr)) * ehdr.e_shnum);
    sm_table = mkcc_mallocz(sizeof(SectionMergeInfo) * ehdr.e_shnum);
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        if (sh->sh_type == SHT_SYMTAB) {
            if (symtab) {
                mkcc_error_noabort("object must contain only one symtab");
            fail:
                ret = -1;
                goto the_end;
            }
    ...
    found:
        if (sh->sh_type != s->sh_type) {
            mkcc_error_noabort("invalid section type");
            goto fail;
        }
    ...
    /* resolve symbols */
    old_to_new_syms = mkcc_mallocz(nb_syms * sizeof(int));
    ...
    for(i = 1; i < ehdr.e_shnum; i++) {
        ...
        case SHT_RELX:
            ...
            for_each_elem(s, (offset / sizeof(*rel)), rel, ElfW_Rel) {
                ...
                invalid_reloc:
                    mkcc_error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x",
...
static int mkcc_load_alacarte(MKCCState *s1, int fd, int size)
{
    ...
    do {
        ...
        for(p = ar_names, i = 0; i < nsyms; i++, p += strlen(p)+1) {
            ...
            load_obj:
                    ...
                    if(mkcc_load_object_file(s1, fd, off) < 0) {
                    fail:
                        ret = -1;
                        goto the_end;
                    }
...
/* load a '.a' file */
ST_FUNC int mkcc_load_archive(MKCCState *s1, int fd)
{
    ArchiveHeader hdr;
    char ar_size[11];
    char ar_name[17];
    ...
    for(;;) {
        ...
        if (len == 0)
            break;
        if (len != sizeof(hdr)) {
            mkcc_error_noabort("invalid archive");
            return -1;
        }
        ...
        if (!strcmp(ar_name, "/")) {
            ...
            if(s1->alacarte_link)
                return mkcc_load_alacarte(s1, fd, size);
            ...
            if (mkcc_load_object_file(s1, fd, file_offset) < 0)
                return -1;
        }
...
ST_FUNC int mkcc_load_dll(MKCCState *s1, int fd, const char *filename, int level)
{
    ElfW(Ehdr) ehdr;
    ...
    if (ehdr.e_ident[5] != ELFDATA2LSB ||
        ehdr.e_machine != EM_MKCC_TARGET) {
        mkcc_error_noabort("bad architecture");
        return -1;
    }
    ...
    dllref = mkcc_mallocz(sizeof(DLLReference) + strlen(soname));
    dllref->level = level;
    ...
    for(i = 0, dt = dynamic; i < nb_dts; i++, dt++) {
        ...
        case DT_NEEDED:
            ...
            if (mkcc_add_dll(s1, name, AFF_REFERENCED_DLL) < 0) {
                mkcc_error_noabort("referenced dll '%s' not found", name);
                ret = -1;
                goto the_end;
            }
        already_loaded:
...
static int ld_next(MKCCState *s1, char *name, int name_size)
{
    int c;
    char *q;
...
static int ld_add_file(MKCCState *s1, const char filename[])
{
    int ret;

    ret = mkcc_add_file_internal(s1, filename, 0, MKCC_FILETYPE_BINARY);
    if (ret)
        ret = mkcc_add_dll(s1, filename, 0);
    return ret;
}
...
static int ld_add_file_list(MKCCState *s1, const char *cmd, int as_needed)
{
    ...
    for(;;) {
        libname[0] = '\0';
        if (t == LD_TOK_EOF) {
            mkcc_error_noabort("unexpected end of file");
            ret = -1;
            ...
            if ((t != LD_TOK_NAME) || (filename[0] != 'l')) {
                mkcc_error_noabort("library name expected");
                ret = -1;
                goto lib_parse_error;
            }
        ...
        if (!strcmp(filename, "AS_NEEDED")) {
            ...
            if (!as_needed) {
                ...
                if (group) {
                    ...
                    dynarray_add((void ***) &libs, &nblibs, mkcc_strdup(filename));
                    if (libname[0] != '\0')
                        dynarray_add((void ***) &libs, &nblibs, mkcc_strdup(libname));
                }
            }
        }
        t = ld_next(s1, filename, sizeof(filename));
...
/* interpret a subset of GNU ldscripts to handle the dummy libc.so
   files */
ST_FUNC int mkcc_load_ldscript(MKCCState *s1)
{
    char cmd[64];
    char filename[1024];
    int t, ret;
