...
#if defined(MKCC_ARM_EABI) && !defined(MKCC_ARM_VFP)
...
#ifndef MKCC_ARM_VERSION
# define MKCC_ARM_VERSION 5
#endif
...
#ifndef MKCC_ARM_HARDFLOAT
    mkcc_warning("soft float ABI currently not supported: default to softfp");
#endif
}
#else
...
ST_FUNC void arm_init(struct MKCCState *s)
{
#if !defined (MKCC_ARM_VFP)
    mkcc_warning("Support for FPA is deprecated and will be removed in next"
                " release");
#endif
#if !defined (MKCC_ARM_EABI)
    mkcc_warning("Support for OABI is deprecated and will be removed in next"
                " release");
#endif
}
#endif
...
char *default_elfinterp(struct MKCCState *s)
{
...
void o(uint32_t i)
{
  ...
  ind1 = ind + 4;
  if (!cur_text_section)
    mkcc_error("compiler error! This happens f.ex. if the compiler\n"
...
ST_FUNC uint32_t encbranch(int pos, int addr, int fail)
{
  ...
  if(addr>=0x1000000 || addr<-0x1000000) {
    if(fail)
      mkcc_error("FIXME: function bigger than 32MB");
    return 0;
  }
...
static uint32_t vfpr(int r)
{
  if(r<TREG_F0 || r>TREG_F7)
    mkcc_error("compiler error! register %i is no vfp register",r);
  return r-5;
}
#else
static uint32_t fpr(int r)
{
  if(r<TREG_F0 || r>TREG_F3)
    mkcc_error("compiler error! register %i is no fpa register",r);
  return r-5;
}
#endif
...
static uint32_t intr(int r)
{
  if(r==4)
    return 12;
  if((r<0 || r>4) && r!=14)
    mkcc_error("compiler error! register %i is no int register",r);
  return r;
}
...
static uint32_t mapcc(int cc)
{
  ...
  mkcc_error("unexpected condition code");
...
void store(int r, SValue *sv)
{
  ...
  mkcc_error("store unimplemented");
}
...
/* Assign parameters to registers and stack with alignment according to the
   ...
   Note: this function allocated an array in plan->pplans with mkcc_malloc. It
...
static int assign_regs(int nb_args, int float_abi, struct plan *plan, int *todo)
{
  ...
  ncrn = nsaa = 0;
  *todo = 0;
  plan->pplans = mkcc_malloc(nb_args * sizeof(*plan->pplans));
...
void gen_opi(int op)
{
  ...
  switch(c) {
    ...
    case 3:
      ...
      break;
    default:
      mkcc_error("gen_opi %i unimplemented!",op);
  }
}
...
/* bug (intention?) in Linux FPU emulator
   doesn't set carry if equal */
	switch(op) {
	  ...
	  case TOK_ULE:
	  case TOK_UGT:
            mkcc_error("unsigned comparison on floats?");
	    break;
	  case TOK_LT:
            op=TOK_Nset;
	    break;
...
ST_FUNC void gen_cvt_itof1(int t)
{
  ...
  mkcc_error("unimplemented gen_cvt_itof %x!",vtop->type.t);
}
...
void gen_cvt_ftoi(int t)
{
  ...
  mkcc_error("unimplemented gen_cvt_ftoi!");
}
...
ST_FUNC void gen_vla_sp_save(int addr) {
    mkcc_error("variable length arrays unsupported for this target");
}
