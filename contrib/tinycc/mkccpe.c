/*
 *  MKCCPE.C - PE file output for the Tiny C Compiler
 *
...
static const char *pe_export_name(MKCCState *s1, ElfW(Sym) *sym)
{
...
static int pe_find_import(MKCCState * s1, ElfW(Sym) *sym)
{
    char buffer[200];
    const char *s, *p;
    int sym_index = 0, n = 0;
...
#define CHARACTERISTICS_DLL 0x230F
    ...
    if (NULL == op) {
        mkcc_error_noabort("could not write '%s': %s", pe->filename, strerror(errno));
        return -1;
    }
        ...
        for (k = 0, n = p->sym_count; k <= n; ++k) {
            if (k < n) {
                ...
                if (dllref)
                    ...
                    ordinal = 0, v = imp_sym->st_value; /* address from mkcc_add_symbol() */
                ...
                if (pe->type == PE_RUN) {
                    ...
                    if (!v)
                        mkcc_error_noabort("can't build symbol '%s'", name);
                } else
#endif
                if (ordinal) {
...
static void pe_build_exports(struct pe_info *pe)
{
    ...
    pe_align_section(pe->thunk, 16);
    dllname = mkcc_basename(pe->filename);
    ...
    strcpy(mkcc_fileextension(buf), ".def");
    op = fopen(buf, "w");
    if (NULL == op) {
        mkcc_error_noabort("could not create '%s': %s", buf, strerror(errno));
    } else {
...
static int pe_assign_addresses (struct pe_info *pe)
{
    ...
    section_order = mkcc_malloc(pe->s1->nb_sections * sizeof (int));
    ...
    pe->sec_info = mkcc_mallocz(o * sizeof (struct section_info));
    addr = pe->imagebase + 1;
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index) {
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (type == STT_FUNC) {
                ...
                /* mkcc_realloc might have altered sym's address */
                sym = (ElfW(Sym) *)symtab_section->data + sym_index;
...
static void pe_print_sections(MKCCState *s1, const char *fname)
{
    Section *s;
    FILE *f;
    int i;
...
static int add_dllref(MKCCState *s1, const char *dllname)
{
    DLLReference *dllref;
    int i;
    for (i = 0; i < s1->nb_loaded_dlls; ++i)
        if (0 == strcmp(s1->loaded_dlls[i]->name, dllname))
            return i + 1;
    dllref = mkcc_mallocz(sizeof(DLLReference) + strlen(dllname));
...
static int pe_load_res(MKCCState *s1, int fd)
{
...
/* ------------------------------------------------------------- */
static int pe_load_def(MKCCState *s1, int fd)
{
...
static int pe_load_dll(MKCCState *s1, const char *dllname, int fd)
{
    char *p, *q;
    int index;
...
ST_FUNC int pe_load_file(struct MKCCState *s1, const char *filename, int fd)
{
    int ret = -1;
    char buf[10];
    if (0 == strcmp(mkcc_fileextension(filename), ".def"))
        ret = pe_load_def(s1, fd);
    else if (pe_load_res(s1, fd) == 0)
        ret = 0;
    else if (read_mem(fd, 0, buf, sizeof buf) && 0 == strncmp(buf, "MZ", 2))
        ret = pe_load_dll(s1, mkcc_basename(filename), fd);
    return ret;
}

/* ------------------------------------------------------------- */
#ifdef MKCC_TARGET_X86_64
static unsigned pe_add_uwwind_info(MKCCState *s1)
{
    if (NULL == s1->uw_pdata) {
        s1->uw_pdata = find_section(mkcc_state, ".pdata");
...
static void pe_add_runtime(MKCCState *s1, struct pe_info *pe)
{
    ...
    if (MKCC_OUTPUT_DLL == s1->output_type) {
        pe_type = PE_DLL;
        /* need this for 'mkccelf.c:relocate_section()' */
        s1->output_type = MKCC_OUTPUT_EXE;
    }
    else
        pe_type = PE_EXE;

    start_symbol =
        MKCC_OUTPUT_MEMORY == s1->output_type
    ...
    /* grab the startup code from libmkcc1 */
    if (MKCC_OUTPUT_MEMORY != s1->output_type || PE_GUI == pe_type)
    ...
    if (0 == s1->nostdlib) {
        static const char *libs[] = {
            "mkcc1", "msvcrt", "kernel32", "", "user32", "gdi32", NULL
        };
        const char **pp, *p;
        for (pp = libs; 0 != (p = *pp); ++pp) {
            if (0 == *p) {
                if (PE_DLL != pe_type && PE_GUI != pe_type)
                    break;
            } else if (mkcc_add_library_err(s1, p) < 0) {
                break;
            }
        }
    }

    if (MKCC_OUTPUT_MEMORY == s1->output_type) {
        pe_type = PE_RUN;
#ifdef MKCC_IS_NATIVE
        s1->runtime_main = start_symbol;
#endif
    } else {
        pe->start_addr = (DWORD)(uintptr_t)mkcc_get_symbol_err(s1, start_symbol);
    }
