...
{
    if (pvtop != vtop)
        mkcc_error("internal compiler error: vstack leak (%d)", vtop - pvtop);
}
...
static Sym *__sym_malloc(void)
{
    ...
    sym_pool = mkcc_malloc(SYM_POOL_NB * sizeof(Sym));
...
static Sym *external_sym(int v, CType *type, int r)
{
    ...
    if (!s) {
        ...
        mkcc_error("incompatible types for redefinition of '%s'", 
              get_tok_str(v, NULL));
    }
...
#if defined(MKCC_TARGET_ARM) && !defined(MKCC_ARM_VFP)
            CValue check;
#endif
...
#elif defined(MKCC_TARGET_C67) || defined(MKCC_TARGET_ARM64)
                mkcc_error("not implemented");
#else
#error not supported
#endif
            }
...
static void gen_opic(int op)
{
    ...
    if (c1 && c2) {
        ...
        case TOK_UMOD:
            ...
            if (l2 == 0) {
                if (const_wanted)
                    mkcc_error("division by zero in constant");
                goto general_case;
            }
            switch(op) {
...
static void check_comparison_pointer_types(SValue *p1, SValue *p2, int op)
{
    ...
    if ((is_integer_btype(bt1) || is_integer_btype(bt2)) && op != '-') {
        if (op != TOK_LOR && op != TOK_LAND )
            mkcc_warning("comparison between pointer and integer");
        return;
    }
    ...
    invalid_operands:
        mkcc_error("invalid operands to binary %s", get_tok_str(op, NULL));
    }
    ...
    if (!is_compatible_types(&tmp_type1, &tmp_type2)) {
        ...
        if (op == '-')
            ...
            mkcc_warning("comparison of distinct pointer types lacks a cast");
    }
}
...
ST_FUNC void gen_op(int op)
{
    ...
    if (bt1 == VT_STRUCT || bt2 == VT_STRUCT) {
        mkcc_error("operation on a struct");
        ...
        if (bt1 == VT_PTR && bt2 == VT_PTR) {
            if (op != '-')
                mkcc_error("cannot use pointers here");
            check_comparison_pointer_types(vtop - 1, vtop, op);
            ...
            if (vtop[-1].type.t & VT_VLA)
                ...
                if (u < 0)
                    mkcc_error("unknown array element size");
#if defined(MKCC_TARGET_ARM64) || defined(MKCC_TARGET_X86_64)
                vpushll(u);
#else
...
/* #ifdef CONFIG_MKCC_BCHECK
    ...
    */
            ...
            if (mkcc_state->do_bounds_check && !const_wanted) {
        ...
        if (op != '+' && op != '-' && op != '*' && op != '/' &&
            ...
            mkcc_error("invalid operands for binary operation");
        goto std_op;
...
#error
                ...
                if (sbt == VT_PTR) {
                    vtop->type.t = VT_INT;
                    mkcc_warning("nonportable conversion from pointer to char/short");
                }
...
ST_FUNC int type_size(CType *type, int *a) /* A return type of 'int' is too small for 64 bits size_t values. JCB */
{
    Sym *s;
    int bt;
...
static void gen_assign_cast(CType *dt)
{
	...
	if (sbt == VT_VOID && dbt == VT_VOID)
		...
		void func2() {
	      ...
	      */
    	    ...
    	    mkcc_error("cannot cast from/to void");
    }
    if (dt->t & VT_CONSTANT)
        mkcc_warning("assignment of read-only location");
    switch(dbt) {
    case VT_PTR:
        ...
        if (is_integer_btype(sbt)) {
            mkcc_warning("assignment makes pointer from integer without a cast");
            goto type_ok;
        }
        ...
        if (sbt == VT_FUNC) {
            if ((type1->t & VT_BTYPE) != VT_VOID &&
                ...
                mkcc_warning("assignment from incompatible pointer type");
            goto type_ok;
        }
        if (sbt != VT_PTR)
            goto error;
        ...
        if ((!(type1->t & VT_CONSTANT) && (type2->t & VT_CONSTANT)) ||
            ...
            mkcc_warning("assignment discards qualifiers from pointer target type");
        break;
    case VT_BYTE:
    case VT_SHORT:
    case VT_INT:
    case VT_LLONG:
        if (sbt == VT_PTR || sbt == VT_FUNC) {
            mkcc_warning("assignment makes integer from pointer without a cast");
    ...
    case_VT_STRUCT:
        ...
        error:
            ...
            mkcc_error("cannot cast '%s' to '%s'", buf1, buf2);
        }
        break;
    }
 type_ok:
...
static void parse_attribute(AttributeDef *ad)
{
    ...
    while (tok != ')') {
        ...
        case TOK_SECTION2:
            ...
            if (tok != TOK_STR)
                expect("section name");
            ad->section = find_section(mkcc_state, (char *)tokc.str.data);
            next();
            skip(')');
            break;
        case TOK_ALIAS1:
	...
	case TOK_VISIBILITY2:
        ...
        case TOK_ALIGNED2:
            if (tok == '(') {
                ...
                n = expr_const();
                if (n <= 0 || (n & (n - 1)) != 0) 
                    mkcc_error("alignment must be a positive power of two");
                skip(')');
            } else {
                n = MAX_ALIGN;
            }
        ...
        case TOK_UNUSED1:
        case TOK_UNUSED2:
            /* currently, no need to handle it because mkcc does not
        ...
        case TOK_MODE:
            ...
            switch(tok) {
                ...
                case TOK_MODE_SI:
                    ...
                    break;
                default:
                    mkcc_warning("__mode__(%s) not supported\n", get_tok_str(tok, NULL));
                    break;
            }
            next();
            skip(')');
        ...
        case TOK_DLLIMPORT:
            ...
            break;
        default:
            if (mkcc_state->warn_unsupported)
                mkcc_warning("'%s' attribute ignored", get_tok_str(t, NULL));
            /* skip parameters */
            if (tok == '(') {
                int parenthesis = 0;
                do {
...
static void struct_decl(CType *type, AttributeDef *ad, int u)
{
    ...
    if (tok != '{') {
        ...
        if (s && (s->scope == local_scope || (tok != '{' && tok != ';'))) {
            if (s->type.t != a)
                mkcc_error("redefinition of '%s'", get_tok_str(v, NULL));
            goto do_decl;
        }
    } else {
        v = anon_sym++;
 ...
 do_decl:
    ...
    if (tok == '{') {
        next();
        if (s->c != -1)
            mkcc_error("struct/union/enum already defined");
        /* cannot be empty */
        c = 0;
        ...
        if (a == TOK_ENUM) {
            for(;;) {
                ...
                ss = sym_find(v);
                if (ss && !local_stack)
                    mkcc_error("redefinition of enumerator '%s'",
                              get_tok_str(v, NULL));
                next();
                if (tok == '=') {
                    next();
            ...
            while (tok != '}') {
                ...
                while (1) {
		    if (flexible)
		        mkcc_error("flexible array member '%s' not at the end of struct",
                              get_tok_str(v, NULL));
                    bit_size = -1;
                    v = 0;
                    type1 = btype;
			    ...
			    if ((a == TOK_STRUCT) && (type1.t & VT_ARRAY) && c)
			        ...
			        mkcc_error("field '%s' has incomplete type",
                                      get_tok_str(v, NULL));
                        }
                        if ((type1.t & VT_BTYPE) == VT_FUNC ||
                            (type1.t & (VT_TYPEDEF | VT_STATIC | VT_EXTERN | VT_INLINE)))
                            mkcc_error("invalid type for '%s'", 
                                  get_tok_str(v, NULL));
                    }
                    if (tok == ':') {
                        next();
                        ...
                        if (bit_size < 0)
                            mkcc_error("negative width in bit-field '%s'", 
                                  get_tok_str(v, NULL));
                        if (v && bit_size == 0)
                            mkcc_error("zero width for bit-field '%s'", 
                                  get_tok_str(v, NULL));
                    }
                    ...
                    if (ad1.a.aligned) {
                        ...
                        align = 1;
                    } else if (*mkcc_state->pack_stack_ptr) {
                        if (align > *mkcc_state->pack_stack_ptr)
                            align = *mkcc_state->pack_stack_ptr;
                    }
                    lbit_pos = 0;
                    if (bit_size >= 0) {
                        bt = type1.t & VT_BTYPE;
                        if (bt != VT_INT && 
                            ...
                            bt != VT_ENUM &&
                            bt != VT_LLONG)
                            mkcc_error("bitfields must have scalar type");
                        bsize = size * 8;
                        if (bit_size > bsize) {
                            mkcc_error("width of '%s' exceeds its type",
                                  get_tok_str(v, NULL));
...
static int parse_btype(CType *type, AttributeDef *ad)
{
    ...
    while(1) {
        ...
        basic_type1:
            if (complete)
                mkcc_error("too many basic types");
            t |= u;
        ...
        case TOK_SIGNED3:
            if ((t & (VT_DEFSIGN|VT_UNSIGNED)) == (VT_DEFSIGN|VT_UNSIGNED))
                mkcc_error("signed and unsigned modifier");
            typespec_found = 1;
            t |= VT_DEFSIGN;
            next();
            break;
...
the_end:
    if (mkcc_state->char_is_unsigned) {
    ...
    if ((t & VT_BTYPE) == VT_LONG)
#if (!defined MKCC_TARGET_X86_64 && !defined MKCC_TARGET_ARM64) || \
    defined MKCC_TARGET_PE
        t = (t & ~VT_BTYPE) | VT_INT;
#else
        t = (t & ~VT_BTYPE) | VT_LLONG;
#endif
    ...
    if (tok == '(') {
            ...
            for(;;) {
                ...
                if (l != FUNC_OLD) {
                    ...
                    if ((pt.t & VT_BTYPE) == VT_VOID)
                        mkcc_error("parameter declared as void");
        ...
        if (tok != ']') {
            ...
            if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST) {
                n = vtop->c.i;
                if (n < 0)
                    mkcc_error("invalid array size");
            } else {
                if (!is_integer_btype(vtop->type.t & VT_BTYPE))
                    mkcc_error("size of variable length array should be an integer");
                t1 = VT_VLA;
            }
        }
        skip(']');
        ...
        post_type(type, ad);
        if (type->t == VT_FUNC)
            mkcc_error("declaration of an array of functions");
        t1 |= type->t & VT_VLA;
...
static void gfunc_param_typed(Sym *func, Sym *arg)
{
    ...
    if (func_type == FUNC_OLD ||
        ...
        mkcc_error("too many arguments to function");
    } else {
        type = arg->type;
...
ST_FUNC void unary(void)
{
 ...
 tok_next:
    ...
    case TOK_STR:
        ...
        t = VT_BYTE;
    str_init:
        if (mkcc_state->warn_write_strings)
            t |= VT_CONSTANT;
        type.t = t;
        mk_pointer(&type);
        type.t |= VT_ARRAY;
    ...
    case '+':
        ...
        unary();
        if ((vtop->type.t & VT_BTYPE) == VT_PTR)
            mkcc_error("pointer not accepted for unary plus");
    ...
    case TOK_ALIGNOF2:
        ...
        if (t == TOK_SIZEOF) {
            if (!(type.t & VT_VLA)) {
                if (size < 0)
                    mkcc_error("sizeof applied to an incomplete type");
                vpushs(size);
            } else {
                vla_runtime_type_size(&type, &align);
            }
    ...
    case TOK_builtin_return_address:
        {
            ...
            skip('(');
            if (tok != TOK_CINT) {
                mkcc_error("%s only takes positive integers",
    ...
    case TOK_builtin_va_start:
        {
            ...
            if ((vtop->r & VT_VALMASK) != VT_LOCAL)
                mkcc_error("__builtin_va_start expects a local variable");
    ...
    case TOK___va_start: {
        if (nocode_wanted)
            mkcc_error("statement in global scope");
        next();
        skip('(');
        expr_eq();
        skip(',');
    ...
    tok_identifier:
        ...
        if (!s) {
            ...
            if (tok != '(')
                mkcc_error("'%s' undeclared", name);
            /* for simple function calls, we tolerate undeclared
               external reference to int() function */
            if (mkcc_state->warn_implicit_function_declaration
#ifdef MKCC_TARGET_PE
                ...
                mkcc_warning("implicit declaration of function '%s'", name);
    ...
    while (1) {
        if (tok == TOK_INC || tok == TOK_DEC) {
            ...
            if (!s)
                mkcc_error("field not found: %s",  get_tok_str(tok & ~SYM_FIELD, &tokc));
            ...
            if (sa)
                mkcc_error("too few arguments to function");
            skip(')');
            if (!nocode_wanted) {
                gfunc_call(nb_args);
            } else {
...
static void block(int *bsym, int *csym, int is_expr)
{
        ...
        if(is_expr) {
            ...
            case VT_FUNC:
                for(p=vtop->type.ref;p;p=p->prev)
                    if(p->prev==s)
                        mkcc_error("unsupported expression type");
            }
        }
    ...
    if (tok == TOK_SWITCH) {
        ...
        for (b = 0; b < sw.n; b++) {
            int v = sw.p[b]->v1;
            if (b && v <= d)
                mkcc_error("duplicate case value");
            d = sw.p[b]->v2;
    ...
    if (tok == TOK_CASE) {
        struct case_t *cr = mkcc_malloc(sizeof(struct case_t));
        if (!cur_switch)
            expect("switch");
        next();
    ...
    if (tok == TOK_GOTO) {
        ...
        if (b) {
            ...
            if (s) {
                if (s->r == LABEL_DEFINED)
                    mkcc_error("duplicate label '%s'", get_tok_str(s->v, NULL));
                gsym(s->jnext);
                s->r = LABEL_DEFINED;
            } else {
        ...
        block_after_label:
            if (tok == '}') {
                mkcc_warning("deprecated use of label at end of compound statement");
            } else {
                if (is_expr)
                    vpop();
                block(bsym, csym, is_expr);
                            ...
                            int size_only)
{
    ...
    if (notfirst) {
        ...
        if (type->t & VT_ARRAY) {
            ...
            f = *cur_field;
            if (!f)
                mkcc_error("too many field init");
            /* XXX: fix this mess by using explicit storage field */
            type1 = f->type;
            type1.t |= (type->t & ~VT_TYPE);
            type = &type1;
    ...
    if (!size_only && nb_elems > 1) {
        ...
        if (!sec)
            mkcc_error("range init not supported yet for dynamic storage");
                      ...
                      int v, int expr_type)
{
    ...
    case EXPR_CONST:
        ...
        if ((vtop->r & (VT_VALMASK | VT_LVAL)) != VT_CONST)
            mkcc_error("initializer element is not constant");
        break;
    case EXPR_ANY:
        expr_eq();
        break;
    ...
    if (sec) {
        ...
        if ((vtop->r & VT_SYM) &&
            ...
            mkcc_error("initializer element is not computable at load time");
        switch(bt) {
                             ...
                             int first, int size_only)
{
    ...
    if (type->t & VT_VLA) {
        ...
        if ((first && tok != TOK_LSTR && tok != TOK_STR) || 
            tok == '{') {
            if (tok != '{')
                mkcc_error("character array initializer must be a literal,"
        ...
        if ((tok == TOK_LSTR && 
            ...
            while (tok == TOK_STR || tok == TOK_LSTR) {
                ...
                if (!size_only) {
                    if (cstr_len > nb)
                        mkcc_warning("initializer-string for array is too long");
        ...
        if (tok == '(') {
            ...
            CType type1;
            next();
	    if (mkcc_state->old_struct_init_code) {
		...
		if (!is_assignable_types(type, &type1))
            	    mkcc_error("invalid type for cast");
		#endif
		skip(')');
    	    }
    	    else
                                   ...
                                   int has_init, int v, int scope)
{
    ...
    if (size < 0 || (flexible_array && has_init)) {
        ...
        if (has_init == 2) {
            ...
            while (level > 0 || (tok != ',' && tok != ';')) {
                if (tok < 0)
                    mkcc_error("unexpected end of file in initializer");
                tok_str_add_tok(&init_str);
                if (tok == '{')
                    level++;
                else if (tok == '}') {
    ...
    if ((r & VT_VALMASK) == VT_LOCAL) {
        sec = NULL;
#ifdef CONFIG_MKCC_BCHECK
        if (mkcc_state->do_bounds_check && (type->t & VT_ARRAY)) {
            loc--;
        }
#endif
        loc = (loc - size) & -align;
    ...
    while (tok != '{' && tok != ';' && tok != ',' && tok != TOK_EOF &&
        ...
        if (((btype.t & VT_BTYPE) == VT_ENUM ||
            ...
            for(;;) {
                ...
                mkcc_error("declaration for parameter '%s' but no such parameter",
                      get_tok_str(v, NULL));
            found:
                /* check that no storage specifier except 'register' was given */
                if (type.t & VT_STORAGE)
                    mkcc_error("storage class specified for '%s'", get_tok_str(v, NULL)); 
...
#ifdef CONFIG_MKCC_BCHECK
    if (mkcc_state->do_bounds_check && !strcmp(funcname, "main")) {
        int i;
        Sym *sym;
...
ST_FUNC void gen_inline_functions(void)
{
    ...
    for(;;) {
        inline_generated = 0;
        for (i = 0; i < mkcc_state->nb_inline_fns; ++i) {
            fn = mkcc_state->inline_fns[i];
            sym = fn->sym;
            if (sym && sym->c) {
    ...
    dynarray_reset(&mkcc_state->inline_fns, &mkcc_state->nb_inline_fns);
}
...
static int decl0(int l, int is_for_loop_init)
{
    ...
    while (1) {
		...
		int v = btype.ref->v;
		if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) >= SYM_FIRST_ANOM)
        	    mkcc_warning("unnamed struct/union that defines no instances");
	    }
            next();
            continue;
        }
        while (1) { /* iterate thru each declaration */
            ...
            if ((type.t & VT_BTYPE) == VT_FUNC) {
                if ((type.t & VT_STATIC) && (l == VT_LOCAL)) {
                    mkcc_error("function without file scope cannot be static");
                }
	    ...
	    type.t |= ad.a.visibility << VT_VIS_SHIFT;
            ...
            if (tok == '{') {
                if (l == VT_LOCAL)
                    mkcc_error("cannot use local functions");
                if ((type.t & VT_BTYPE) != VT_FUNC)
		    ...
		    if (! (type.t & VT_VIS_MASK))
                ...
                if ((type.t & (VT_INLINE | VT_STATIC)) == 
                    ...
                    fn = mkcc_malloc(sizeof *fn + strlen(filename));
                    ...
                    for(;;) {
                        int t;
                        if (tok == TOK_EOF)
                            mkcc_error("unexpected end of file");
                    ...
                    dynarray_add((void ***)&mkcc_state->inline_fns, &mkcc_state->nb_inline_fns, fn);
                ...
                if (btype.t & VT_TYPEDEF) {
                    ...
                    if (sym && sym->scope == local_scope) {
                        if (!is_compatible_types(&sym->type, &type)
                            ...
                            mkcc_error("incompatible redefinition of '%s'",
                                get_tok_str(v, NULL));
                        sym->type = type;
                    } else {
                    ...
                    has_init = (tok == '=');
                    if (has_init && (type.t & VT_VLA))
                        mkcc_error("Variable length array cannot be initialized");
                    if ((btype.t & VT_EXTERN) || ((type.t & VT_BTYPE) == VT_FUNC) ||
                        ...
                        if (ad.alias_target) {
                            ...
                            if (!alias_target || !alias_target->c)
                                mkcc_error("unsupported forward __alias__ attribute");
