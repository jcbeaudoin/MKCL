/*
 *  MKCC - Tiny C Compiler - Support for -run switch
 *
...
static int mkcc_relocate_ex(MKCCState *s1, void *ptr);

#ifdef _WIN64
static void win64_add_function_table(MKCCState *s1);
#endif
...
LIBMKCCAPI int mkcc_relocate(MKCCState *s1, void *ptr)
{
    ...
    if (MKCC_RELOCATE_AUTO != ptr)
        return mkcc_relocate_ex(s1, ptr);

    ret = mkcc_relocate_ex(s1, NULL);
    if (ret < 0)
        return ret;
    ...
    {   /* Use mmap instead of malloc for Selinux.  Ref:
        ...
        char tmpfname[] = "/tmp/.mkccrunXXXXXX";
        ...
        if (s1->write_mem == MAP_FAILED)
            mkcc_error("/tmp not writeable");

        s1->runtime_mem = mmap (NULL, ret, PROT_READ|PROT_EXEC,
            MAP_SHARED, fd, 0);
        if (s1->runtime_mem == MAP_FAILED)
            mkcc_error("/tmp not executable");

        ret = mkcc_relocate_ex(s1, s1->write_mem);
    }
#else
    s1->runtime_mem = mkcc_malloc(ret);
    ret = mkcc_relocate_ex(s1, s1->runtime_mem);
#endif
    return ret;
}

/* launch the compiled program with the given arguments */
LIBMKCCAPI int mkcc_run(MKCCState *s1, int argc, char **argv)
{
    int (*prog_main)(int, char **);
    int ret;

    if (mkcc_relocate(s1, MKCC_RELOCATE_AUTO) < 0)
        return -1;

    prog_main = mkcc_get_symbol_err(s1, s1->runtime_main);
    ...
    if (s1->do_bounds_check) {
        ...
        /* set error function */
# ifdef CONFIG_MKCC_BACKTRACE
        rt_bound_error_msg = mkcc_get_symbol_err(s1, "__mkcc_bound_error_msg");
# endif
        /* XXX: use .init section so that it also work in binary ? */
        bound_init = mkcc_get_symbol_err(s1, "__mkcc_bound_init");
        bound_exit = mkcc_get_symbol_err(s1, "__mkcc_bound_exit");
        bound_new_region = mkcc_get_symbol_err(s1, "__mkcc_bound_new_region");
        bound_delete_region = mkcc_get_symbol_err(s1, "__mkcc_bound_delete_region");
        bound_init();
...
static void set_exception_handler(void)
{
    ...
    /* install MKCC signal handlers to print debug info on fatal
       runtime errors */
...
static MKCCSyms mkcc_syms[] = {
#if !defined(CONFIG_MKCCBOOT)
#define MKCCSYM(a) { #a, &a, },
    MKCCSYM(printf)
    MKCCSYM(fprintf)
    MKCCSYM(fopen)
    MKCCSYM(fclose)
...
ST_FUNC void *resolve_sym(MKCCState *s1, const char *sym)
{
    return dlsym(RTLD_DEFAULT, sym);
}

#endif /* CONFIG_MKCC_STATIC */
#endif /* MKCC_IS_NATIVE */
/* ------------------------------------------------------------- */
