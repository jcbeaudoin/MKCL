...
static void set_options(MKCCState *s, int argc, char **argv)
{
    int i;
    for (i = 1; i < argc; ++i) {
        char *a = argv[i];
...
static int run_callback(const char *src, callback_type callback) {
  ...
  if (mkcc_set_output_type(s, MKCC_OUTPUT_MEMORY) == -1)
    return -1;
  if (mkcc_compile_string(s, src) == -1)
    return -1;
  if (mkcc_relocate(s, MKCC_RELOCATE_AUTO) == -1)
    return -1;
  
  ptr = mkcc_get_symbol(s, "f");
  if (!ptr)
    return -1;
  result = callback(ptr);
 ...
 * address, thus causing a success message even though MKCC actually
