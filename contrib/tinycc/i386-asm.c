/*
 *  i386 specific functions for MKCC assembler
 *
...
static inline int get_reg_shift(MKCCState *s1)
{
    int shift, v;
    v = asm_int_expr(s1);
    switch(v) {
...
static void parse_operand(MKCCState *s1, Operand *op)
{
    ExprValue e;
    int reg, indir;
    const char *p;
...
static void gen_disp32(ExprValue *pe)
{
    ...
    if (sym && sym->r == cur_text_section->sh_num) {
        /* same section: we can output an absolute value. Note
           that the MKCC compiler behaves differently here because
...
ST_FUNC void asm_opcode(MKCCState *s1, int opcode)
{
    ...
    for(;;) {
        ...
        if (nb_ops >= MAX_OPERANDS) {
            mkcc_error("incorrect number of operands");
        }
        parse_operand(s1, pop);
        if (tok == ':') {
           if (pop->type != OP_SEG || seg_prefix)
               mkcc_error("incorrect prefix");
           seg_prefix = segment_prefixes[pop->reg];
           next();
           parse_operand(s1, pop);
           if (!(pop->type & OP_EA)) {
               mkcc_error("segment prefix must be followed by memory reference");
           }
        }
        pop++;
        nb_ops++;
    ...
    for(pa = asm_instrs; pa->sym != 0; pa++) {
	    ...
	    /* cmovxx is a test opcode but accepts multiple sizes.
	       MKCC doesn't accept the suffixed mnemonic, instead we 
    ...
    if (pa->sym == 0) {
        if (opcode >= TOK_ASM_first && opcode <= TOK_ASM_last) {
            ...
            mkcc_error("bad operand with opcode '%s'",
                  get_tok_str(opcode, NULL));
        } else {
            mkcc_error("unknown opcode '%s'",
                  get_tok_str(opcode, NULL));
        }
    }
    ...
    if (s == autosize) {
        ...
        if (s == autosize) {
            if ((opcode == TOK_ASM_push || opcode == TOK_ASM_pop) &&
                ...
                mkcc_error("cannot infer opcode suffix");
        }
    }
    ...
    if (pa->instr_type & OPC_SHORTJMP) {
        ...
        no_short_jump:
            if (pa->instr_type & OPC_JMP) {
                ...
                mkcc_error("invalid displacement");
            }
        }
    }
    op1 = v >> 8;
...
static inline int constraint_priority(const char *str)
{
    ...
    for(;;) {
        ...
        case 'g':
            ...
            break;
        default:
            mkcc_error("unknown constraint '%c'", c);
            pr = 0;
        }
        if (pr > priority)
            priority = pr;
                                    ...
                                    int *pout_reg)
{
    ...
    for(i=0;i<nb_operands;i++) {
        ...
        if (isnum(*str) || *str == '[') {
            ...
            if ((unsigned)k >= i || i < nb_outputs)
                mkcc_error("invalid reference in constraint %d ('%s')",
                      i, str);
            op->ref_index = k;
            if (operands[k].input_index >= 0)
                mkcc_error("cannot reference twice the same operand");
    ...
    try_next:
        ...
        case '&':
            if (j >= nb_outputs)
                mkcc_error("'%c' modifier can only be applied to outputs", c);
            reg_mask = REG_IN_MASK | REG_OUT_MASK;
            goto try_next;
        case 'A':
        ...
        case 'g':
            ...
            break;
        default:
            mkcc_error("asm constraint %d ('%s') could not be satisfied",
                  j, op->constraint);
            break;
        }
    ...
    *pout_reg = -1;
    for(i=0;i<nb_operands;i++) {
        ...
        if (op->reg >= 0 &&
            ...
            mkcc_error("could not find free output register for reloading");
        reg_found2:
            *pout_reg = reg;
            break;
        }
                              ...
                              SValue *sv, int modifier)
{
    ...
    if ((r & VT_VALMASK) == VT_CONST) {
        ...
        reg = r & VT_VALMASK;
        if (reg >= VT_CONST)
            mkcc_error("internal compiler error");
        ...
        if (modifier == 'b') {
            if (reg >= 4)
                mkcc_error("cannot use byte register");
            size = 1;
        } else if (modifier == 'h') {
            if (reg >= 4)
                mkcc_error("cannot use byte register");
...
ST_FUNC void asm_clobber(uint8_t *clobber_regs, const char *str)
{
    ...
    if (reg >= TOK_ASM_eax && reg <= TOK_ASM_edi) {
        ...
        mkcc_error("invalid clobber register '%s'", str);
    }
    clobber_regs[reg] = 1;
}
