...
static const char mkcc_keywords[] = 
...
{
    if (tok != c)
        mkcc_error("'%c' expected (got \"%s\")", c, get_tok_str(tok, &tokc));
    next();
}

ST_FUNC void expect(const char *msg)
{
    mkcc_error("%s expected", msg);
}
...
{
    TinyAlloc *al = mkcc_mallocz(sizeof(TinyAlloc));
    al->p = al->buffer = mkcc_malloc(size);
    al->limit = limit;
    al->size = size;
    if (pal) *pal = al;
    return al;
...
static TokenSym *tok_alloc_new(TokenSym **pts, const char *str, int len)
{
    ...
    if (tok_ident >= SYM_FIRST_ANOM) 
        mkcc_error("memory full (symbols)");

    /* expand token table if needed */
    i = tok_ident - TOK_IDENT;
    if ((i % TOK_ALLOC_INCR) == 0) {
        ptable = mkcc_realloc(table_ident, (i + TOK_ALLOC_INCR) * sizeof(TokenSym *));
        table_ident = ptable;
    }
        ...
        if (c == '\n') {
            ...
            for(;;) {
                ...
                if (c == '*') {
                    ...
                    p = file->buf_ptr;
                    if (c == CH_EOF)
                        mkcc_error("unexpected end of file in comment");
                    if (c == '\\') {
    ...
    for(;;) {
        ...
        if (c == sep) {
            ...
            unterminated_string:
                ...
                mkcc_error("missing terminating %c character", sep);
            } else if (c == '\\') {
                /* escape : just skip \[\r]\n */
                PEEKC_EOB(c, p);
                if (c == '\n') {
...
ST_INLN void define_push(int v, int macro_type, TokenString *str, Sym *first_arg)
{
	...
	mkcc_warning("%s redefined", get_tok_str(v, NULL));
}
...
ST_FUNC void label_pop(Sym **ptop, Sym *slast)
{
    ...
    for(s = *ptop; s != slast; s = s1) {
        ...
        if (s->r == LABEL_DECLARED) {
            mkcc_warning("label '%s' declared but not used", get_tok_str(s->v, NULL));
        } else if (s->r == LABEL_FORWARD) {
                mkcc_error("label '%s' used but not defined",
...
ST_FUNC void parse_define(void)
{
    ...
    v = tok;
    if (v < TOK_IDENT)
        mkcc_error("invalid macro name '%s'", get_tok_str(tok, &tokc));
    ...
    if (tok == '(') {
        ...
        if (tok != ')') for (;;) {
            ...
            if (varg < TOK_IDENT)
        bad_list:
                mkcc_error("bad macro parameter list");
    ...
    if (3 == spc)
bad_twosharp:
        mkcc_error("'##' cannot appear at either end of macro");
...
static CachedInclude *search_cached_include(MKCCState *s1, const char *filename)
{
    CachedInclude *e;
    int i, h;
...
static inline void add_cached_include(MKCCState *s1, const char *filename, int ifndef_macro)
{
    CachedInclude *e;
    int h;
...
#endif
    e = mkcc_malloc(sizeof(CachedInclude) + strlen(filename));
...
static void pragma_parse(MKCCState *s1)
{
    next_nomacro();
    if (tok == TOK_push_macro || tok == TOK_pop_macro) {
        ...
        if (s)
            ...
            mkcc_warning("unbalanced #pragma pop_macro");
        ...
        /* mkcc -E: keep pragmas below unchanged */
        unget_tok(' ');
        ...
        if (tok != TOK_STR)
            goto pragma_err;
        file = mkcc_strdup((char *)tokc.str.data);
...
pragma_warn:
        if (s1->warn_unsupported)
            mkcc_warning("#pragma %s is ignored", get_tok_str(tok, &tokc));
    }
    return;

pragma_err:
    mkcc_error("malformed #pragma directive");
    return;
}
 ...
 redo:
    ...
    case TOK_INCLUDE_NEXT:
            ...
            *q = '\0';
            ...
            if (tok == TOK_STR) {
                while (tok != TOK_LINEFEED) {
                    if (tok != TOK_STR) {
                    include_syntax:
                        mkcc_error("'#include' expects \"FILENAME\" or <FILENAME>");
                    }
        ...
        if (s1->include_stack_ptr >= s1->include_stack + INCLUDE_STACK_SIZE)
            mkcc_error("#include recursion too deep");
        ...
        *s1->include_stack_ptr = file;
        ...
        for (; i < n; ++i) {
            ...
            if (i == 0) {
                ...
                if (c != '\"')
                    continue;
                path = file->filename;
                pstrncpy(buf1, path, mkcc_basename(path) - path);
        ...
        mkcc_error("include file '%s' not found", buf);
include_done:
        break;
    case TOK_IFNDEF:
        c = 1;
    ...
    do_ifdef:
        next_nomacro();
        if (tok < TOK_IDENT)
            mkcc_error("invalid argument for '#if%sdef'", c ? "n" : "");
        if (is_bof) {
            if (c) {
#ifdef INC_DEBUG
    ...
    do_if:
        if (s1->ifdef_stack_ptr >= s1->ifdef_stack + IFDEF_STACK_SIZE)
            mkcc_error("memory full (ifdef)");
        *s1->ifdef_stack_ptr++ = c;
        goto test_skip;
    case TOK_ELSE:
        if (s1->ifdef_stack_ptr == s1->ifdef_stack)
            mkcc_error("#else without matching #if");
        if (s1->ifdef_stack_ptr[-1] & 2)
            mkcc_error("#else after #else");
        c = (s1->ifdef_stack_ptr[-1] ^= 3);
        goto test_else;
    case TOK_ELIF:
        if (s1->ifdef_stack_ptr == s1->ifdef_stack)
            mkcc_error("#elif without matching #if");
        c = s1->ifdef_stack_ptr[-1];
        if (c > 1)
            mkcc_error("#elif after #else");
    ...
    case TOK_ENDIF:
        if (s1->ifdef_stack_ptr <= file->ifdef_stack_ptr)
            mkcc_error("#endif without matching #if");
        s1->ifdef_stack_ptr--;
...
_line_num:
    ...
    case TOK_WARNING:
        ...
        *q = '\0';
        if (c == TOK_ERROR)
            mkcc_error("#error %s", buf);
        else
            mkcc_warning("#warning %s", buf);
        break;
    case TOK_PRAGMA:
        pragma_parse(s1);
        break;
    ...
    default:
        ...
        if (tok == '!' && is_bof)
            ...
            goto ignore;
        mkcc_warning("Ignoring unknown preprocessing directive #%s", get_tok_str(tok, &tokc));
    ignore:
...
static void parse_escape_string(CString *outstr, const uint8_t *buf, int is_long)
{
    ...
    for(;;) {
        ...
        if (c == '\\') {
            ...
            invalid_escape:
                if (c >= '!' && c <= '~')
                    mkcc_warning("unknown escape sequence: \'\\%c\'", c);
                else
                    mkcc_warning("unknown escape sequence: \'\\x%x\'", c);
                break;
            }
        }
        p++;
...
static void parse_string(const char *s, int len)
{
    ...
    if (sep == '\'') {
        ...
        if (tokcstr.size <= char_size)
            mkcc_error("empty character constant");
        if (tokcstr.size > 2 * char_size)
            mkcc_warning("multi-character character constant");
        if (!is_long) {
    ...
    if (ch == '.' ||
        ...
        *q = '\0';
        ...
        while(1) {
            ...
            if (n < n1)
                mkcc_error("integer constant overflow");
        }
        ...
        for(;;) {
            ...
            if (t == 'L') {
                if (lcount >= 2)
                    mkcc_error("three 'l's in integer constant");
                if (lcount && *(p - 1) != ch)
                    mkcc_error("incorrect integer suffix: %s", p1);
                lcount++;
                ...
                if (ucount >= 1)
                    mkcc_error("two 'u's in integer constant");
                ucount++;
                ch = *p++;
            } else {
                break;
...
maybe_newline:
    ...
    default:
        if (parse_flags & PARSE_FLAG_ASM_FILE)
            goto parse_simple;
        mkcc_error("unrecognized character \\x%02x", c);
        break;
    }
    tok_flags = 0;
keep_tok_flags:
    ...
    int can_read_stream)
{
    ...
    empty_arg:
                ...
                if (!args && !sa && tok == ')')
                    break;
                if (!sa)
                    mkcc_error("macro '%s' used with too many args",
                          get_tok_str(s->v, 0));
                tok_str_new(&str);
                parlevel = spc = 0;
            ...
            if (sa) {
                mkcc_error("macro '%s' used with too few args",
                      get_tok_str(s->v, 0));
            }
...
static int paste_tokens(int t1, CValue *v1, int t2, CValue *v2)
{
    ...
    mkcc_open_bf(mkcc_state, ":paste:", cstr.size);
    ...
    for (;;) {
        ...
        if (is_space(tok))
            continue;
        mkcc_warning("pasting <%.*s> and <%s> does not give a valid preprocessing token",
...
{
    TokenString *str = mkcc_malloc(sizeof *str);
    tok_str_new(str);
    tok_str_add2(str, tok, &tokc);
    tok_str_add(str, 0);
    begin_macro(str, 1);
...
static void pp_line(MKCCState *s1, BufferedFile *f, int level)
{
...
static void define_print(MKCCState *s1, int v)
{
    FILE *fp;
    Sym *s;
...
static void pp_debug_defines(MKCCState *s1)
{
    int v, t;
    const char *vs;
    FILE *fp;
...
static void pp_debug_builtins(MKCCState *s1)
{
    int v;
