/*
 *  TMS320C67xx code generator for MKCC
 * 
...
// although mkcc thinks it is passing parameters on the stack,
...
do {\
   if (!(x))\
       mkcc_error("internal compiler error file at %s:%d", __FILE__, __LINE__);\
} while (0)
...
// these are regs that mkcc doesn't really know about, 
...
// map MKCC reg to C67 reg number

int C67_map_regn(int r)
{
    if (r == 0)			// normal mkcc regs
	return 0x2;		// A2
    else if (r == 1)		// normal mkcc regs
	return 3;		// A3
    else if (r == 2)		// normal mkcc regs
	return 0;		// B0
    else if (r == 3)		// normal mkcc regs
	return 1;		// B1
...
// mapping from mkcc reg number to 
...
int C67_map_regc(int r)
{
	...
	return 0x2;
    else if (r == C67_B2)	// normal mkcc regs
	return 0x3;
    else if (r == C67_CREG_ZERO)
	return 0;		// Special code for no condition reg test
    else
...
// map MKCC reg to C67 reg side A or B
...
void load(int r, SValue * sv)
{
	...
	if (v == VT_LLOCAL) {
	    ...
	    mkcc_error("long double not supported");
	...
	// if it is mkcc is referencing what it thinks is a parameter
...
void store(int r, SValue * v)
{
	...
	if ((v->r & VT_VALMASK) == VT_CONST) {
	    ...
	    // mkcc thinks is on the stack but for C67 is
...
void gfunc_call(int nb_args)
{
	...
	mkcc_error("more than 10 function params not currently supported");
	...
	if ((vtop->type.t & VT_BTYPE) == VT_STRUCT) {
		...
		mkcc_error("long long not supported");
...
void gfunc_prolog(CType * func_type)
{
	...
	// we can translate where mkcc thinks they
...
void gen_opf(int op)
{
	...
	mkcc_error("long doubles not supported");
	...
	vtop->r = VT_CMP;	// tell MKCC that result is in "flags" actually B2
    } else {
	if (op == '+') {
	    if ((ft & VT_BTYPE) == VT_DOUBLE) {
