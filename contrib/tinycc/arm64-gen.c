...
#define EM_MKCC_TARGET EM_AARCH64
...
ST_FUNC void gsym_addr(int t_, int a_)
{
    ...
    while (t) {
        ...
        if (a - t + 0x8000000 >= 0x10000000)
            mkcc_error("branch out of range");
...
{
    // Currently MKCC's linker does not generate COPY relocations for
    // STT_OBJECTs when mkcc is invoked with "-run". This typically
...
ST_FUNC void gfunc_call(int nb_args)
{
    ...
    t = mkcc_malloc((nb_args + 1) * sizeof(*t));
    a = mkcc_malloc((nb_args + 1) * sizeof(*a));
    a1 = mkcc_malloc((nb_args + 1) * sizeof(*a1));
...
ST_FUNC void gfunc_prolog(CType *func_type)
{
    ...
    for (sym = func_type->ref; sym; sym = sym->next)
        ++n;
    t = mkcc_malloc(n * sizeof(*t));
    a = mkcc_malloc(n * sizeof(*a));
