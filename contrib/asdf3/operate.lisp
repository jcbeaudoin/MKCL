            ...
            (let ((*verbose-out* (make-broadcast-stream)))
              (let ((system (find-system module nil)))
                (when system
                  ...
                  ;; In MKCL 2 load into compilation environment are not permanent and disappear on return from compile-file.
                  #+mkcl (when si::*compilation-unit-environment* (clear-system name))
                  t)))))))))
