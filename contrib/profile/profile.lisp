...
(defconstant +wrap+ (ffi:c-inline () () :object "mkcl_make_unsigned_integer(env, ~((size_t)0))"
			     :one-liner t))

(defun get-bytes-consed (orig)
  (let ((bytes (ffi:c-inline () () :object "mkcl_make_unsigned_integer(env, mkcl_GC_get_total_bytes())"
			     :one-liner t)))
    (if (< bytes orig)
	(+ (- +wrap+ orig) bytes)
	(- bytes orig))))
