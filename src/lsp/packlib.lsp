...
is used."
  ...
  (mapcan #'(lambda (p)
		    ...
		    (when (eq p (symbol-package s))
		      ;; Count it only in its home package! JCB
		      (list s))
                    nil)))
          (list-all-packages)))
...
(defmacro with-package-iterator ((iterator package-list &rest conditions) &rest body)
  (if conditions
      (let ((aux (set-difference conditions '(:external :internal :inherited))))
	(when aux
	  (error 'simple-program-error :format-control "Clauses ~{~S~} are not allowed." :format-arguments (list aux))))
      (error 'simple-program-error :format-control "Must supply at least one of :inherited, :external or :internal"))
...
(defmacro do-symbols ((var &optional (package '*package*) (result-form nil)) &rest body)
...
(defmacro do-external-symbols ((var &optional (package '*package*) (result-form nil)) &rest body)
...
to NIL) and returns all values."
  (expand-do-symbols var '(list-all-packages) result-form body 
		     '(:internal :external :home)))

(defvar *apropos-print-level* 3)
(defvar *apropos-print-length* 3)
...
(defun print-symbol-apropos (symbol)
  ...
  (when (boundp symbol)
        (if (constantp symbol)
            (princ "  Constant: ")
            (princ "  has value: "))
	(let ((*print-level* *apropos-print-level*)
...
;;
;; Code provided by Franz Inc. to the public domain and adapted for MKCL.
;;
