...
(defmacro with-fpe (new-excepts &body body)
  (let ((set-diff-sym (gensym "%except-set-")))
    `(let* ((old-set (si:all-enabled-fpe))
	    (excepts-arg ,new-excepts)
	    (new-excepts (if (listp excepts-arg) excepts-arg (list excepts-arg)))
	    (,set-diff-sym (set-difference new-excepts old-set))
	    )
       (unwind-protect
	   (progn
	     (dolist (except ,set-diff-sym) (si:enable-fpe except))
	     ,@body
	     )
	 (dolist (except ,set-diff-sym) (si:disable-fpe except))))))

(defmacro without-fpe (not-excepts &body body)
  (let ((set-intersec-sym (gensym "%except-set-"))
	(not-excepts-sym (gensym "%not-excepts-")))
    `(let* ((old-set (si:all-enabled-fpe))
	    (excepts-arg ,not-excepts)
	    (not-excepts (if (listp excepts-arg) excepts-arg (list excepts-arg)))
	    (,not-excepts-sym not-excepts)
	    (,set-intersec-sym (intersection not-excepts old-set))
	    )
       (unwind-protect
	   (progn
	     (dolist (except ,set-intersec-sym) (si:disable-fpe except))
	     ,@body
	     )
	 (dolist (except ,not-excepts-sym) (si:clear-fpe except))
	 (dolist (except ,set-intersec-sym) (si:enable-fpe except))))))


(defmacro with-all-fpe (&body body)
  `(with-fpe '(floating-point-inexact 
...
(defmacro without-any-fpe (&body body)
  `(without-fpe '(floating-point-inexact 
		  floating-point-invalid-operation
		  floating-point-underflow
		  floating-point-overflow
		  division-by-zero)
...
(defmacro with-default-fpe (&body body)
  `(with-fpe *default-floating-point-exception-set* ,@body))

(defmacro without-default-fpe (&body body)
  `(without-fpe *default-floating-point-exception-set* ,@body))

(export '(with-fpe without-fpe with-all-fpe without-any-fpe with-default-fpe without-default-fpe))
...
Returns the arc sine of NUMBER."
  (if #+mkcl-min t #-mkcl-min (complexp x)
      (complex-asin x)
      #-mkcl-min
      (let* ((x (float x))
	     (xr (float x 1l0)))
...
Returns the hyperbolic arc sine of NUMBER."
  ;(log (+ x (sqrt (+ 1.0 (* x x)))))
  (if #+(or mkcl-min) t #-(or mkcl-min) (complexp x)
...
starting the least-significant-bit but POSITION bits of integers.  In MKCL, a
byte specifier is represented by a dotted pair (SIZE . POSITION)."
  (cons size position))

(defun byte-size (bytespec)
  "Args: (byte)
Returns the size part (in MKCL, the car part) of the byte specifier BYTE."
  (car bytespec))

(defun byte-position (bytespec)
  "Args: (byte)
Returns the position part (in MKCL, the cdr part) of the byte specifier BYTE."
  (cdr bytespec))
