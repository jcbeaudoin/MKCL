...
#+cmu
(ext:file-comment
 "$Header: /project/cmucl/cvsroot/src/code/loop.lisp,v 1.30 2008-07-15 15:44:33 rtoy Exp $")
...
(defstruct (loop-minimax
	     #+mkcl (:type vector) ;; otherwise defstruct needs defclass which is not loaded yet. JCB
...
(defstruct (loop-universe
	     #+mkcl (:type vector) ;; otherwise defstruct needs defclass which is not loaded yet. JCB
	     #-mkcl ;; this is a consequence of the just above.
	  ...
	  ((symbolp (car x))
	   (let ((fn (car x)) (tem nil) (n 0))
	     ...
	     (macrolet ((f (overhead &optional (args nil args-p))
	       ...
	       (cond ((setq tem (get-sysprop #|get|# fn 'estimate-code-size))
		      (typecase tem
			(fixnum (f tem))
			(t (funcall tem x env))))
			...
			(setq n (+ n (estimate-code-size-1 (cadr l) env) 1))))
		     ...
		     ((eq fn 'function)
		      ...
		      (if #+mkcl (or (symbolp (cadr x))
				     (and (consp (cadr x)) (eq (caadr x) 'setf)))
			  #-mkcl (ext:valid-function-name-p (cadr x))
			  1
...
(defun add-loop-path (names function universe &key preposition-groups inclusive-permitted user-data)
  ...
  #-(or Genera (and CLOE Source-Bootstrap) mkcl) (check-type universe loop-universe)
