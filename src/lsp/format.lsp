...
;;;
;;; Most probably from cmucl 18e (JCB).
;;;
;;;
;;; **********************************************************************
;;;
...
(declaim (notinline flonum-to-string))
...
(defun scale-exponent (original-x)
  (without-fpe 'floating-point-inexact ;; JCB
	...
	(if (= x 0.0l0)
	  ...
	  (let* ((ex (round (* exponent (log 2l0 10))))
		 (x (if (minusp ex)
			(if #-mkcl(float-denormalized-p x)
			  #+mkcl(< least-negative-normalized-long-float
				   x
  ...
  (if (output-float-free-format-p x)
	...
	(print-float-exponent x 0 stream))
      ...
      (multiple-value-bind (f ex)
        ...
        (declare (ignorable f))
	(multiple-value-bind (str len lpoint tpoint)
			     ;;(si::flonum-to-string f nil nil 1) ;; This one clobbers the rightmost 2 digits. JCB
			     (si::flonum-to-string x nil nil (1+ (- ex))) ;; JCB
	  (declare (ignore len))
	  (when lpoint (write-char #\0 stream))
	  (write-string str stream)
	  (when tpoint (write-char #\0 stream))
...
(defun output-float (x stream)
  (cond
   ((si::float-infinity-p x)
    (si:output-float-infinity x stream))
   ((si::float-nan-p x)
    (si:output-float-nan x stream))
   (t
...
(defstruct (format-directive
	    #-mkcl(:print-function %print-format-directive)
	    #+mkcl :named
	    #+mkcl(:type vector))
...
(defun interpret-directive-list (stream directives orig-args args)
  (if directives
	...
	(etypecase directive
	  ...
	  (#-mkcl format-directive #+mkcl vector
	   (multiple-value-bind
...
(defmacro def-complex-format-directive (char lambda-list &body body)
  ...
  (let* ((name (or (char-name char) (string char)))
    ...
    `(%set-format-directive-expander ,char
       (si::lambda-block ,defun-name (,directive ,directives)
	 ,@(if lambda-list
  ...
  (if directives
	...
	(if (format-directive-p next)
	    (let ((char (format-directive-character next)))
		  ...
		  (car directives)
		  (find-directive
		   (cdr (macrolet ;;flet ;; this flet creates too many closures at runtime. JCB
			 #+(or)
			 ((after (char)
...
(defun format-fixed-aux (stream number w d k ovf pad atsign)
  (cond
   ((or (not (or w d))
	;#-mkcl
	(and (floatp number)
	     (or (si:float-infinity-p number)
		 (si:float-nan-p number))))
    (prin1 number stream)
    nil)
   (t
    (let ((spaceleft w))
...
(defun format-relative-tab (stream colrel colinc)
  (if (#-mkcl pp:pretty-stream-p #+mkcl sys::pretty-stream-p stream)
      (pprint-tab :line-relative colrel colinc stream)
      (let* ((cur (#-mkcl sys::charpos #+mkcl sys::file-column stream))
	...
	params
	 ...
	 (block nil
	   ,@(when newline-segment-p
		 ...
		 ,(expand-bind-defaults
		      ((extra 0)
		       (line-len '(or #-mkcl (sys::line-length stream) 72)))
  ...
  (interpret-bind-defaults
    ...
    (let ((newline-string nil)
      ...
      (setf args
		...
		(interpret-bind-defaults
		    ((extra 0)
		     (len (or #-mkcl (sys::line-length stream) 72)))
		    (format-directive-params first-semi)
		  (setf newline-string
			(with-output-to-string (stream)
			  (setf args
...
(defun format-justification (stream newline-prefix extra-space line-len strings
  ...
  (let* ((num-gaps (1- (length strings)))
    ...
    (when (and newline-prefix
	       (> (+ (or (#-mkcl sys::charpos #+mkcl sys:file-column stream) 0)
		     length extra-space)
		  line-len))
...
(setq *features* (delete :formatter *features*))
