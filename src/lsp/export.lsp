...
(let ((f #'(si::lambda-block dolist (whole env)
	   ...
	   (let (body control var list-form result-form)
		 ...
		 (setq list-form (first control) result-form (rest control))
		 (simple-program-error "Syntax error in ~A:~%~A" 'DOLIST whole))
		 ...
		 (let ((%dolist-var ,list-form))
		   (si::while %dolist-var
		      (let ((,var (first %dolist-var)))
                        ...
                        (tagbody ,@body)
                        (locally (declare (type cons %dolist-var))
	   ...
	   (declare (ignore env))
           (let (body control var list-form result-form)
             ...
             (if (<= 1 (length control) 2)
                 (setq list-form (first control) result-form (rest control))
               (simple-program-error "Syntax error in ~A:~%~A" 'MKCL:DOLIST! whole))
             (flet ((remove-var-from-decl (decl &aux (decl-ident (first decl)))
                      (case decl-ident
                        ((declaration ftype inline notinline optimize) decl) ;; pass through
                        ((dynamic-extent ignorable ignore special) `(,decl-ident ,@(remove var (rest decl))))
                        (type `(type ,(second decl) ,@(remove var (cddr decl))))
                        (t ;; abbreviated 'type' declaration.
                         `(,decl-ident ,@(remove var (rest decl)))))))
               (multiple-value-bind (declarations body)
                   (process-declarations body nil)
                 `(block nil
                    (let ((%dolist!-var ,list-form))
                      ...
                      ,(when result-form
                         `(locally (declare ,@(mapcar #'remove-var-from-decl declarations)) ,@result-form))))))))))
...
(let ((f #'(si::lambda-block dotimes (whole env)
           (declare (ignore env))
	   (let (body control var count-form result-form)
	     (setq body (rest whole))
	     (when (endp body)
	     ...
	     (setq var (first control) control (rest control))
	     (if (<= 1 (length control) 2)
		 (setq count-form (first control) result-form (rest control))
		 (simple-program-error "Syntax error in ~A:~%~A" 'DOTIMES whole))
	     (multiple-value-bind (declarations body)
		 (process-declarations body nil)
	       `(block nil
...
(let ((f #'(si::lambda-block do/do*-expand (whole env)
	   (declare (ignore env))
           (let (do/do* control test result-forms vl step let psetq body)
	     ...
	     (when (endp test)
	       (simple-program-error "Syntax error in ~A:~%~A" do/do* whole))
	     (setq result-forms (rest test) test (first test))
	     (dolist (c control)
