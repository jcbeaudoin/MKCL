...
;;;;  Copyright (c) 2026, Jean-Claude Beaudoin.
...
;;;(push :debug-compilation *features*)
...
(defconstant +work-pool-lock+ (mt:make-lock :name "work-pool-lock"))
(defconstant +work-pool-debug-lock+ (mt:make-lock :name "work-pool-debug-lock"))
(defconstant +work-pool-cv+ (mt:make-condition-variable))
(defvar *all-workers* nil)
(defvar *busy-workers* nil)
(defvar *newly-all-done* nil)
...
(defun clean-up-on-failure (destdir sources)
...
(defun work-on-compiling-file (source object)
  (#-debug-compilation progn #+debug-compilation mt:with-lock #+debug-compilation (+work-pool-debug-lock+)
    (mt:with-lock (+work-pool-lock+)
      (format t "~&(compile-file ~S :output-file ~S~{ ~S~})~%"
	      source object *compile-extra-options*)
      (finish-output))
	...
	(apply #'compile-file source
      ...
      #+debug-compilation
      (mt:with-lock (+work-pool-lock+)
	(format t "~&Done with (compile-file ~S :output-file ~S~{ ~S~})~%"
		source object *compile-extra-options*)
	(finish-output))
      (mt:with-lock (+work-pool-lock+)
	(setq *busy-workers* (delete mt:*thread* *busy-workers*))
	(push mt:*thread* *newly-all-done*)
	(mt:condition-signal +work-pool-cv+)
	)
      (unless failure-p
	(list output-truename warnings-p))
      )
    )
  )

(defun start-worker-to-compile-file (source object)
  (mt:thread-enable  
   (mt:with-lock (+work-pool-lock+)
     (let ((worker (mt:make-thread :name (str+ "worker-" (pathname-name source)))))
       (mt:thread-preset worker #'work-on-compiling-file source object)
       (push worker *all-workers*)
       (push worker *busy-workers*)
       worker))))

;;;
;;; * Timed compilation facility.
;;;
(defun compile-if-old (destdir sources)
  (let (objects failure-p)
    (dolist (source sources)
      #+(or)
	...
	(unless (and (probe-file object)
	  ...
	  (start-worker-to-compile-file source object))
	(push object objects)))
    ;; Collect worker results
    (mt:with-lock (+work-pool-lock+)
      (do (actual-objects)
	  ((or failure-p (null *busy-workers*)) #| verify that objects and actual-objects actually match |#)
	#-(and)
	(progn (finish-output) (format t "~&Waiting for workers to finish~%") (finish-output))
	(mt:condition-wait +work-pool-cv+ +work-pool-lock+)
	#-(and)
	(progn (finish-output) (format t "~&Woke up from Waiting for workers to finish~%") (finish-output))
	(do ()
	    ((null *newly-all-done*))
	  (let ((worker (pop *newly-all-done*)))
	    (when worker
	      #-(and)
	      (progn (finish-output) (format t "~&Gettin result from ~S~%" (mt:thread-name worker)) (finish-output))
	      (let ((worker-result (mt:thread-join worker)))
		(if worker-result
		    (push (first worker-result) actual-objects)
		  (setq failure-p t))))))))
    (when failure-p
      (clean-up-on-failure destdir sources))
    (nreverse objects)))
...
(defun build-module (name sources &key
  ...
  (handler-bind 
   ...
   (let* ((asdf-module-name (string-downcase name)) ;; ASDF is hell-bent on downcasing system names by default.
     ...
     (unless (probe-file destdir)  ;; ensure destdir exists.
