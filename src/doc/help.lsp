...
MKCL supports two formats for floats.  One format is called SHORT-FLOAT and the
other format is called SINGLE-FLOAT, DOUBLE-FLOAT, or LONG-FLOAT.  Precisions
and exponent sizes of floats depends on the version of MKCL.  See the MKCL
...
Returns the time since the invocation of MKCL.")

(docfun get-internal-run-time function () "
Returns the CPU time allocated to the current thread since its creation.")
...
(docfun mkcl:getenv function (string) "
MKCL/UNIX specific.
...
Ignore this number; there is no such upper bound in MKCL.")
...
some versions of MKCL, this function does not work correctly because the
...
Loads the contents of the specified file into MKCL.
If the filetype is not specified, MKCL first tries to load the fasl file with
...
(docfun macro-function function (symbol &optional environment) "
...
(docfun make-string-input-stream function (string &optional (start 0) (end (length string)) &key (external-format :default)) "
...
(docfun make-string-output-stream function (&key element-type (external-format :default)) "
...
(docfun si::make-string-output-stream-from-string function (string external-format) ")
MKCL specific.
...
VERSION is simply ignored in MKCL.  FILESPEC and DEFAULTS may be a symbol, a
...
The negative fixnum with the largest absolute value.  - 2^29 in MKCL.")
...
The largest positive fixnum.  2^29 - 1 in MKCL.")
...
however, there is no such upper bound in MKCL.")
...
(docfun mkcl:make-pipe function ()
...
#<package s> where S is actually the name of the package.  MKCL provides ten
built-in packages:
	COMMON-LISP	 standard symbols of Common Lisp.
	COMMON-LISP-USER the package that the user uses by default.
	KEYWORD    	 keyword symbols.
	SYSTEM           system internal symbols.  Has nicknames SYS and SI.
	COMPILER         system internal symbols for the MKCL compiler.
        CLOS             system internal symbols for the CLOS subsystem.
        GRAY             system internal symbols for CLOS stream extension.
        MULTI-THREADING  symbols for the multi-thread extension.
        FFI              symbols for the foreign function interface extension.
        MKCL-EXTENSIONS  symbols of the portable MKCL extensions.")
...
and DEFAULTS are simply ignored in MKCL.")
...
NAME, and TYPE.  HOST, DEVICE, and VERSION slots are meaningless in MKCL,
though they are harmless at all.
A pathname is notated as #\\\"...\", where '...' is actually some information
on the pathname.  This depends on the version of MKCL.  Refer to the MKCL Report
for details.")
...
(docvar pi constant "
The floating-point number that is approximately equal to the ratio of
...
(docfun rename-file function (filespec new-name &key (if-exists :error)) "
Renames the file specified by FILESPEC as specified by NEW-NAME.  Returns
as three values the new pathname, the old full pathname, and the new full
pathname.  FILESPEC and NEW-NAME may be a symbol, a string, a pathname, or
a file stream.")
...
macro character CHAR in READTABLE.  When the MKCL reader reads an object that
begins with CHAR followed by SUBCHAR, it calls FUNCTION with the input stream,
SUBCHAR, and NIL as arguments.  When the MKCL reader reads an object that
...
macro associated with CHAR.  When the MKCL reader reads an object that begins
...
SINGLE-FLOAT as a type specifier is equivalent to LONG-FLOAT in MKCL.")
...
In addition, MKCL implements the following macros as special forms, though of
...
MKCL specific.
Returns T if the SYMBOL names a special variable in the global environment; NIL otherwise.")
...
(docfun gray:stream-read-char method ((obj stream-class)) "
Reads the next character object from the CLOS stream OBJ.")


(docfun gray:stream-read-line method ((obj stream-class) &rest make-array-options) "
...
(docfun gray:stream-unread-char method ((obj stream-class) character) "
Unreads the character object.
CHARACTER will be the next character read by STREAM-READ-CHAR .")


(docfun gray:stream-peek-char method ((obj stream-class) peek-type) "
...
(docfun gray:stream-listen method ((obj stream-class)) "
...
(docfun gray:stream-clear-input method ((obj stream-class)) "
Clears any buffered characters received from the CLOS stream OBJ.
Returns NIL.")


(docfun gray:stream-write-char method ((obj stream-class) character) "
Outputs the CHARACTER to the CLOS stream OBJ and returns the CHARACTER.")


(docfun gray:stream-write-string method ((obj stream-class) string &optional start end) "
...
(docfun gray:stream-fresh-line method ((obj stream-class)) "
...
(docfun gray:stream-clear-output method ((obj stream-class)) "
Aborts any outstanding output operation on the CLOS stream OBJ
and returns NIL .")


(docfun gray:stream-force-output method ((obj stream-class)) "
...
(docfun si::concatenate-strings function (&rest strings) "
MKCL specific.
...
(docfun si::fast-read-from-base-string function (string) "
MKCL specific.
...
Dmkclares that FORM evaluates to a value of TYPE.  Evaluates FORM and checks if
