/* -*- mode: c -*- */
/*
    ffi_x86_Linux.c -- Nonportable component of the FFI
*/
/*
    Copyright (c) 2005, Juan Jose Garcia Ripoll.
    Copyright (c) 2010-2016,2026,  Jean-Claude Beaudoin.
...
mkcl_fficall_push_arg(MKCL, union mkcl_ffi_values *data, enum mkcl_ffi_tag type)
{
  ...
  case MKCL_FFI_LONG_DOUBLE:
    mkcl_fficall_align4(env);
    mkcl_fficall_push_bytes(env, &data->ld, sizeof(long double));
    break;
...
void
mkcl_fficall_execute_void(MKCL, void *f_ptr, struct mkcl_fficall *fficall)
{
  const char* buf = fficall->buffer;
  const size_t bufsize = fficall->buffer_sp - buf;
  ...
  asm volatile (
		...
		"rep movsb\n\t"  /* copy args to stack */
		"call	*%2\n\t"  /* call said function */
		"movl	%%edi, %%esp\n\t"  /* release args from stack */
		: 
		: "c" (bufsize), "m" (buf) , "m" (f_ptr)
		: "%edi", "%esi" /* we do clobber these when we copy the args with "rep movsb" */
		);
}

void
mkcl_fficall_execute_integral(MKCL, void *f_ptr, struct mkcl_fficall *fficall)
{
  const char* buf = fficall->buffer;
  const size_t bufsize = fficall->buffer_sp - buf;
  void * output;
  ...
  fficall->output.pv = output;
}

void
mkcl_fficall_execute_long_long(MKCL, void *f_ptr, struct mkcl_fficall *fficall)
{
  const char* buf = fficall->buffer;
  const size_t bufsize = fficall->buffer_sp - buf;
  unsigned long output_l0;
  unsigned long output_l1;
  ...
  asm volatile (
		...
		"rep movsb\n\t"  /* copy args to stack */
		"call   *%4\n\t"  /* call said function */
		"movl	%%eax, %0\n\t"  /* store returned value bottom */
		"movl	%%edx, %1\n\t"  /* store returned value top */
		"movl	%%edi, %%esp\n\t"  /* release args from stack */
		: "=m" (output_l0), "=m" (output_l1)
		: "c" (bufsize), "m" (buf) , "m" (f_ptr)
		: "%edi", "%esi" /* we do clobber these when we copy the args with "rep movsb" */
		);
  fficall->output.l2[0] = output_l0;
  fficall->output.l2[1] = output_l1;
}

void
mkcl_fficall_execute_float(MKCL, void *f_ptr, struct mkcl_fficall *fficall)
{
  const char* buf = fficall->buffer;
  const size_t bufsize = fficall->buffer_sp - buf;
  float output;
  ...
  fficall->output.f = output;
}

void
mkcl_fficall_execute_double(MKCL, void *f_ptr, struct mkcl_fficall *fficall)
{
  const char* buf = fficall->buffer;
  const size_t bufsize = fficall->buffer_sp - buf;
  double output;
  ...
  fficall->output.d = output;
}

void
mkcl_fficall_execute_long_double(MKCL, void *f_ptr, struct mkcl_fficall *fficall)
{
  const char* buf = fficall->buffer;
  const size_t bufsize = fficall->buffer_sp - buf;
  long double output;
  ...
  fficall->output.ld = output;
}

void
mkcl_fficall_execute_struct(MKCL, void *f_ptr, struct mkcl_fficall *fficall)
{
  const char* buf = fficall->buffer;
  const size_t bufsize = fficall->buffer_sp - buf;
  const void * output_struct = fficall->return_struct_buffer;
  ...
  asm volatile (
		...
		"movl	%%edi, %%esp\n\t"  /* release args from stack */
		:
		: "m" (output_struct), "c" (bufsize), "m" (buf) , "m" (f_ptr)
		: "%edi", "%esi" /* we do clobber these when we copy the args with "rep movsb" */
		);
}

static void (* ffi_proc[MKCL_FFI_VOID+1])(MKCL, void *, struct mkcl_fficall *) =
  {
    mkcl_fficall_execute_integral,  /* char */
    mkcl_fficall_execute_integral,  /* unsigned char */
    mkcl_fficall_execute_integral,  /* byte */
    mkcl_fficall_execute_integral,  /* unsigned byte */
    mkcl_fficall_execute_integral,  /* short */
    mkcl_fficall_execute_integral,  /* unsigned short */
    mkcl_fficall_execute_integral,  /* int */
    mkcl_fficall_execute_integral,  /* unsigned int */
    mkcl_fficall_execute_integral,  /* long */
    mkcl_fficall_execute_integral,  /* unsigned long */
    mkcl_fficall_execute_long_long,  /* long long */
    mkcl_fficall_execute_long_long,  /* unsigned long long */
    mkcl_fficall_execute_integral,  /* pointer void */
    mkcl_fficall_execute_integral,  /* cstring */
    mkcl_fficall_execute_integral,  /* object */
    mkcl_fficall_execute_float,  /* float */
    mkcl_fficall_execute_double,  /* double */
    mkcl_fficall_execute_long_double,  /* long double */
    mkcl_fficall_execute_struct,  /* struct/union */
    mkcl_fficall_execute_void,  /* void */
  };
...
mkcl_fficall_execute(MKCL, void *f_ptr, struct mkcl_fficall *fficall, enum mkcl_ffi_tag return_type)
{
  ...
  ffi_proc[return_type](env, f_ptr, fficall);
}
