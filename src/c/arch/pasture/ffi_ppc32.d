...
struct mkcl_fficall_reg *
mkcl_fficall_prepare_extra(struct mkcl_fficall_reg *registers)
{
	if (registers == 0) {
		registers = (struct mkcl_fficall_reg *)mkcl_alloc(env, sizeof(*registers));
	}
...
void
mkcl_fficall_push_arg(union mkcl_ffi_values *data, enum mkcl_ffi_tag type)
{
	long i;
	struct mkcl_fficall *fficall = mk_cl_env.fficall;
	struct mkcl_fficall_reg *registers = mk_cl_env.fficall->registers;
	switch (type) {
	...
	INT:
		...
		mkcl_fficall_push_bytes(&i, sizeof(long));
		break;
	case MKCL_FFI_DOUBLE:
		...
		mkcl_fficall_align4(env);
		mkcl_fficall_push_bytes(&data->d, sizeof(double), sizeof(long));
		break;
	case MKCL_FFI_FLOAT:
		if (registers->fp_registers_size < MAX_FP_REGISTERS) {
		...
		mkcl_fficall_align4(env);
		mkcl_fficall_push_bytes(&data->f, sizeof(float), sizeof(long));
		break;
	case MKCL_FFI_VOID:
		mkcl_FEerror("VOID is not a valid argument type for a C function", 0);
	}
}

static void
mkcl_fficall_do_execute(mkcl_index buf_size, void *stack, void *gpr, void *gpfr, void *f)
{
}

void
mkcl_fficall_execute(void *_f_ptr, struct mkcl_fficall *fficall, enum mkcl_ffi_tag return_type)
{
	struct mkcl_fficall_reg *registers = fficall->registers;
	size_t bufsize = fficall->buffer_sp - fficall->buffer;
...
"L3:	 lfd	f1, 0(r16)\n\t"		/* load fp registers from (r16) */
	...
	if (return_type <= MKCL_FFI_UNSIGNED_LONG) {
		...
		fficall->output.o = *((mkcl_object *)data);
...
void*
mkcl_dynamic_callback_make(mkcl_object data, enum mkcl_ffi_calling_convention cc_type)
{
	exit(42);
}
