...
struct ecl_fficall_reg *
ecl_fficall_prepare_extra(MECL, struct ecl_fficall_reg *registers)
{
  if (registers == 0) {
    registers = ecl_alloc_atomic_align(env, sizeof(*registers), sizeof(long));
  }
...
void
ecl_fficall_push_arg(MECL, union ecl_ffi_values *data, enum ecl_ffi_tag type)
{
  long i;
  ...
  INT:
    if (registers->int_registers_size < MAX_INT_REGISTERS) {
      ...
      ecl_fficall_align(env, sizeof(long));
      ecl_fficall_push_bytes(env, &i, sizeof(long));
    }
    break;
  case ECL_FFI_DOUBLE:
    if (registers->fp_registers_size < MAX_FP_REGISTERS) {
      ...
      ecl_fficall_align(env, sizeof(long));
      ecl_fficall_push_bytes(env, &data->d, sizeof(double));
    }
    break;
  case ECL_FFI_FLOAT:
    if (registers->fp_registers_size < MAX_FP_REGISTERS) {
      ...
      i = 0;
      ecl_fficall_align(env, sizeof(long));
      ecl_fficall_push_bytes(env, &data->f, sizeof(float));
      ecl_fficall_push_bytes(env, &i, sizeof(float));
    }
    break;
  case ECL_FFI_VOID:
    FEerror(env, "VOID is not a valid argument type for a C function", 0);
  }
}

void
ecl_fficall_execute(MECL, void *_f_ptr, struct ecl_fficall *fficall, enum ecl_ffi_tag return_type)
{
  struct ecl_fficall_reg *registers = fficall->registers;
...
static void
ecl_dynamic_callback_execute(MECL, long i1, long i2, long i3, long i4, long i5, long i6,
			     ...
			     cl_object cbk_info, char *arg_buffer)
{
  ...
  for (i=0; !ecl_endp(env, argtypes); argtypes = CDR(argtypes), i++) {
    tag = ecl_foreign_type_code(env, CAR(argtypes));
    size = fix(si_size_of_foreign_elt_type(env, CAR(argtypes)));
    if (tag <= ECL_FFI_OBJECT) {
      if (i_reg_index < MAX_INT_REGISTERS)
	result = ecl_foreign_data_ref_elt(env, &i_reg[i_reg_index++], tag);
      else
	goto ARG_FROM_STACK;
    } else if (tag <= ECL_FFI_DOUBLE) {
      if (f_reg_index < MAX_FP_REGISTERS)
	result = ecl_foreign_data_ref_elt(env, &f_reg[f_reg_index++], tag);
      else
	goto ARG_FROM_STACK;
    } else {
    ARG_FROM_STACK:
      result = ecl_foreign_data_ref_elt(env, arg_buffer, tag);
      {
	int mask = 7;
    ...
    ecl_stack_frame_push(env, frame, result);
  }

  result = ecl_apply_from_stack_frame(env, frame, fun);
  ecl_stack_frame_close(env, frame);

  tag = ecl_foreign_type_code(env, rtype);
  memset(&output, 0, sizeof(output));
  ecl_foreign_data_set_elt(env, &output, tag, result);
...
void*
ecl_dynamic_callback_make(MECL, cl_object data, enum ecl_ffi_calling_convention cc_type)
{
  /*
   *	push    %rbp                    55
   *	push    %rsp                    54
   ...
   *	nop				90
   */
  char *buf = (char*)ecl_alloc_atomic_align(env, sizeof(char)*32, 8);
  *(char*) (buf+0)  = 0x55;
  *(char*) (buf+1)  = 0x54;
  *(short*)(buf+2)  = 0xb848;
