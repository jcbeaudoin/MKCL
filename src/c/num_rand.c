...
rando(MKCL, mkcl_object x, mkcl_object rs)
{
 ...
 AGAIN:
  ...
  default:
  _MKCL_ERROR:
    x = mkcl_type_error(env, (mkcl_object) &MK_CL_random,"limit",x,
...
mkcl_make_random_state(MKCL, mkcl_object rs)
{
  ...
  if (rs == mk_cl_Ct) {
    ...
    if (mkcl_Null(rs)) {
      rs = mkcl_symbol_value(env, (mkcl_object) &MK_CL_DYNVAR_random_state);
    }
    if (mkcl_type_of(rs) != mkcl_t_random) {
      mkcl_FEwrong_type_argument(env, (mkcl_object) &MK_CL_random_state, rs);
    }
...
struct mkcl_cfun mk_cl_random_cfunobj = MKCL_CFUN_VA(mk_cl_random, (mkcl_object) &MK_CL_random);

mkcl_object mk_cl_random(MKCL, mkcl_narg narg, mkcl_object x, ...)
{
  mkcl_call_stack_check(env);
  {
    mkcl_object rs = ((narg == 1) ? mkcl_symbol_value(env, (mkcl_object) &MK_CL_DYNVAR_random_state) : mk_cl_Cnil);
    MKCL_RECEIVE_1_OPTIONAL_ARGUMENT(env, (mkcl_object) &MK_CL_random, narg, 1, x, &rs);

    rs = mkcl_check_cl_type(env, (mkcl_object) &MK_CL_random, rs, mkcl_t_random);
    mkcl_return_value(rando(env, x, rs));
  }
}

struct mkcl_cfun mk_cl_make_random_state_cfunobj = MKCL_CFUN_VA(mk_cl_make_random_state, (mkcl_object) &MK_CL_make_random_state);

mkcl_object mk_cl_make_random_state(MKCL, mkcl_narg narg, ...)
{
  mkcl_call_stack_check(env);
  {
    mkcl_object rs = mkcl_symbol_value(env, (mkcl_object) &MK_CL_DYNVAR_random_state);
    MKCL_RECEIVE_1_OPTIONAL_ARGUMENT(env, (mkcl_object) &MK_CL_random, narg, 0, narg, &rs);

    mkcl_return_value(mkcl_make_random_state(env, rs));
  }
}

struct mkcl_cfun mk_cl_random_state_p_cfunobj = MKCL_CFUN1(mk_cl_random_state_p, (mkcl_object) &MK_CL_random_state_p);
