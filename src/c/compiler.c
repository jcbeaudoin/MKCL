...
asm_end(MKCL, mkcl_index beginning, mkcl_object definition) {
	...
	mkcl_opcode *code;
	mkcl_object file = MKCL_SYM_VAL(env,(mkcl_object) &MK_CL_DYNVAR_load_truename);
	mkcl_object end_position = mk_cl_cdr(env, MKCL_SYM_VAL(env,(mkcl_object) &MK_SI_DYNVAR_source_location));
...
c_register_block(MKCL, mkcl_object name)
{
	mkcl_object loc = new_location(env, name);
        const mk_cl_compiler_ptr c_env = env->c_env;
	c_env->variables = MKCL_CONS(env, mk_cl_list(env, 4, (mkcl_object) &MK_KEY_block, name, mk_cl_Cnil, loc),
                                c_env->variables);
...
{
	mkcl_object loc = new_location(env, (mkcl_object) &MK_KEY_tag);
        const mk_cl_compiler_ptr c_env = env->c_env;
	c_env->variables = MKCL_CONS(env, mk_cl_list(env, 4, (mkcl_object) &MK_KEY_tag, all_tags, mk_cl_Cnil, loc),
                                     c_env->variables);
	return mkcl_fixnum_to_word(MKCL_CONS_CDR(loc));
}
...
{
        const mk_cl_compiler_ptr c_env = env->c_env;
	c_env->variables = MKCL_CONS(env, mk_cl_list(env, 4, (mkcl_object) &MK_KEY_function, name, mk_cl_Cnil,
                                                     new_location(env, name)),
                                     c_env->variables);
	c_env->macros = MKCL_CONS(env, mk_cl_list(env, 2, name, (mkcl_object) &MK_CL_function), c_env->macros);
}
...
{
        const mk_cl_compiler_ptr c_env = env->c_env;
	c_env->variables = MKCL_CONS(env, mk_cl_list(env, 3, name, (mkcl_object) &MK_SI_symbol_macro, exp_fun),
                                     c_env->variables);
}
...
{
        const mk_cl_compiler_ptr c_env = env->c_env;
	c_env->macros = MKCL_CONS(mk_cl_list(3, name, (mkcl_object) &MK_SI_macro, exp_fun), c_env->macros);
}
*/
...
c_register_var(MKCL, mkcl_object var, bool special, bool bound)
{
	...
	if (bound || (c_var_ref(env, var, 0, FALSE) >= MKCL_UNDEFINED_VAR_REF)) {
		...
		c_env->variables = MKCL_CONS(env, mk_cl_list(env, 4,
							     var,
							     special ? (mkcl_object) &MK_KEY_special : mk_cl_Cnil,
          ...
          mkcl_compiler_env_ptr old)
{
	...
	if (old) {
		...
		for (env = new->variables; !mkcl_Null(env); env = MKCL_CDR(env)) {
			...
			if (MKCL_ATOM(record))
				continue;
			if (MKCL_SYMBOLP(MKCL_CAR(record)) && MKCL_CADR(record) != (mkcl_object) &MK_SI_symbol_macro) {
				continue;
			} else {
				new->lexical_level = 1;
				break;
...
c_tag_ref(MKCL, mkcl_object the_tag, mkcl_object the_type)
{
	...
	for (l = c_env->variables; MKCL_CONSP(l); l = MKCL_CONS_CDR(l)) {
		...
		if (type == ((mkcl_object) &MK_KEY_tag)) {
			if (type == the_type) {
			...
			n++;
		} else if (type == ((mkcl_object) &MK_KEY_block) || type == ((mkcl_object) &MK_KEY_function)) {
...
c_var_ref(MKCL, mkcl_object var, int allow_symbol_macro, bool ensure_defined)
{
	...
	for (l = c_env->variables; MKCL_CONSP(l); l = MKCL_CONS_CDR(l)) {
		...
		if (name == ((mkcl_object) &MK_KEY_block) || name == ((mkcl_object) &MK_KEY_tag) || name == ((mkcl_object) &MK_KEY_function)) {
			n++;
		} else if (name == ((mkcl_object) &MK_KEY_declare)) {
			/* Ignored */
		} else if (name != var) {
			/* Symbol not yet found. Only count locals. */
			if (mkcl_Null(special)) n++;
		} else if (special == ((mkcl_object) &MK_SI_symbol_macro)) {
	...
	if (ensure_defined) {
	  l = mkcl_symbol_value(env, (mkcl_object) &MK_SI_DYNVAR_action_on_undefined_variable);
		if (l != mk_cl_Cnil) {
		  mkcl_funcall2(env, l,
...
c_undo_bindings(mkcl_env the_env, mkcl_object old_vars, int only_specials)
{
	...
	for (env = c_env->variables; env != old_vars && !mkcl_Null(env); env = MKCL_CONS_CDR(env))
	{
		...
		name = MKCL_CONS_CAR(record);
                record = MKCL_CONS_CDR(record);
		special = MKCL_CONS_CAR(record);
		if (name == ((mkcl_object) &MK_KEY_block) || name == ((mkcl_object) &MK_KEY_tag)) {
			(void)0;
		} else if (name == ((mkcl_object) &MK_KEY_function) || mkcl_Null(special)) {
...
/* 		asm_op(env, OP_POP1); */
		flags = FLAG_VALUES;
		   ...
		   ((flags & FLAG_GLOBAL) || mkcl_Null(c_tag_ref(env, name, (mkcl_object) &MK_KEY_function))))
	{
...
c_funcall(MKCL, mkcl_object args, int flags) {
	...
	if (MKCL_CONSP(name)) {
		...
		if (kind == ((mkcl_object) &MK_CL_function)) {
		...
		if (kind == ((mkcl_object) &MK_CL_quote) && /* JCB */ MKCL_SYMBOLP(MKCL_CADR(name))) {
		  if (mk_cl_list_length(env, name) != MKCL_MAKE_FIXNUM(2))
		    mkcl_FEprogram_error(env, "FUNCALL: Invalid function name ~S",
						1, name);
...
perform_c_case(MKCL, mkcl_object args, int flags) {
	...
	if (((mkcl_object) &MK_CL_otherwise) == test || test == mk_cl_Ct) {
...
{
  return mkcl_member_eq(env, (mkcl_object) &MK_CL_eval, situation) ||
    mkcl_member_eq(env, (mkcl_object) &MK_KEY_execute, situation);
}

static int
when_compile_p(MKCL, mkcl_object situation)
{
  return mkcl_member_eq(env, (mkcl_object) &MK_CL_compile, situation) ||
    mkcl_member_eq(env, (mkcl_object) &MK_KEY_compile_toplevel, situation);
}

static int
when_load_p(MKCL, mkcl_object situation)
{
  return mkcl_member_eq(env, (mkcl_object) &MK_CL_load, situation) ||
    mkcl_member_eq(env, (mkcl_object) &MK_KEY_load_toplevel, situation);
}
...
c_eval_when(MKCL, mkcl_object args, int flags) {
  mkcl_object situation = pop(env, &args);
	  ...
	  if (when_compile_p(env, situation)) {
                        ...
                        eval_form(env, MKCL_CONS(env, (mkcl_object) &MK_CL_progn, args));
...
asm_function(MKCL, mkcl_object function, int flags) {
  if (!mkcl_Null(mk_si_valid_function_name_p(env, function))) {
    mkcl_object ndx = c_tag_ref(env, function, (mkcl_object) &MK_KEY_function);
    if (mkcl_Null(ndx)) {
  ...
  if (MKCL_CONSP(function)) {
    ...
    if (kind == ((mkcl_object) &MK_CL_lambda)) {
      asm_op2c(env, OP_CLOSE, mkcl_make_lambda(env, mk_cl_Cnil, form));
      return FLAG_REG0;
    } else if (kind == ((mkcl_object) &MK_SI_lambda_block)) {
...
c_go(MKCL, mkcl_object args, int flags) {
  mkcl_object tag = pop(env, &args);
	mkcl_object info = c_tag_ref(env, tag, (mkcl_object) &MK_KEY_tag);
	if (mkcl_Null(info))
...
c_load_time_value(MKCL, mkcl_object args, int flags)
{
...

  {
    ...
    { /* Assure that this is the NULL environment, both dynamically and lexically. */
      ...
      mkcl_object quoted_val = mkcl_list1(env, mkcl_cons(env, (mkcl_object) &MK_CL_quote, mkcl_list1(env, val)));
...
c_macrolet(MKCL, mkcl_object args, int flags)
{
	...
	mkcl_object m_env = mkcl_funcall2(env,
					  MK_SI_cmp_env_register_macrolet.gfdef,
					  pop(env, &args),
...
c_multiple_value_setq(MKCL, mkcl_object orig_args, int flags) {
	  ...
	  mkcl_object v = pop(env, &orig_vars);
		...
		if (!MKCL_SYMBOLP(v)) {
			...
			return compile_form(env, mk_cl_listX(env, 3,
							     (mkcl_object) &MK_CL_setf,
							     MKCL_CONS(env, (mkcl_object) &MK_CL_values, MKCL_CAR(args)),
							     MKCL_CDR(args)),
					    flags);
		}
	...
	if (nvars == 0) {
		/* No variables */
	  return compile_form(env, mk_cl_list(env, 2, (mkcl_object) &MK_CL_values, values), flags);
	}
...
c_psetq(MKCL, mkcl_object old_args, int flags) {
	...
	*/
	...
	if (use_psetf) {
	  return compile_form(env, MKCL_CONS(env, (mkcl_object) &MK_CL_psetf, args), flags);
	}
...
{
	mkcl_object ndx = c_tag_ref(env, name, (mkcl_object) &MK_KEY_block);
	...
	do {
	  ...
	  mkcl_object value = pop(env, &args);
		...
		if (MKCL_SYMBOLP(var)) {
		  ...
		  compile_form(env, mk_cl_list(env, 3, (mkcl_object) &MK_CL_setf, var, value), flags);
		}
	} while (!mkcl_endp(env, args));
	return flags;
}
...
declared special and appear in a symbol-macrolet.", 1, name);
		...
		definition = mk_cl_list(env, 2, arglist, mk_cl_list(env, 2, (mkcl_object) &MK_CL_quote, expansion));
...
{
  const mk_cl_compiler_ptr c_env = env->c_env;
  mkcl_object code_walker = MKCL_SYM_VAL(env, (mkcl_object) &MK_SI_DYNVAR_code_walker);
  mkcl_object function;
  bool push = flags & FLAG_PUSH;
  int new_flags;

  mkcl_bds_bind(env, (mkcl_object) &MK_SI_DYNVAR_current_form, stmt);
 BEGIN:
  ...
  if (code_walker != MKCL_OBJNULL && !mkcl_Null(code_walker)) {
    stmt = mkcl_funcall2(env, MKCL_SYM_VAL(env,(mkcl_object) &MK_SI_DYNVAR_code_walker), stmt,
			 MKCL_CONS(env, c_env->variables, c_env->macros));
  }
  /*
  ...
  if (!MKCL_SYMBOLP(function))
    goto ORDINARY_CALL;
  if (function == ((mkcl_object) &MK_CL_quote)) {
  ...
  }
  {
	...
	if (c_env->stepping && function != (mkcl_object) &MK_CL_function &&
	    c_env->lexical_level)
...
#define push_var(e, v, list)					\
  if (context == ((mkcl_object) &MK_CL_function)) {		\
 ...
 */
struct mkcl_cfun mk_si_process_declarations_cfunobj = MKCL_CFUN_VA(mk_si_process_declarations, (mkcl_object) &MK_SI_process_declarations);
...
mkcl_object mk_si_process_declarations(MKCL, mkcl_narg narg, mkcl_object body, ...)
{
  mkcl_call_stack_check(env);
  {
    ...
    MKCL_RECEIVE_1_OPTIONAL_ARGUMENT(env, (mkcl_object) &MK_SI_process_declarations, narg, 1, body, &doc);
    ...
    for (; !mkcl_endp(env, body); body = MKCL_CONS_CDR(body)) {
      ...
      if (MKCL_ATOM(form) || (MKCL_CONS_CAR(form) != ((mkcl_object) &MK_CL_declare)))
        break;
      ...
      for (decls = MKCL_CONS_CDR(form); !mkcl_endp(env, decls); decls = MKCL_CONS_CDR(decls)) {
        ...
        if (MKCL_CONS_CAR(sentence) == ((mkcl_object) &MK_CL_special))
...
mk_si_process_lambda(MKCL, mkcl_object lambda)
{
	...
	MKCL_VALUES(0) = mk_si_process_lambda_list(env, lambda_list, (mkcl_object) &MK_CL_function);
...
struct mkcl_cfun mk_si_process_lambda_list_cfunobj = MKCL_CFUN2(mk_si_process_lambda_list, (mkcl_object) &MK_SI_process_lambda_list);
 ...
 LOOP:
  if (MKCL_ATOM(lambda_list)) {
    if (lambda_list == mk_cl_Cnil)
      goto OUTPUT;
    else if (context == ((mkcl_object) &MK_CL_function))
      goto ILLEGAL_LAMBDA;
    else {
      v = lambda_list;
  ...
  if (v == ((mkcl_object) &MK_CL_LKEY_optional)) {
    if (stage >= AT_OPTIONALS)
      goto ILLEGAL_LAMBDA;
    stage = AT_OPTIONALS;
    goto LOOP;
  }
  if (v == ((mkcl_object) &MK_CL_LKEY_rest) || (v == ((mkcl_object) &MK_CL_LKEY_body) && (context == ((mkcl_object) &MK_SI_macro) || context == ((mkcl_object) &MK_CL_destructuring_bind)))) {
    if (MKCL_ATOM(lambda_list))
      goto ILLEGAL_LAMBDA;
    v = MKCL_CONS_CAR(lambda_list);
    lambda_list = MKCL_CONS_CDR(lambda_list);
  ...
  if (v == ((mkcl_object) &MK_CL_LKEY_key)) {
    if (stage >= AT_KEYS)
      goto ILLEGAL_LAMBDA;
    key_flag = mk_cl_Ct;
    stage = AT_KEYS;
  ...
  if (v == ((mkcl_object) &MK_CL_LKEY_aux)) {
    if (stage >= AT_AUXS)
      goto ILLEGAL_LAMBDA;
    stage = AT_AUXS;
    goto LOOP;
  }
  if (v == ((mkcl_object) &MK_CL_LKEY_allow_other_keys)) {
...
mkcl_make_lambda(MKCL, mkcl_object name, mkcl_object lambda)
{
  ...
  mkcl_bds_bind(env, (mkcl_object) &MK_SI_DYNVAR_current_form,
		mk_cl_listX(env, 3, (mkcl_object) &MK_SI_lambda_block, name, lambda));
  ...
  if (!mkcl_Null(name)) {
    compile_form(env, mk_cl_listX(env, 3, (mkcl_object) &MK_CL_block, mk_si_function_block_name(env, name), body),
                 FLAG_VALUES);
  } else {
  ...
  if ( mkcl_Null(mkcl_symbol_value(env, (mkcl_object) &MK_SI_DYNVAR_keep_definitions)) )
    lambda = mk_cl_Cnil;
  output = asm_end(env, handle, lambda);
  output->bytecode.name = name;
  output->bytecode.definition 
    = mkcl_Null(mkcl_symbol_value(env, (mkcl_object) &MK_SI_DYNVAR_keep_definitions))
    ? mk_cl_Cnil : lambda;
...
mkcl_function_block_name(mkcl_object name)
{
  if (MKCL_SYMBOLP(name)) {
    return name;
  } else if (MKCL_CONSP(name) && MKCL_CONS_CAR(name) == ((mkcl_object) &MK_CL_setf)) {
...
struct mkcl_cfun mk_si_function_block_name_cfunobj = MKCL_CFUN1(mk_si_function_block_name, (mkcl_object) &MK_SI_function_block_name);
...
struct mkcl_cfun mk_si_valid_function_name_p_cfunobj = MKCL_CFUN1(mk_si_valid_function_name_p, (mkcl_object) &MK_SI_valid_function_name_p);
...
struct mkcl_cfun mk_si_make_lambda_cfunobj = MKCL_CFUN2(mk_si_make_lambda, (mkcl_object) &MK_SI_make_lambda);
...
struct mkcl_cfun mk_si_eval_in_env_cfunobj = MKCL_CFUN_VA(mk_si_eval_in_env, (mkcl_object) &MK_SI_eval_in_env);
...
mkcl_object mk_si_eval_in_env(MKCL, mkcl_narg narg, mkcl_object form, ...)
{
  mkcl_call_stack_check(env);
  {
    ...
    MKCL_RECEIVE_4_OPTIONAL_ARGUMENTS(env, (mkcl_object) &MK_SI_eval_in_env, narg, 1, form, &lex_env, &stepping, &compiler_env_p, &execute);
