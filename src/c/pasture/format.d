...
@
  if (Null(strm)) {
#ifdef ECL_UNICODE
    strm = ecl_alloc_adjustable_extended_string(env, 64);
#else
    strm = cl_alloc_adjustable_base_string(env, 64);
#endif
    null_strm = 1;
  } else if (strm == Ct) {
    strm = ecl_symbol_value(env, @'*standard-output*');
  }
  if (ecl_stringp(env, strm)) {
    output = strm;
    if (!output->base_string.hasfillp) {
      cl_error(env, 7, @'si::format-error',
	       @':format-control',
	       make_constant_base_string(env, 
					 "Cannot output to a non adjustable string."),
	       @':control-string', string,
	       @':offset', MAKE_FIXNUM(0));
    }
    strm = si_make_string_output_stream_from_string(env, strm);
    if (null_strm == 0)
      output = Cnil;
  }
  if (!Null(cl_functionp(env, string))) {
    cl_apply(env, 3, string, strm, cl_grab_rest_args(env, args));
  } else {
#ifdef ECL_CMU_FORMAT
    cl_funcall(env, 4, @'si::formatter-aux', strm, string,
