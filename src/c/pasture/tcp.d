...
/*
    ...
    MECL is free software; you can redistribute it and/or modify it
 ...
 */
static
int connect_to_server(MECL, char *host, int port)
{
...
static int
create_server_port(MECL, int port)
{
  struct sockaddr_in inaddr;	/* INET socket address. */
  struct sockaddr *addr;	/* address to connect to */
  int addrlen;			/* length of address */
  ...
  if (bind(request, (struct sockaddr *)&inaddr, sizeof (inaddr)))
    FElibc_error(env, "Binding TCP socket", 0);
  if (listen(request, 1))
    FElibc_error(env, "TCP listening", 0);
#if 0
...
#else
  if ((conn = accept(request, (struct sockaddr *)NULL, (int *)NULL)) < 0)
    FElibc_error(env, "Accepting requests", 0);
#endif	/* THREADS */
...
*/
cl_object
si_open_client_stream(MECL, cl_object host, cl_object port)
{
   int fd, p;			/* file descriptor */
   cl_object stream;

   /* Ensure "host" is a string that we can pass to a C function */
   host = si_copy_to_simple_base_string(env, host);

   p = ecl_fixnum_in_range(env, @'si::open-client-stream', "port", port, 0, 65535);

   if (host->base_string.fillp > BUFSIZ - 1)
     FEerror(env, "~S is a too long file name.", 1, host);

   ecl_disable_interrupts();
   fd = connect_to_server(env, (char*)host->base_string.self, fix(port)); 
   ecl_enable_interrupts();
...
#if defined(_MSC_VER) || defined(mingw32)
       stream = ecl_make_stream_from_fd(env, host, fd, smm_io_wsock, 8, 0, Cnil);
#else
   stream = ecl_make_stream_from_fd(env, host, fd, smm_io, 8, 0, Cnil);
#endif
...
cl_object
si_open_server_stream(MECL, cl_object port)
{
  ...
  p = ecl_fixnum_in_range(env, @'si::open-client-stream', "port", port, 0, 65535);
  ecl_disable_interrupts();
  fd = create_server_port(env, p);
  ecl_enable_interrupts();

  @(return ((fd == 0) ? Cnil : ecl_make_stream_from_fd(env, Cnil, fd, smm_io, 8, 0, Cnil)));
}

/************************************************************
 * Unix sockets						    *
...
cl_object
si_open_unix_socket_stream(MECL, cl_object path)
{
#if defined(_MSC_VER) || defined(mingw32)
  ...
  if (type_of(path) != t_base_string)
    FEwrong_type_argument(env, @'string', path);
  if (path->base_string.fillp > UNIX_MAX_PATH-1)
    FEerror(env, "~S is a too long file name.", 1, path);

  fd = socket(PF_UNIX, SOCK_STREAM, 0);
  if (fd < 0) {
    FElibc_error(env, "Unable to create unix socket", 0);
    @(return Cnil);
  }
  ...
  if (connect(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    close(fd);
    FElibc_error(env, "Unable to connect to unix socket ~A", 1, path);
    @(return Cnil);
  }

  @(return ecl_make_stream_from_fd(env, path, fd, smm_io, 8, 0, Cnil));
#endif
}

/************************************************************
 ...
 ************************************************************/
cl_object
si_lookup_host_entry(MECL, cl_object host_or_address)
{
  const mecl_env the_env = env;
  struct hostent *he;
  unsigned long l;
  char address[4];
  ...
  case t_base_string:
    host_or_address = si_copy_to_simple_base_string(env, host_or_address);
  ...
  addr:	address[0] = l & 0xFF;
    ...
    break;
  default:
    FEerror(env, "LOOKUP-HOST-ENTRY: Number or string expected, got ~S",
	    1, host_or_address);
  }
  if (he == NULL)
    { @(return Cnil Cnil Cnil); }
  name = make_base_string_copy(env, he->h_name);
  aliases = Cnil;
  for (i = 0; he->h_aliases[i] != 0; i++)
    aliases = CONS(env, make_base_string_copy(env, he->h_aliases[i]), aliases);
  addresses = Cnil;
  for (i = 0; he->h_addr_list[i]; i++) {
    unsigned long *s = (unsigned long*)(he->h_addr_list[i]);
    l = *s;
    addresses = CONS(env, ecl_make_integer(env, l), addresses);
  }
