 ...
 BEGIN: /* This label is need by the non-threaded bytecode interpreter. */
  ...
  DO_CALL: {
    ...
    */
	...
	mkcl_FEprogram_error(env, "Too many arguments passed to "
			...
			2, bytecode, mk_cl_apply(env, 2, (mkcl_object) &MK_CL_list, frame));
      THREAD_NEXT;
    }
    /* OP_POPREST
    ...
    */
	...
	if (mkcl_Null(aok)) {
	  ...
	  mkcl_object *p = first;
	  for (; p != last; ++p) {
	    if (*(p++) == ((mkcl_object) &MK_KEY_allow_other_keys)) {
	  ...
	  if (count && (aok & 1) == 0) {
	    mkcl_FEprogram_error(env, "Unknown keyword argument "
				 ...
				 "Argument list: ~S",
				 2, bytecode,
				 mk_cl_apply(env, 2, (mkcl_object) &MK_CL_list, frame));
	  }
	}
      }
      THREAD_NEXT;
  ...
  PUSH_VALUES:
    ...
    */
    ...
    CASE(OP_STEPIN); {
      mkcl_object form;
      mkcl_object a = MKCL_SYM_VAL(env, (mkcl_object) &MK_SI_DYNVAR_step_action);
      mkcl_index n;
	 ...
	 * what to do. */
	MKCL_SETQ(env, (mkcl_object) &MK_SI_DYNVAR_step_level,
		 mk_cl_1P(env, MKCL_SYM_VAL(env, (mkcl_object) &MK_SI_DYNVAR_step_level)));
	MKCL_TEMP_STACK_PUSH(env, form);
	INTERPRET_FUNCALL(form, env, frame_aux, 1, (mkcl_object) &MK_SI_stepper);
	...
	MKCL_SETQ(env, (mkcl_object) &MK_SI_DYNVAR_step_action, mk_cl_1P(env, a));
      } else {
    ...
    CASE(OP_STEPCALL); {
      ...
      GET_OPARG(n, vector);
      SETUP_ENV(env);
      if (MKCL_SYM_VAL(env, (mkcl_object) &MK_SI_DYNVAR_step_action) == mk_cl_Ct) {
	MKCL_TEMP_STACK_PUSH(env, reg0);
	INTERPRET_FUNCALL(reg0, env, frame_aux, 1, (mkcl_object) &MK_SI_stepper);
      }
	...
	MKCL_SETQ(env, (mkcl_object) &MK_SI_DYNVAR_step_level,
		 mk_cl_1M(env, MKCL_SYM_VAL(env, (mkcl_object) &MK_SI_DYNVAR_step_level)));
      } else if (a == MKCL_MAKE_FIXNUM(0)) {
	/* We are back to the level in which the user
	 * selected to step over. */
	MKCL_SETQ(env, (mkcl_object) &MK_SI_DYNVAR_step_action, mk_cl_Ct);
      } else if (a != mk_cl_Cnil) {
	MKCL_SETQ(env, (mkcl_object) &MK_SI_DYNVAR_step_action, mk_cl_1M(env, a));
      } else {
