...
(defun unwind-bds (bds-lcl bds-bind temp-stack-frame ihs-p)
  (declare (fixnum bds-bind))
  (when temp-stack-frame
    (if (stringp temp-stack-frame)
	(wt-nl "mkcl_temp_stack_frame_close(env, " temp-stack-frame ");")
	(wt-nl "MKCL_TEMP_STACK_SET_INDEX(env," temp-stack-frame ");")))
  (when bds-lcl
    (wt-nl "mkcl_bds_unwind(env," bds-lcl ");"))
  (if (< bds-bind 4)
      (dotimes (n bds-bind)
	(declare (fixnum n))
	(wt-nl "mkcl_bds_unwind1(env);"))
      (wt-nl "mkcl_bds_unwind_n(env," bds-bind ");"))
  (case ihs-p
    (IHS (wt-nl "mkcl_ihs_pop(env);"))
    (IHS-ENV (wt-nl "ihs.lex_env = _mkcl_debug_env;"))))

(defun unwind-exit (loc &optional (jump-p nil) &aux (bds-lcl nil) (bds-bind 0) (temp-stack-frame nil) (ihs-p nil))
  (declare (fixnum bds-bind))
    ...
    (cond
      ((consp ue)		    ; ( label# . ref-flag )| (STACK n) |(LCL n)
       (cond ((eq (car ue) 'STACK)
	      (setf temp-stack-frame (second ue)))
	     ((eq (car ue) 'LCL)
	     ...
	     ((eq ue *exit*)
	      ...
	      (cond ((and (consp *destination*)
		     ...
		     (unwind-bds bds-lcl bds-bind temp-stack-frame ihs-p))
		    ((not (or bds-lcl (plusp bds-bind) temp-stack-frame))
		     (set-loc loc))
...
(defun unwind-no-exit (exit &aux (bds-lcl nil) (bds-bind 0) (temp-stack-frame nil) (ihs-p nil))
  (declare (fixnum bds-bind))
  (dolist (ue *unwind-exit* (baboon))
    (cond
       ((consp ue)
	(cond ((eq ue exit)
       ...
       ((member ue '(RETURN RETURN-OBJECT RETURN-FIXNUM RETURN-CHARACTER
        ...
        (if (eq exit ue)
          (progn (unwind-bds bds-lcl bds-bind temp-stack-frame ihs-p)
                 (return))
          (baboon))
        ;;; Never reached
        )
