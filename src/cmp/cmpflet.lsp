  ...
  (let ((new-env (cmp-env-copy))
    ...
    (dolist (def (car args))
      ...
      (let* ((name (car def))
	     ...
	     (fun (make-fun :name name :var var :lex-local-p t)))
    ...
    (dolist (lfun local-funs)
      (unless (eq (fun-closure lfun) 'CLOSURE)
	(push lfun *lex-local-funs*)))
...
(defun compute-fun-closure-type (fun)
  (labels
	...
	#+(or)
	(when (fun-global fun) ;; why must this be an error? JCB
...
(defun c1macrolet (args)
  ...
  (let ((*cmp-env* (cmp-env-copy)))
    (si::cmp-env-register-macrolet (first args) *cmp-env*)
    (c1locally (cdr args))))
...
(defun local-function-ref (fname &optional build-object)
  (multiple-value-bind (fun ccb clb unw)
	...
	(return-from local-function-ref nil))
      ...
      (let ((var (fun-var fun)))
	(incf (var-ref var)) ;; JCB
	(add-to-read-nodes var (make-c1form* 'VAR :args var)) ;; fake var ref. JCB
	(cond (ccb (when t ;; build-object ;; JCB
		     (setf (var-ref-ccb var) t
			   (var-kind var) 'CLOSURE))
		   (setf (fun-ref-ccb fun) t))
	      (clb (when t ;; build-object  ;; JCB
