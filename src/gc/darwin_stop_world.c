...
# else
    MK_GC_ASSERT(stack_start != 0); /* not implemented */
# endif /* !POWERPC */
  /* else */ {
...
# ifdef DEBUG_THREADS_EXTRA
    MK_GC_log_printf("FindTopOfStack start at sp = %p\n", frame);
# endif
...
# ifdef DEBUG_THREADS_EXTRA
    MK_GC_log_printf("FindTopOfStack finish at sp = %p\n", frame);
# endif
  return (ptr_t)frame;
}
...
# endif
# ifdef DEBUG_THREADS
    MK_GC_log_printf("Darwin: Stack for thread %p = [%p,%p)\n",
                  (void *)thread, lo, *phi);
# endif
  return lo;
}
...
#   ifdef DEBUG_THREADS
      MK_GC_log_printf("Attempting to suspend thread %p\n", (void *)thread);
#   endif
...
#   ifdef DEBUG_THREADS
      MK_GC_log_printf("Thread state for %p = %d\n", (void *)thread, info.run_state);
#   endif
...
# ifdef DEBUG_THREADS
    MK_GC_log_printf("Stopping the world from thread %p\n", (void *)my_thread);
# endif
# ifdef PARALLEL_MARK
    if (MK_GC_parallel) {
...
# endif
# ifdef DEBUG_THREADS
    MK_GC_log_printf("Resuming thread %p with state %d\n", (void *)thread,
                  info.run_state);
# endif
  /* Resume the thread */
...
#           ifdef DEBUG_THREADS
              MK_GC_log_printf("Not resuming already suspended thread %p\n",
                            (void *)thread);
#           endif
          } else {
