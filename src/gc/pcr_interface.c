/*
 * Copyright (c) 1991-1994 by Xerox Corporation.  All rights reserved.
 *
 * THIS MATERIAL IS PROVIDED AS IS, WITH ABSOLUTELY NO WARRANTY EXPRESSED
 * OR IMPLIED.  ANY USE IS AT YOUR OWN RISK.
 *
 * Permission is hereby granted to use or copy this program
 * for any purpose,  provided the above notices are retained on all copies.
 * Permission to modify the code and to distribute modified code is granted,
 * provided the above notices are retained, and a notice that the code was
 * modified is included with the above copyright notice.
 */
# include "private/gc_priv.h"

# ifdef PCR
/*
 * Note that POSIX PCR requires an ANSI C compiler.  Hence we are allowed
 * to make the same assumption here.
 * We wrap all of the allocator functions to avoid questions of
 * compatibility between the prototyped and nonprototyped versions of the f
 */
# include "config/PCR_StdTypes.h"
# include "mm/PCR_MM.h"
# include <errno.h>

# define MY_MAGIC 17L
# define MY_DEBUGMAGIC 42L

void * MK_GC_AllocProc(size_t size, PCR_Bool ptrFree, PCR_Bool clear )
{
    if (ptrFree) {
        void * result = (void *)MK_GC_malloc_atomic(size);
        if (clear && result != 0) BZERO(result, size);
        return(result);
    } else {
        return((void *)MK_GC_malloc(size));
    }
}

void * MK_GC_DebugAllocProc(size_t size, PCR_Bool ptrFree, PCR_Bool clear )
{
    if (ptrFree) {
        void * result = (void *)MK_GC_debug_malloc_atomic(size, __FILE__,
                                                             __LINE__);
        if (clear && result != 0) BZERO(result, size);
        return(result);
    } else {
        return((void *)MK_GC_debug_malloc(size, __FILE__, __LINE__));
    }
}

# define MK_GC_ReallocProc MK_GC_realloc
void * MK_GC_DebugReallocProc(void * old_object, size_t new_size_in_bytes)
{
    return(MK_GC_debug_realloc(old_object, new_size_in_bytes, __FILE__, __LINE__));
}

# define MK_GC_FreeProc MK_GC_free
# define MK_GC_DebugFreeProc MK_GC_debug_free

typedef struct {
  PCR_ERes (*ed_proc)(void *p, size_t size, PCR_Any data);
  MK_GC_bool ed_pointerfree;
  PCR_ERes ed_fail_code;
  PCR_Any ed_client_data;
} enumerate_data;

void MK_GC_enumerate_block(struct hblk *h; enumerate_data * ed)
{
    register hdr * hhdr;
    register int sz;
    ptr_t p;
    ptr_t lim;
    word descr;
#   error This code was updated without testing.
#   error and its precursor was clearly broken.

    hhdr = HDR(h);
    descr = hhdr -> hb_descr;
    sz = hhdr -> hb_sz;
    if (descr != 0 && ed -> ed_pointerfree
        || descr == 0 && !(ed -> ed_pointerfree)) return;
    lim = (ptr_t)(h+1) - sz;
    p = (ptr_t)h;
    do {
        if (PCR_ERes_IsErr(ed -> ed_fail_code)) return;
        ed -> ed_fail_code =
            (*(ed -> ed_proc))(p, sz, ed -> ed_client_data);
        p+= sz;
    } while ((word)p <= (word)lim);
}

struct PCR_MM_ProcsRep * MK_GC_old_allocator = 0;

PCR_ERes MK_GC_EnumerateProc(
    PCR_Bool ptrFree,
    PCR_ERes (*proc)(void *p, size_t size, PCR_Any data),
    PCR_Any data
)
{
    enumerate_data ed;

    ed.ed_proc = proc;
    ed.ed_pointerfree = ptrFree;
    ed.ed_fail_code = PCR_ERes_okay;
    ed.ed_client_data = data;
    MK_GC_apply_to_all_blocks(MK_GC_enumerate_block, &ed);
    if (ed.ed_fail_code != PCR_ERes_okay) {
        return(ed.ed_fail_code);
    } else {
        /* Also enumerate objects allocated by my predecessors */
        return((*(MK_GC_old_allocator->mmp_enumerate))(ptrFree, proc, data));
    }
}

void MK_GC_DummyFreeProc(void *p) {}

void MK_GC_DummyShutdownProc(void) {}

struct PCR_MM_ProcsRep MK_GC_Rep = {
        MY_MAGIC,
        MK_GC_AllocProc,
        MK_GC_ReallocProc,
        MK_GC_DummyFreeProc,       /* mmp_free */
        MK_GC_FreeProc,            /* mmp_unsafeFree */
        MK_GC_EnumerateProc,
        MK_GC_DummyShutdownProc    /* mmp_shutdown */
};

struct PCR_MM_ProcsRep MK_GC_DebugRep = {
        MY_DEBUGMAGIC,
        MK_GC_DebugAllocProc,
        MK_GC_DebugReallocProc,
        MK_GC_DummyFreeProc,       /* mmp_free */
        MK_GC_DebugFreeProc,               /* mmp_unsafeFree */
        MK_GC_EnumerateProc,
        MK_GC_DummyShutdownProc    /* mmp_shutdown */
};

MK_GC_bool MK_GC_use_debug = 0;

void MK_GC_pcr_install()
{
    PCR_MM_Install((MK_GC_use_debug? &MK_GC_DebugRep : &MK_GC_Rep), &MK_GC_old_allocator);
}

PCR_ERes
PCR_MK_GC_Setup(void)
{
    return PCR_ERes_okay;
}

PCR_ERes
PCR_MK_GC_Run(void)
{

    if( !PCR_Base_TestPCRArg("-nogc") ) {
        MK_GC_quiet = ( PCR_Base_TestPCRArg("-gctrace") ? 0 : 1 );
        MK_GC_use_debug = (MK_GC_bool)PCR_Base_TestPCRArg("-debug_alloc");
        MK_GC_init();
        if( !PCR_Base_TestPCRArg("-nogc_incremental") ) {
            /*
             * awful hack to test whether VD is implemented ...
             */
            if( PCR_VD_Start( 0, NIL, 0) != PCR_ERes_FromErr(ENOSYS) ) {
                MK_GC_enable_incremental();
            }
        }
    }
    return PCR_ERes_okay;
}

void MK_GC_push_thread_structures(void)
{
    /* PCR doesn't work unless static roots are pushed.  Can't get here. */
    ABORT("In MK_GC_push_thread_structures()");
}

# endif
