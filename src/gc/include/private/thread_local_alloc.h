...
/* implementation also exports MK_GC_malloc and friends, which     */
/* are declared in gc.h.                                        */

#ifndef MK_GC_THREAD_LOCAL_ALLOC_H
#define MK_GC_THREAD_LOCAL_ALLOC_H
...
#   define USE_COMPILER_TLS
# elif defined(MK_GC_DGUX386_THREADS) || defined(MK_GC_OSF1_THREADS) \
       || defined(MK_GC_AIX_THREADS) || defined(MK_GC_DARWIN_THREADS) \
       || defined(MK_GC_FREEBSD_THREADS) || defined(MK_GC_NETBSD_THREADS) \
       || defined(MK_GC_LINUX_THREADS) || defined(MK_GC_RTEMS_PTHREADS)
#   define USE_PTHREAD_SPECIFIC
# elif defined(MK_GC_HPUX_THREADS)
#   ifdef __GNUC__
#     define USE_PTHREAD_SPECIFIC
...
/* structure pointed to by a MK_GC_thread.                         */
...
# define MK_GC_getspecific pthread_getspecific
# define MK_GC_setspecific pthread_setspecific
# define MK_GC_key_create pthread_key_create
# define MK_GC_remove_specific(key)  /* No need for cleanup on exit. */
  typedef pthread_key_t MK_GC_key_t;
#elif defined(USE_COMPILER_TLS) || defined(USE_WIN32_COMPILER_TLS)
# define MK_GC_getspecific(x) (x)
# define MK_GC_setspecific(key, v) ((key) = (v), 0)
# define MK_GC_key_create(key, d) 0
# define MK_GC_remove_specific(key)  /* No need for cleanup on exit. */
  typedef void * MK_GC_key_t;
...
#ifdef ENABLE_DISCLAIM
  MK_GC_EXTERN ptr_t * MK_GC_finalized_objfreelist;
#endif
...
/* that MK_GC_thread_key is traced, if necessary.                          */

#endif /* THREAD_LOCAL_ALLOC */

#endif /* MK_GC_THREAD_LOCAL_ALLOC_H */
