...
#else
# define START_FLAG MK_GC_WORD_C(0xFEDCEDCBfedcedcb)
# define END_FLAG MK_GC_WORD_C(0xBCDECDEFbcdecdef)
#endif
...
#     define HIDE_BACK_PTR(p) MK_GC_HIDE_POINTER(~1 & (MK_GC_word)(p))
#   else
#     define HIDE_BACK_PTR(p) MK_GC_HIDE_POINTER(p)
#   endif
#   ifdef KEEP_BACK_PTRS
      MK_GC_hidden_pointer oh_back_ptr;
#   endif
#   ifdef MAKE_BACK_GRAPH
      MK_GC_hidden_pointer oh_bg_ptr;
#   endif
#   if defined(KEEP_BACK_PTRS) != defined(MAKE_BACK_GRAPH)
...
# define ADD_CALL_CHAIN(base, ra) MK_GC_save_callers(((oh *)(base)) -> oh_ci)
# define PRINT_CALL_CHAIN(base) MK_GC_print_callers(((oh *)(base)) -> oh_ci)
#elif defined(MK_GC_ADD_CALLER)
  struct callinfo;
  MK_GC_INNER void MK_GC_print_callers(struct callinfo info[NFRAMES]);
...
/* of the heap.                                                 */
#ifdef SHORT_DBG_HDRS
# define MK_GC_has_other_debug_info(p) 1
#else
  MK_GC_INNER int MK_GC_has_other_debug_info(ptr_t p);
#endif

#if defined(KEEP_BACK_PTRS) || defined(MAKE_BACK_GRAPH)
# define MK_GC_HAS_DEBUG_INFO(p) \
        ((*((word *)p) & 1) && MK_GC_has_other_debug_info(p) > 0)
#else
# define MK_GC_HAS_DEBUG_INFO(p) (MK_GC_has_other_debug_info(p) > 0)
#endif
