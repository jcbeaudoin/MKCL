...
# ifndef MK_GC_PRIVATE_H
    ...
    /* This avoids many instances if "ifndef MK_GC_PRIVATE_H" below.       */
    typedef struct MK_GC_undefined_struct * ptr_t;
 ...
 * SEARCH_FOR_DATASTART will cause MK_GC_data_start to
 ...
 * HEURISTIC1:  Take an address inside MK_GC_init's frame, and round it up to
 *              the next multiple of STACK_GRAN.
 * HEURISTIC2:  Take an address inside MK_GC_init's frame, increment it repeatedly
 ...
 * 1) MK_GC_mark_roots can be changed to somehow mark from the correct stack(s)
 ...
 * 2) Client code may set MK_GC_stackbottom before calling any MK_GC_ routines.
 *    If the author of the client code controls the main program, this is
 *    easily accomplished by introducing a new main program, setting
 *    MK_GC_stackbottom to the address of a local variable, and then calling
 ...
 * defined MK_GC_register_dynamic_libraries() for the architecture.
 *
 ...
 * clear the two words at MK_GC_malloc-aligned address x.  By default,
...
#       define HEURISTIC2
#       ifdef __ELF__
          extern ptr_t MK_GC_data_start;
#         define DATASTART MK_GC_data_start
...
#       endif
        extern char etext[];
        ptr_t MK_GC_FreeBSDGetDataStart(size_t, ptr_t);
#       define DATASTART MK_GC_FreeBSDGetDataStart(0x1000, (ptr_t)etext)
#       define DATASTART_IS_FUNC
#   endif
#   ifdef NETBSD
#     define ALIGNMENT 4
...
#       define OS_TYPE "SOLARIS"
        extern int _etext[];
        extern int _end[];
        ptr_t MK_GC_SysVGetDataStart(size_t, ptr_t);
#       define DATASTART MK_GC_SysVGetDataStart(0x10000, (ptr_t)_etext)
#       define DATASTART_IS_FUNC
#       define DATAEND (ptr_t)(_end)
...
#       define OS_TYPE "DRSNX"
        ptr_t MK_GC_SysVGetDataStart(size_t, ptr_t);
        extern int etext[];
#       define DATASTART MK_GC_SysVGetDataStart(0x10000, (ptr_t)etext)
#       define DATASTART_IS_FUNC
#       define MPROTECT_VDB
...
#     define SVR4
      ptr_t MK_GC_SysVGetDataStart(size_t, ptr_t);
#     ifdef __arch64__
#       define DATASTART MK_GC_SysVGetDataStart(0x100000, (ptr_t)_etext)
#     else
#       define DATASTART MK_GC_SysVGetDataStart(0x10000, (ptr_t)_etext)
#     endif
#     define DATASTART_IS_FUNC
...
#       define DATAEND (MK_GC_find_limit (DATASTART, TRUE))
...
#       define OS_TYPE "SOLARIS"
        ...
        ptr_t MK_GC_SysVGetDataStart(size_t, ptr_t);
#       define DATASTART MK_GC_SysVGetDataStart(0x1000, (ptr_t)_etext)
#       define DATASTART_IS_FUNC
#       define DATAEND (ptr_t)(_end)
...
#      ifndef MAX_NACL_MK_GC_THREADS
#        define MAX_NACL_MK_GC_THREADS 1024
#      endif
#   endif /* NACL */
...
#       define DATASTART ((ptr_t)MK_GC_DATASTART)  /* From gc.h */
#       define DATAEND   ((ptr_t)MK_GC_DATAEND)
#       undef STACK_GRAN
#       define STACK_GRAN 0x10000
#       ifdef USE_MMAP
#         define NEED_FIND_LIMIT
...
#       endif
        extern char etext[];
        char * MK_GC_FreeBSDGetDataStart(size_t, ptr_t);
#       define DATASTART MK_GC_FreeBSDGetDataStart(0x1000, (ptr_t)etext)
#       define DATASTART_IS_FUNC
#   endif
#   ifdef NETBSD
...
#     define ALIGNMENT 4
#   endif
#   if !defined(MK_GC_HPUX_THREADS) && !defined(MK_GC_LINUX_THREADS) \
...
#       define BACKING_STORE_ALIGNMENT 0x1000
        extern ptr_t MK_GC_register_stackbottom;
#       define BACKING_STORE_BASE MK_GC_register_stackbottom
...
#       define LINUX_STACKBOTTOM
        ...
        /* MK_GC_linux_register_stack_base based on the following  */
        /* constants:                                           */
...
#   ifdef DARWIN
      /* iPhone */
#     define OS_TYPE "DARWIN"
#     ifndef MK_GC_DONT_REGISTER_MAIN_STATIC_DATA
#       define DYNAMIC_LOADING
#     endif
#     define DATASTART ((ptr_t) get_etext())
#     define DATAEND   ((ptr_t) get_end())
...
# define DATAEND (ptr_t)(end)
  ...
  /* not needed to be specified manually) since MK_GC_linux_main_stack_base */
...
#if !defined(MK_GC_EXPLICIT_SIGNALS_UNBLOCK) && defined(SUNOS5SIGS) \
    && !defined(MK_GC_NO_PTHREAD_SIGMASK)
# define MK_GC_EXPLICIT_SIGNALS_UNBLOCK
#endif

#ifdef MK_GC_NETBSD_THREADS
# define SIGRTMIN 33
# define SIGRTMAX 63
#endif
...
# include <sys/param.h>
  ...
  /* special handling.                                                  */
# if OpenBSD < 201211
#   define MK_GC_OPENBSD_UTHREADS 1
# endif
...
#if defined(SMALL_CONFIG) && !defined(MK_GC_DISABLE_INCREMENTAL)
  /* Presumably not worth the space it takes.   */
# define MK_GC_DISABLE_INCREMENTAL
#endif
...
#if defined(MK_GC_DISABLE_INCREMENTAL) || defined(MANUAL_VDB)
# undef GWW_VDB
# undef MPROTECT_VDB
# undef PCR_VDB
# undef PROC_VDB
...
#if defined(MPROTECT_VDB) && defined(MK_GC_PREFER_MPROTECT_VDB)
...
# undef MPROTECT_VDB
    ...
    && !defined(MK_GC_DISABLE_INCREMENTAL)
# define DEFAULT_VDB
#endif
...
# define REGISTER_LIBRARIES_EARLY
  ...
  /* call MK_GC_register_dynamic_libraries before stopping the world.      */
...
#if defined(MK_GC_IRIX_THREADS) && !defined(IRIX5)
# error --> inconsistent configuration
#endif
#if defined(MK_GC_LINUX_THREADS) && !defined(LINUX) && !defined(NACL)
# error --> inconsistent configuration
#endif
#if defined(MK_GC_NETBSD_THREADS) && !defined(NETBSD)
# error --> inconsistent configuration
#endif
#if defined(MK_GC_FREEBSD_THREADS) && !defined(FREEBSD)
# error --> inconsistent configuration
#endif
#if defined(MK_GC_SOLARIS_THREADS) && !defined(SOLARIS)
# error --> inconsistent configuration
#endif
#if defined(MK_GC_HPUX_THREADS) && !defined(HPUX)
# error --> inconsistent configuration
#endif
#if defined(MK_GC_AIX_THREADS) && !defined(_AIX)
# error --> inconsistent configuration
#endif
#if defined(MK_GC_GNU_THREADS) && !defined(HURD)
# error --> inconsistent configuration
#endif
#if defined(MK_GC_WIN32_THREADS) && !defined(MSWIN32) && !defined(CYGWIN32) \
    && !defined(MSWINCE)
# error --> inconsistent configuration
#endif
...
# error "invalid config - PARALLEL_MARK requires MK_GC_THREADS"
#endif
...
# define IF_CANCEL(x) /* empty */
    ...
    && ((defined(MK_GC_PTHREADS) && !defined(NACL) \
         && !defined(MK_GC_WIN32_PTHREADS) && !defined(USE_WINALLOC)) \
        || (defined(DARWIN) && defined(MPROTECT_VDB)) || defined(HANDLE_FORK))
  /* Attempts (where supported and requested) to make MK_GC_malloc work in */
...
#ifndef MK_GC_WORD_C
# if defined(_WIN64) && !defined(__GNUC__)
#   define MK_GC_WORD_C(val) val##ui64
# elif defined(_LLP64) || defined(__LLP64__) || defined(_WIN64)
#   define MK_GC_WORD_C(val) val##ULL
# else
#   define MK_GC_WORD_C(val) ((word)val##UL)
# endif
#endif /* !MK_GC_WORD_C */
...
/* MK_GC_ADD_CALLER is defined.                                            */
/* MK_GC_CAN_SAVE_CALL_STACKS is set in gc.h.                              */
#if defined(SPARC)
...
#if defined(SAVE_CALL_COUNT) && !defined(MK_GC_ADD_CALLER) \
    && defined(MK_GC_CAN_SAVE_CALL_STACKS)
...
# define POINTER_MASK ((MK_GC_word)(-1))
#endif
...
#   define GET_MEM(bytes) HBLKPTR(real_malloc((size_t)(bytes) + MK_GC_page_size) \
                                          + MK_GC_page_size-1)
# elif defined(OS2)
    void * os2_alloc(size_t bytes);
#   define GET_MEM(bytes) HBLKPTR((ptr_t)os2_alloc((size_t)(bytes) \
                                            + MK_GC_page_size) + MK_GC_page_size-1)
# elif defined(NEXT) || defined(DOS4GW) || defined(NONSTOP) \
        || (defined(AMIGA) && !defined(MK_GC_AMIGA_FASTALLOC)) \
        || (defined(SOLARIS) && !defined(USE_MMAP)) || defined(RTEMS) \
        || defined(__CC_ARM)
#   define GET_MEM(bytes) HBLKPTR((size_t)calloc(1, \
                                            (size_t)(bytes) + MK_GC_page_size) \
                                  + MK_GC_page_size - 1)
# elif defined(MSWIN32) || defined(CYGWIN32)
    ptr_t MK_GC_win32_get_mem(MK_GC_word bytes);
#   define GET_MEM(bytes) (struct hblk *)MK_GC_win32_get_mem(bytes)
# elif defined(MACOS)
#   if defined(USE_TEMPORARY_MEMORY)
      Ptr MK_GC_MacTemporaryNewPtr(size_t size, Boolean clearMemory);
#     define GET_MEM(bytes) HBLKPTR( \
                        MK_GC_MacTemporaryNewPtr((bytes) + MK_GC_page_size, true) \
                        + MK_GC_page_size-1)
#   else
#     define GET_MEM(bytes) HBLKPTR(NewPtrClear((bytes) + MK_GC_page_size) \
                                    + MK_GC_page_size-1)
#   endif
# elif defined(MSWINCE)
    ptr_t MK_GC_wince_get_mem(MK_GC_word bytes);
#   define GET_MEM(bytes) (struct hblk *)MK_GC_wince_get_mem(bytes)
# elif defined(AMIGA) && defined(MK_GC_AMIGA_FASTALLOC)
    void *MK_GC_amiga_get_mem(size_t size);
#   define GET_MEM(bytes) HBLKPTR((size_t) \
                          MK_GC_amiga_get_mem((size_t)(bytes) + MK_GC_page_size) \
                          + MK_GC_page_size-1)
# elif defined(SN_TARGET_PS3)
    void *ps3_get_mem(size_t size);
#   define GET_MEM(bytes) (struct hblk*)ps3_get_mem(bytes)
# else
    ptr_t MK_GC_unix_get_mem(MK_GC_word bytes);
#   define GET_MEM(bytes) (struct hblk *)MK_GC_unix_get_mem(bytes)
# endif
