...
typedef struct MK_GC_ms_entry {
    ...
    MK_GC_word mse_descr;  /* Descriptor; low order two bits are tags,     */
                        /* as described in gc_mark.h.                   */
} mse;

MK_GC_EXTERN size_t MK_GC_mark_stack_size;

MK_GC_EXTERN mse * MK_GC_mark_stack_limit;

#ifdef PARALLEL_MARK
  MK_GC_EXTERN mse * volatile MK_GC_mark_stack_top;
#else
  MK_GC_EXTERN mse * MK_GC_mark_stack_top;
#endif

MK_GC_EXTERN mse * MK_GC_mark_stack;
...
{ \
    ...
    if (EXPECT((gran_offset | byte_offset) != 0, FALSE))  { \
        if (hhdr -> hb_large_block) { \
          ...
          MK_GC_ASSERT((ptr_t)(hhdr -> hb_block) <= (ptr_t) current); \
        } else { \
...
{ \
    ...
    MK_GC_ASSERT(high_prod >= 0 && high_prod <= HBLK_OBJS(hhdr -> hb_sz)); \
    TRACE(source, MK_GC_log_printf("GC:%u: passed validity tests\n", \
                                (unsigned)MK_GC_gc_no)); \
...
# define MK_GC_PUSH_ONE_STACK(p, source) \
      if ((ptr_t)(p) >= (ptr_t)MK_GC_least_plausible_heap_addr \
          && (ptr_t)(p) < (ptr_t)MK_GC_greatest_plausible_heap_addr) { \
 ...
 */
#define MK_GC_PUSH_ONE_HEAP(p,source) \
    FIXUP_POINTER(p); \
    if ((ptr_t)(p) >= (ptr_t)MK_GC_least_plausible_heap_addr \
         && (ptr_t)(p) < (ptr_t)MK_GC_greatest_plausible_heap_addr) { \
      MK_GC_mark_stack_top = MK_GC_mark_and_push( \
                            (void *)(p), MK_GC_mark_stack_top, \
