...
MK_GC_INNER int MK_GC_key_create_inner(tsd ** key_ptr)
{
    ...
    MK_GC_ASSERT((word)(&invalid_tse.next) % sizeof(tse *) == 0);
...
#   ifdef MK_GC_ASSERTIONS
      for (i = 0; i < TS_HASH_SIZE; ++i) {
        MK_GC_ASSERT(result -> hash[i].p == 0);
      }
#   endif
    *key_ptr = result;
    return 0;
...
MK_GC_INNER int MK_GC_setspecific(tsd * key, void * value)
{
    ...
    MK_AO_store_release(&key->hash[hash_val].ao, (MK_AO_t)entry);
