...
int PREFIXED(key_create) (tsd ** key_ptr, void (* destructor)(void *)) {
    ...
    MK_GC_ASSERT((unsigned long)(&invalid_tse.next) % sizeof(tse *) == 0);
...
int PREFIXED(setspecific) (tsd * key, void * value) {
    ...
    MK_GC_ASSERT(self != INVALID_THREADID);
    ...
    entry -> thread = self;
    entry -> value = value;
    MK_GC_ASSERT(entry -> qtid == INVALID_QTID);
    /* There can only be one writer at a time, but this needs to be     */
    /* atomic with respect to concurrent readers.                       */
    MK_AO_store_release((volatile MK_AO_t *)(key -> hash + hash_val), (MK_AO_t)entry);
...
void * PREFIXED(slow_getspecific) (tsd * key, unsigned long qtid,
  ...
  void PREFIXED(check_tsd_marks) (tsd *key)
  {
    ...
    if (!MK_GC_is_marked(MK_GC_base(key))) {
      ABORT("Unmarked thread-specific-data table");
    }
    for (i = 0; i < TS_HASH_SIZE; ++i) {
      for (p = key -> hash[i]; p != 0; p = p -> next) {
        if (!MK_GC_is_marked(MK_GC_base(p))) {
          MK_GC_err_printf("Thread-specific-data entry at %p not marked\n", p);
          ABORT("Unmarked tse");
        }
      }
    }
    for (i = 0; i < TS_CACHE_SIZE; ++i) {
      p = key -> cache[i];
      if (p != &invalid_tse && !MK_GC_is_marked(MK_GC_base(p))) {
        MK_GC_err_printf("Cached thread-specific-data entry at %p not marked\n",
                      p);
