...
char * CORD_to_char_star(CORD x)
{
    ...
    char * result = MK_GC_MALLOC_ATOMIC(len + 1);
...
CORD CORD_from_char_star(const char *s)
{
    ...
    result = MK_GC_MALLOC_ATOMIC(len + 1);
    if (result == 0) OUT_OF_MEMORY;
...
{
    register chr_data * d = (chr_data *)client_data;
    register const char * occ = strchr(s, d -> target);
...
char CORD_lf_func(size_t i, void * client_data)
{
    ...
    if (cl == 0 || cl -> tag != DIV_LINE_SZ(i)) {
        ...
        rd.state = state;
        rd.file_pos =  i;
        rd.new_cache = MK_GC_NEW_ATOMIC(cache_line);
        if (rd.new_cache == 0) OUT_OF_MEMORY;
        return((char)(MK_GC_word)
              MK_GC_call_with_alloc_lock((MK_GC_fn_type) refill_cache, &rd));
    }
...
{
    register lf_state * state = MK_GC_NEW(lf_state);
    register int i;
    ...
    if (len != 0) {
        ...
        /* is redirected to MK_GC_malloc and the           */
    ...
    state -> lf_current = 0;
    MK_GC_REGISTER_FINALIZER(state, CORD_lf_close_proc, 0, 0, 0);
