...
# if defined(SPARC) || defined(IA64)
    ...
    /* or ar.bsp (IA64).                                                */
    MK_GC_INNER ptr_t MK_GC_save_regs_ret_val = NULL;
# endif
...
#       ifdef __GNUC__
          ...
          asm("mov.l %a2,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %a3,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %a4,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %a5,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %a6,(%sp)"); asm("jsr _MK_GC_push_one");
          /* Skip frame pointer and stack pointer */
          asm("mov.l %d2,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %d3,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %d4,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %d5,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %d6,(%sp)"); asm("jsr _MK_GC_push_one");
          asm("mov.l %d7,(%sp)"); asm("jsr _MK_GC_push_one");

          asm("addq.w &0x4,%sp");       /* put stack back where it was  */
#       else /* !__GNUC__ */
          MK_GC_push_one(getreg(REG_A2));
          MK_GC_push_one(getreg(REG_A3));
#         ifndef __SASC
            /* Can probably be changed to #if 0 -Kjetil M. (a4=globals) */
            MK_GC_push_one(getreg(REG_A4));
#         endif
          MK_GC_push_one(getreg(REG_A5));
          MK_GC_push_one(getreg(REG_A6));
          /* Skip stack pointer */
          MK_GC_push_one(getreg(REG_D2));
          MK_GC_push_one(getreg(REG_D3));
          MK_GC_push_one(getreg(REG_D4));
          MK_GC_push_one(getreg(REG_D5));
          MK_GC_push_one(getreg(REG_D6));
          MK_GC_push_one(getreg(REG_D7));
#       endif /* !__GNUC__ */
    }
#   define HAVE_PUSH_REGS
...
# error MK_GC_push_regs cannot be used with threads
 /* Would fail for MK_GC_do_blocking.  There are probably other safety     */
 /* issues.                                                             */
# undef HAVE_PUSH_REGS
#endif
...
#     if defined(SPARC) || defined(IA64)
        ...
        MK_GC_save_regs_ret_val = MK_GC_save_regs_in_stack();
...
# ifdef LINT
    ptr_t MK_GC_clear_stack_inner(ptr_t arg, word limit)
    {
