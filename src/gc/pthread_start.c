...
/* We want to make sure that MK_GC_thread_exit_proc() is unconditionally   */
/* invoked, even if the client is not compiled with -fexceptions, but   */
/* the GC is.  The workaround is to put MK_GC_inner_start_routine() in its */
...
void * MK_GC_CALLBACK MK_GC_inner_start_routine(struct MK_GC_stack_base *sb, void *arg)
{
  ...
  void * start_arg;
  void * result;
  volatile MK_GC_thread me =
                MK_GC_start_rtn_prepare_thread(&start, &start_arg, sb, arg);
