 ...
 *      ptr_t MK_GC_build_flXXX(h, old_fl)
 *      void MK_GC_new_hblk(size)
 */
...
#include <stdio.h>
  ...
  STATIC ptr_t MK_GC_build_fl_clear2(struct hblk *h, ptr_t ofl)
  {
  ...
  STATIC ptr_t MK_GC_build_fl_clear4(struct hblk *h, ptr_t ofl)
  {
    word * p = (word *)(h -> hb_body);
    word * lim = (word *)(h + 1);
  ...
  STATIC ptr_t MK_GC_build_fl2(struct hblk *h, ptr_t ofl)
  {
    word * p = (word *)(h -> hb_body);
    word * lim = (word *)(h + 1);
  ...
  STATIC ptr_t MK_GC_build_fl4(struct hblk *h, ptr_t ofl)
  {
    word * p = (word *)(h -> hb_body);
    word * lim = (word *)(h + 1);
...
MK_GC_INNER ptr_t MK_GC_build_fl(struct hblk *h, size_t sz, MK_GC_bool clear,
                           ptr_t list)
{
  word *p, *prev;
...
# ifndef SMALL_CONFIG
    ...
    switch (sz) {
        case 2: if (clear) {
                    return MK_GC_build_fl_clear2(h, list);
                } else {
                    return MK_GC_build_fl2(h, list);
                }
        case 4: if (clear) {
                    return MK_GC_build_fl_clear4(h, list);
                } else {
                    return MK_GC_build_fl4(h, list);
                }
        default:
                break;
    }
...
MK_GC_INNER void MK_GC_new_hblk(size_t gran, int kind)
{
  struct hblk *h;       /* the new heap block */
  MK_GC_bool clear = MK_GC_obj_kinds[kind].ok_init;

  MK_GC_STATIC_ASSERT((sizeof (struct hblk)) == HBLKSIZE);

  if (MK_GC_debugging_started) clear = TRUE;

  /* Allocate a new heap block */
    h = MK_GC_allochblk(GRANULES_TO_BYTES(gran), kind, 0);
    if (h == 0) return;

  /* Mark all objects if appropriate. */
      if (IS_UNCOLLECTABLE(kind)) MK_GC_set_hdr_marks(HDR(h));

  /* Build the free list */
      MK_GC_obj_kinds[kind].ok_freelist[gran] =
        MK_GC_build_fl(h, GRANULES_TO_WORDS(gran), clear,
                    MK_GC_obj_kinds[kind].ok_freelist[gran]);
}
