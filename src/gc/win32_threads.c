...
#if defined(MK_GC_DLL) && !defined(MK_GC_NO_THREADS_DISCOVERY) && !defined(MSWINCE) \
...
struct MK_GC_Thread_Rep {
  union {
#   ifndef MK_GC_NO_THREADS_DISCOVERY
      MK_AO_t in_use;      /* Updated without lock.                */
...
#ifndef MK_GC_NO_THREADS_DISCOVERY
  STATIC MK_AO_t MK_GC_attached_thread = FALSE;
#endif
...
#   ifndef MK_GC_NO_THREADS_DISCOVERY
      ...
      if (MK_GC_win32_dll_threads) {
        ...
        result = MK_AO_load(&MK_GC_attached_thread);
        if (result) {
...
# ifndef MSWINCE
    ...
    if (!DuplicateHandle(GetCurrentProcess(), GetCurrentThread(),
        ...
        if (MK_GC_print_stats)
          MK_GC_log_printf("DuplicateHandle failed with error code: %d\n",
                        (int)GetLastError());
...
#elif defined(MK_GC_WIN32_PTHREADS)
...
/* MK_GC_win32_dll_threads is set.                                 */
/* If MK_GC_win32_dll_threads is set it should be called from the  */
/* thread being deleted.                                        */
STATIC void MK_GC_delete_gc_thread(MK_GC_vthread t)
{
# ifndef MSWINCE
...
# ifndef MK_GC_NO_THREADS_DISCOVERY
    if (MK_GC_win32_dll_threads) {
      ...
      t -> id = 0;
#     ifdef MK_GC_PTHREADS
        MK_GC_PTHREAD_PTRVAL(t->pthread_id) = 0;
#     endif
...
{
  if (MK_GC_win32_dll_threads) {
    MK_GC_thread t = MK_GC_lookup_thread_inner(id);

    if (0 == t) {
      WARN("Removing nonexistent thread, id = %" MK_GC_PRIdPTR "\n", id);
    } else {
...
# define SET_PTHREAD_MAP_CACHE(pthread_id, win32_id) \
          (MK_GC_pthread_map_cache[PTHREAD_MAP_INDEX(pthread_id)] = (win32_id))
...
#     ifdef MK_GC_PTHREADS
        ...
        for (i = 0; i <= my_max; i++)
          if (dll_thread_table[i].tm.in_use)
            MK_GC_push_all((ptr_t)&(dll_thread_table[i].status),
...
# ifndef CYGWIN32
    ...
    /* MK_GC_get_max_thread_index(), MK_GC_suspend(), MK_GC_delete_gc_thread()   */
    /* (only if MK_GC_win32_dll_threads), MK_GC_size() and                    */
    /* MK_GC_remove_protection()).                                         */
...
# ifndef SMALL_CONFIG
    if (MK_GC_print_stats == VERBOSE) {
      MK_GC_log_printf("Pushed %d thread stacks%s\n", nthreads,
            MK_GC_win32_dll_threads ? " based on DllMain thread tracking" : "");
    }
# endif
...
#   endif
  ...
  *hi = current_min;
  ...
  MK_GC_ASSERT(current_min > start && plast_stack_min != NULL);
# ifdef MSWINCE
    if (MK_GC_dont_query_stack_min) {
...
#   endif
    ...
    /* - MK_GC_markers value is adjusted already;                              */
...
#     ifdef DONT_USE_SIGNALANDWAIT
        /* Initialize MK_GC_marker_cv[] and MK_GC_marker_Id[] fully before    */
        /* starting the first helper thread.                            */
        for (i = 0; i < MK_GC_markers - 1; ++i) {
          MK_GC_marker_Id[i] = GetCurrentThreadId();
...
#       endif
      ...
      /* Adjust MK_GC_markers (and free unused resources) in case of failure. */
#     ifdef DONT_USE_SIGNALANDWAIT
        while ((int)MK_GC_markers > i + 1) {
          MK_GC_markers--;
          CloseHandle(MK_GC_marker_cv[(int)MK_GC_markers - 1]);
        }
#     else
        MK_GC_markers = i + 1;
#     endif
...
#     endif
      ...
      MK_GC_ASSERT(MK_GC_mark_lock_holder == NO_THREAD);
#     ifdef MK_GC_ASSERTIONS
        MK_GC_mark_lock_holder = (unsigned long)GetCurrentThreadId();
#     endif
    }

    MK_GC_INNER void MK_GC_release_mark_lock(void)
    {
      MK_GC_ASSERT(MK_GC_mark_lock_holder == (unsigned long)GetCurrentThreadId());
#     ifdef MK_GC_ASSERTIONS
        MK_GC_mark_lock_holder = NO_THREAD;
#     endif
#     ifdef DONT_USE_SIGNALANDWAIT
...
#   else /* DONT_USE_SIGNALANDWAIT */
      ...
      /* MK_GC_mark_mutex_waitcnt and calling WaitForSingleObject()).      */
...
#       ifdef MSWINCE
          ...
          MK_GC_markers = MK_GC_sysinfo.dwNumberOfProcessors;
#       else
#         ifdef _WIN64
...
#       ifdef MK_GC_MIN_MARKERS
          ...
          if (MK_GC_markers < MK_GC_MIN_MARKERS)
            MK_GC_markers = MK_GC_MIN_MARKERS;
#       endif
        if (MK_GC_markers >= MAX_MARKERS)
...
#     endif
      if (MK_GC_win32_dll_threads || MK_GC_markers <= 1
#         if !defined(MK_GC_PTHREADS_PARAMARK) && !defined(MSWINCE) \
                && !defined(DONT_USE_SIGNALANDWAIT)
            || MK_GC_wnt == FALSE
...
# ifdef PARALLEL_MARK
    ...
    if (MK_GC_parallel) start_mark_threads();
    if (MK_GC_print_stats) {
...
#   endif
    ...
    MK_GC_free(si); /* was allocated uncollectable */
        ...
        if (MK_GC_win32_dll_threads) {
          ...
          for (i = 0; i <= my_max; ++i) {
           if (MK_AO_load(&(dll_thread_table[i].tm.in_use)))
             MK_GC_delete_gc_thread(dll_thread_table + i);
          }
          MK_GC_deinit();
          DeleteCriticalSection(&MK_GC_allocate_ml);
        }
