...
# define N_HBLK_FLS (HUGE_THRESHOLD - UNIQUE_THRESHOLD)/FL_COMPRESSION \
  ...
  /* Is MK_GC_large_allocd_bytes + the number of free bytes on lists       */
  /* n .. N_HBLK_FLS > MK_GC_max_large_allocd_bytes.                       */
...
# define INCR_FREE_BYTES(n, b) MK_GC_free_bytes[n] += (b);

# define FREE_ASSERT(e) MK_GC_ASSERT(e)
...
#     ifdef USE_MUNMAP
        if (0 != h) MK_GC_printf("Free list %u:\n", i);
#     else
...
#   ifndef USE_MUNMAP
      if (total_free != MK_GC_large_free_bytes) {
        MK_GC_printf("MK_GC_large_free_bytes = %lu (INCONSISTENT!!)\n",
                  (unsigned long) MK_GC_large_free_bytes);
      }
#   endif
    MK_GC_printf("Total of %lu bytes on free list\n", (unsigned long)total_free);
}
 ...
 */
STATIC void MK_GC_remove_from_fl(hdr *hhdr, int n)
{
    int index;
...
#   ifndef USE_MUNMAP
      ...
      if (FL_UNKNOWN == n) {
          index = MK_GC_hblk_fl_from_blocks(divHBLKSZ(hhdr -> hb_sz));
      } else {
          index = n;
      }
#   endif
...
#       endif
    ...
    FREE_ASSERT(MK_GC_free_bytes[index] >= hhdr -> hb_sz);
...
#   endif
    ...
    FREE_ASSERT(MK_GC_free_bytes[index] <= MK_GC_large_free_bytes);
    hhdr -> hb_next = second;
    hhdr -> hb_prev = 0;
    if (0 != second) {
      ...
      while (h != 0) {
        ...
        /* Coalesce with successor, if possible */
          if (0 != nexthdr && HBLK_IS_FREE(nexthdr)
            ...
            MK_GC_remove_from_fl(hhdr, i);
            MK_GC_remove_from_fl(nexthdr, FL_UNKNOWN);
                                       ...
                                       size_t bytes, int index)
{
    ...
    MK_GC_remove_from_fl(hhdr, index);
                           ...
                           hdr *nhdr, int index /* Index of free list */)
{
    ...
    /* Replace h with n on its freelist */
      ...
      FREE_ASSERT(MK_GC_free_bytes[index] > 0);
#   ifdef USE_MUNMAP
      hhdr -> hb_last_reclaimed = (unsigned short)MK_GC_gc_no;
#   endif
    hhdr -> hb_sz = h_size;
...
#     ifdef USE_MUNMAP
        ...
        split_limit = 0;
#     else
        if (MK_GC_finalizer_bytes_freed > (MK_GC_heapsize >> 4)) {
...
#     endif
    ...
    for (i = start_list; i <= split_limit; ++i) {
        struct hblk * result = MK_GC_allochblk_nth(sz, kind, flags, i, TRUE);
        if (0 != result) return result;
    }
    return 0;
}
 ...
 */
STATIC struct hblk *
MK_GC_allochblk_nth(size_t sz, int kind, unsigned flags, int n,
                 MK_GC_bool may_split)
{
    struct hblk *hbp;
    ...
    /* search for a big enough block in free list */
        ...
        for(; 0 != hbp; hbp = hhdr -> hb_next) {
            ...
            if ( !IS_UNCOLLECTABLE(kind) && (kind != PTRFREE
              ...
              if (size_avail >= size_needed) {
                if (thishbp != hbp &&
                    0 != (thishdr = MK_GC_install_header(thishbp))) {
                ...
                if (!MK_GC_find_leak) {
                  ...
                  if ((++count & 3) == 0) {
                    ...
                    /* later.                                           */
                      ...
                      MK_GC_remove_from_fl(hhdr, n);
                      for (h = hbp; h < limit; h++) {
                        if (h == hbp || 0 != (hhdr = MK_GC_install_header(h))) {
                          (void) setup_header(
                                  hhdr, h,
                                  HBLKSIZE,
...
MK_GC_INNER void MK_GC_freehblk(struct hblk *hbp)
{
    ...
    if (size <= 0)
      ...
      /* later.                                                             */
    MK_GC_remove_counts(hbp, (word)size);
    hhdr->hb_sz = size;
#   ifdef USE_MUNMAP
      hhdr -> hb_last_reclaimed = (unsigned short)MK_GC_gc_no;
#   endif
    ...
    /* Check for duplicate deallocation in the easy case */
      if (HBLK_IS_FREE(hhdr)) {
        if (MK_GC_print_stats)
          MK_GC_log_printf("Duplicate large block deallocation of %p\n", hbp);
    ...
    /* Coalesce with predecessor, if possible. */
      if (0 != prev) {
        ...
        if (IS_MAPPED(prevhdr)
          ...
          MK_GC_remove_from_fl(prevhdr, FL_UNKNOWN);
