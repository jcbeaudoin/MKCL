...
static void sort_heap_sects(struct HeapSect *base, size_t number_of_elements)
{
    ...
    while (nsorted < n) {
      while (nsorted < n &&
          ...
          ++nsorted;
      if (nsorted == n) break;
      MK_GC_ASSERT((word)base[nsorted-1].hs_start > (word)base[nsorted].hs_start);
      i = nsorted - 1;
...
#   endif
    ...
    for (;;) {
        ...
        if (prot[1] == 'w') {
            ...
            /* accounted for.                                   */
            if ((word)start <= (word)MK_GC_stackbottom
                && (word)end >= (word)MK_GC_stackbottom) {
...
#           endif
            ...
            /* Add sections that don't belong to us. */
              i = 0;
              while ((word)(MK_GC_our_memory[i].hs_start
                                + MK_GC_our_memory[i].hs_bytes) < (word)start)
                  ++i;
              MK_GC_ASSERT(i < MK_GC_n_memory);
              if ((word)MK_GC_our_memory[i].hs_start <= (word)start) {
                  start = MK_GC_our_memory[i].hs_start
                          + MK_GC_our_memory[i].hs_bytes;
                  ++i;
              }
              while (i < MK_GC_n_memory
                     && (word)MK_GC_our_memory[i].hs_start < (word)end
                     && (word)start < (word)end) {
                  if ((word)start < (word)MK_GC_our_memory[i].hs_start)
            ...
            for (j = n_load_segs; --j >= 0; ) {
              if ((word)start >= (word)load_segs[j].start
                ...
                if (load_segs[j].start2 != 0) {
                  ...
                  MK_GC_ASSERT((word)end <= (word)load_segs[j].end);
...
#   endif /* SOLARISDL */
    ...
    if (MK_GC_n_heap_sects > 0) {
        ...
        if ((word)heap_end < (word)MK_GC_scratch_last_end_ptr)
          heap_end = MK_GC_scratch_last_end_ptr;
    }
...
#   ifdef MK_GC_WIN32_THREADS
      ...
      if ((word)curr_base < (word)limit)
        MK_GC_add_roots_inner(curr_base, limit, TRUE);
#   else
      char * stack_top
      ...
      if ((word)limit > (word)stack_top
          && (word)base < (word)MK_GC_stackbottom) {
...
STATIC const char * const MK_GC_dyld_add_sect_fmts[] = {
  "__bss%u",
  "__pu_bss%u",
  "__zo_bss%u",
  "__zo_pu_bss%u"
...
#     endif
  ...
  for (j = 0; j < sizeof(MK_GC_dyld_add_sect_fmts) / sizeof(char *); j++) {
    fmt = MK_GC_dyld_add_sect_fmts[j];
