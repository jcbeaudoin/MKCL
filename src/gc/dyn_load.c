...
/* BTL: avoid circular redefinition of dlopen if MK_GC_SOLARIS_THREADS defined */
#undef MK_GC_MUST_RESTORE_REDEFINED_DLOPEN
#if defined(MK_GC_PTHREADS) && !defined(MK_GC_NO_DLOPEN) \
    && !defined(MK_GC_NO_THREAD_REDIRECTS) && !defined(MK_GC_USE_LD_WRAP)
...
STATIC MK_GC_has_static_roots_func MK_GC_has_static_roots = 0;
...
MK_GC_FirstDLOpenedLinkMap(void)
{
...
/* BTL: added to fix circular dlopen definition if MK_GC_SOLARIS_THREADS defined */
...
# if !defined(PCR) && !defined(MK_GC_SOLARIS_THREADS) && defined(THREADS)
...
MK_GC_INNER void MK_GC_register_dynamic_libraries(void)
{
  ...
  for (lm = MK_GC_FirstDLOpenedLinkMap(); lm != 0; lm = lm->l_next) {
        ElfW(Ehdr) * e;
        ElfW(Phdr) * p;
...
static void sort_heap_sects(struct HeapSect *base, size_t number_of_elements)
{
    ...
    while (nsorted < n) {
      while (nsorted < n &&
          ...
          ++nsorted;
      if (nsorted == n) break;
      MK_GC_ASSERT(base[nsorted-1].hs_start > base[nsorted].hs_start);
      i = nsorted - 1;
...
STATIC word MK_GC_register_map_entries(char *maps)
{
    char *prot;
    char *buf_ptr = maps;
    ptr_t start, end;
...
#   endif
    ...
    sort_heap_sects(MK_GC_our_memory, MK_GC_n_memory);
    least_ha = MK_GC_our_memory[0].hs_start;
    greatest_ha = MK_GC_our_memory[MK_GC_n_memory-1].hs_start
                  + MK_GC_our_memory[MK_GC_n_memory-1].hs_bytes;

    for (;;) {
        buf_ptr = MK_GC_parse_map_entry(buf_ptr, &start, &end, &prot,
                                     &maj_dev, 0);
        ...
        if (prot[1] == 'w') {
            ...
            /* accounted for.                                   */
            if (start <= MK_GC_stackbottom && end >= MK_GC_stackbottom) {
...
#           ifdef THREADS
              ...
              if (MK_GC_segment_is_thread_stack(start, end)) continue;
...
#           endif
            ...
            if (end <= least_ha || start >= greatest_ha) {
              ...
              MK_GC_add_roots_inner((char *)start, (char *)end, TRUE);
              continue;
            }
            /* Add sections that don't belong to us. */
              i = 0;
              while (MK_GC_our_memory[i].hs_start + MK_GC_our_memory[i].hs_bytes
                     < start)
                  ++i;
              MK_GC_ASSERT(i < MK_GC_n_memory);
              if (MK_GC_our_memory[i].hs_start <= start) {
                  start = MK_GC_our_memory[i].hs_start
                          + MK_GC_our_memory[i].hs_bytes;
                  ++i;
              }
              while (i < MK_GC_n_memory && MK_GC_our_memory[i].hs_start < end
                     && start < end) {
                  if ((char *)start < MK_GC_our_memory[i].hs_start)
                    MK_GC_add_roots_inner((char *)start,
                                       MK_GC_our_memory[i].hs_start, TRUE);
                  start = MK_GC_our_memory[i].hs_start
                          + MK_GC_our_memory[i].hs_bytes;
                  ++i;
              }
...
MK_GC_INNER void MK_GC_register_dynamic_libraries(void)
{
    if (!MK_GC_register_map_entries(MK_GC_get_maps()))
...
/* MK_GC_exclude_static_roots instead would be superficially cleaner.  But */
...
STATIC int MK_GC_register_dynlib_callback(struct dl_phdr_info * info,
            ...
            for (j = n_load_segs; --j >= 0; ) {
              if (start >= load_segs[j].start && start < load_segs[j].end) {
                if (load_segs[j].start2 != 0) {
                  ...
                  MK_GC_ASSERT(end <= load_segs[j].end);
...
#     endif
      ...
      case PT_LOAD:
        {
          MK_GC_has_static_roots_func callback = MK_GC_has_static_roots;
          if( !(p->p_flags & PF_W) ) break;
...
#           if CPP_WORDSZ == 64
              /* FIXME: MK_GC_push_all eventually does the correct         */
              /* rounding to the next multiple of ALIGNMENT, so, most   */
              /* probably, we should remove the corresponding assertion */
              /* check in MK_GC_add_roots_inner along with this code line. */
...
#           endif
            ...
            ++n_load_segs;
#         else
            MK_GC_add_roots_inner(start, end, TRUE);
#         endif /* PT_GNU_RELRO */
        }
      break;
      default:
...
STATIC MK_GC_bool MK_GC_register_dynamic_libraries_dl_iterate_phdr(void)
{
  int did_something;
  if (MK_GC_register_main_static_data())
    return FALSE;

# ifdef PT_GNU_RELRO
    {
      static MK_GC_bool excluded_segs = FALSE;
      n_load_segs = 0;
      if (!excluded_segs) {
        MK_GC_exclude_static_roots_inner((ptr_t)load_segs,
...
# endif
  ...
  did_something = 0;
  dl_iterate_phdr(MK_GC_register_dynlib_callback, &did_something);
  if (did_something) {
#   ifdef PT_GNU_RELRO
      int i;
      ...
      for (i = 0; i < n_load_segs; ++i) {
        if (load_segs[i].end > load_segs[i].start) {
          MK_GC_add_roots_inner(load_segs[i].start, load_segs[i].end, TRUE);
        }
        if (load_segs[i].end2 > load_segs[i].start2) {
          MK_GC_add_roots_inner(load_segs[i].start2, load_segs[i].end2, TRUE);
        }
      }
#   endif
  } else {
...
#     endif
      ...
      MK_GC_add_roots_inner(datastart, dataend, TRUE);
#     if defined(DATASTART2)
        MK_GC_add_roots_inner(DATASTART2, (char *)(DATAEND2), TRUE);
#     endif
  }
  return TRUE;
}
...
# ifdef HAVE_DL_ITERATE_PHDR
    if (MK_GC_register_dynamic_libraries_dl_iterate_phdr()) {
        return;
    }
# endif
  for (lm = MK_GC_FirstDLOpenedLinkMap(); lm != 0; lm = lm->l_next)
            ...
            case PT_LOAD:
              {
                if( !(p->p_flags & PF_W) ) break;
                start = ((char *)(p->p_vaddr)) + offset;
                MK_GC_add_roots_inner(start, start + p->p_memsz, TRUE);
              }
              break;
            default:
              break;
...
#   endif /* SOLARISDL */
    ...
    if (ioctl(fd, PIOCNMAP, &needed_sz) < 0) {
        MK_GC_err_printf("fd = %d, errno = %d\n", fd, errno);
        ABORT("/proc PIOCNMAP ioctl failed");
    }
    if (needed_sz >= current_sz) {
        current_sz = needed_sz * 2 + 1;
                        /* Expansion, plus room for 0 record */
        addr_map = (prmap_t *)MK_GC_scratch_alloc(
                                (word)current_sz * sizeof(prmap_t));
        if (addr_map == NULL)
          ABORT("Insufficient memory for address map");
    }
    if (ioctl(fd, PIOCMAP, addr_map) < 0) {
        MK_GC_err_printf("fd = %d, errno = %d, needed_sz = %d, addr_map = %p\n",
                        fd, errno, needed_sz, addr_map);
        ABORT("/proc PIOCMAP ioctl failed");
    };
    if (MK_GC_n_heap_sects > 0) {
        heap_end = MK_GC_heap_sects[MK_GC_n_heap_sects-1].hs_start
                        + MK_GC_heap_sects[MK_GC_n_heap_sects-1].hs_bytes;
        if (heap_end < MK_GC_scratch_last_end_ptr) heap_end = MK_GC_scratch_last_end_ptr;
    }
    for (i = 0; i < needed_sz; i++) {
        ...
        if (MK_GC_roots_present(start)) goto irrelevant;
        if (start < heap_end && start >= heap_start)
                goto irrelevant;
#       ifdef MMAP_STACKS
          if (MK_GC_is_thread_stack(start)) goto irrelevant;
#       endif /* MMAP_STACKS */
...
#       endif /* !IRIX6 */
        MK_GC_add_roots_inner(start, limit, TRUE);
      irrelevant: ;
    }
...
# include <stdlib.h>
  ...
  STATIC void MK_GC_cond_add_roots(char *base, char * limit)
  {
#   ifdef MK_GC_WIN32_THREADS
      ...
      for(;;) {
          MK_GC_get_next_stack(curr_base, limit, &next_stack_lo, &next_stack_hi);
          if (next_stack_lo >= limit) break;
          if (next_stack_lo > curr_base)
            MK_GC_add_roots_inner(curr_base, next_stack_lo, TRUE);
          curr_base = next_stack_hi;
      }
      if (curr_base < limit) MK_GC_add_roots_inner(curr_base, limit, TRUE);
#   else
      char * stack_top
         = (char *)((word)MK_GC_approx_sp() &
                        ~(MK_GC_sysinfo.dwAllocationGranularity - 1));
      if (base == limit) return;
      if (limit > stack_top && base < MK_GC_stackbottom) {
          /* Part of the stack; ignore it. */
          return;
      }
      MK_GC_add_roots_inner(base, limit, TRUE);
#   endif
  }

#ifdef DYNAMIC_LOADING
  /* MK_GC_register_main_static_data is not needed unless DYNAMIC_LOADING. */
  MK_GC_INNER MK_GC_bool MK_GC_register_main_static_data(void)
  {
...
# ifdef DEBUG_VIRTUALQUERY
  void MK_GC_dump_meminfo(MEMORY_BASIC_INFORMATION *buf)
  {
    MK_GC_printf("BaseAddress = 0x%lx, AllocationBase = 0x%lx,"
              " RegionSize = 0x%lx(%lu)\n", buf -> BaseAddress,
              buf -> AllocationBase, buf -> RegionSize, buf -> RegionSize);
    MK_GC_printf("\tAllocationProtect = 0x%lx, State = 0x%lx, Protect = 0x%lx, "
...
# if defined(MSWINCE) || defined(CYGWIN32)
    ...
    /* MK_GC_mark_from() (and, even if we use WRAP_MARK_SOME, WinCE prints */
    /* a "Data Abort" message to the debugging console).                */
    /* To workaround that, use -DMK_GC_REGISTER_MEM_PRIVATE.               */
#   define MK_GC_wnt TRUE
# endif
...
#   endif
    base = limit = p = MK_GC_sysinfo.lpMinimumApplicationAddress;
    while (p < MK_GC_sysinfo.lpMaximumApplicationAddress) {
        result = VirtualQuery(p, &buf, sizeof(buf));
#       ifdef MSWINCE
          if (result == 0) {
            /* Page is free; advance to the next possible allocation base */
            new_limit = (char *)
                (((DWORD) p + MK_GC_sysinfo.dwAllocationGranularity)
                 & ~(MK_GC_sysinfo.dwAllocationGranularity-1));
          } else
#       endif
        /* else */ {
            ...
            if (buf.State == MEM_COMMIT
                ...
                && (buf.Type == MEM_IMAGE
#                   ifdef MK_GC_REGISTER_MEM_PRIVATE
...
#                   else
                      ...
                      /* that case.                                     */
                      || (!MK_GC_wnt && buf.Type == MEM_PRIVATE)
#                   endif
                   )
                && !MK_GC_is_heap_base(buf.AllocationBase)) {
#               ifdef DEBUG_VIRTUALQUERY
                  MK_GC_dump_meminfo(&buf);
#               endif
                if ((char *)p != limit) {
                    MK_GC_cond_add_roots(base, limit);
                    base = p;
                }
                limit = new_limit;
            }
      ...
      /* of a bug in the non-shared ldr_next_module stub */
        if (status != 0) {
          if (MK_GC_print_stats) {
            MK_GC_log_printf("dynamic_load: status = %d\n", status);
            if (errno < sys_nerr) {
              MK_GC_log_printf("dynamic_load: %s\n", sys_errlist[errno]);
            } else {
              MK_GC_log_printf("dynamic_load: err_code = %d\n", errno);
            }
          }
...
#     ifdef DL_VERBOSE
        MK_GC_log_printf("---Module---\n");
        MK_GC_log_printf("Module ID\t = %16ld\n", moduleinfo.lmi_modid);
        MK_GC_log_printf("Count of regions = %16d\n", moduleinfo.lmi_nregion);
        MK_GC_log_printf("flags for module = %16lx\n", moduleinfo.lmi_flags);
        MK_GC_log_printf("module pathname\t = \"%s\"\n", moduleinfo.lmi_name);
#     endif
...
#         ifdef DL_VERBOSE
            MK_GC_log_printf("--- Region ---\n");
            MK_GC_log_printf("Region number\t = %16ld\n",
                          regioninfo.lri_region_no);
            MK_GC_log_printf("Protection flags = %016x\n", regioninfo.lri_prot);
            MK_GC_log_printf("Virtual address\t = %16p\n", regioninfo.lri_vaddr);
            MK_GC_log_printf("Mapped address\t = %16p\n",
                          regioninfo.lri_mapaddr);
            MK_GC_log_printf("Region size\t = %16ld\n", regioninfo.lri_size);
            MK_GC_log_printf("Region name\t = \"%s\"\n", regioninfo.lri_name);
#         endif

          /* register region as a garbage collection root */
          MK_GC_add_roots_inner((char *)regioninfo.lri_mapaddr,
...
#     ifdef DL_VERBOSE
        MK_GC_log_printf("---Shared library---\n");
        MK_GC_log_printf("\tfilename\t= \"%s\"\n", shl_desc->filename);
        MK_GC_log_printf("\tindex\t\t= %d\n", index);
        MK_GC_log_printf("\thandle\t\t= %08x\n",
                      (unsigned long) shl_desc->handle);
        MK_GC_log_printf("\ttext seg.start\t= %08x\n", shl_desc->tstart);
        MK_GC_log_printf("\ttext seg.end\t= %08x\n", shl_desc->tend);
        MK_GC_log_printf("\tdata seg.start\t= %08x\n", shl_desc->dstart);
        MK_GC_log_printf("\tdata seg.end\t= %08x\n", shl_desc->dend);
        MK_GC_log_printf("\tref.count\t= %lu\n", shl_desc->ref_count);
#     endif

      /* register shared library's data segment as a garbage collection root */
        MK_GC_add_roots_inner((char *) shl_desc->dstart,
...
STATIC const char * MK_GC_dyld_add_sect_fmts[] =
{
  "__bss%u",
  "__pu_bss%u",
  "__zo_bss%u",
...
STATIC const char *MK_GC_dyld_name_for_hdr(const struct MK_GC_MACH_HEADER *hdr)
{
    unsigned long i, c;
    c = _dyld_image_count();
    for (i = 0; i < c; i++)
      if ((const struct MK_GC_MACH_HEADER *)_dyld_get_image_header(i) == hdr)
        return _dyld_get_image_name(i);
    return NULL;
}

/* This should never be called by a thread holding the lock.    */
STATIC void MK_GC_dyld_image_add(const struct MK_GC_MACH_HEADER *hdr,
                              intptr_t slide)
{
  unsigned long start, end;
  unsigned i, j;
  const struct MK_GC_MACH_SECTION *sec;
  const char *name;
  MK_GC_has_static_roots_func callback = MK_GC_has_static_roots;
  char secnam[16];
  const char *fmt;
  ...
  if (MK_GC_no_dls) return;
# ifdef DARWIN_DEBUG
    name = MK_GC_dyld_name_for_hdr(hdr);
# else
    name = callback != 0 ? MK_GC_dyld_name_for_hdr(hdr) : NULL;
# endif
  for (i = 0; i < sizeof(MK_GC_dyld_sections)/sizeof(MK_GC_dyld_sections[0]); i++) {
    sec = MK_GC_GETSECTBYNAME(hdr, MK_GC_dyld_sections[i].seg,
                           MK_GC_dyld_sections[i].sect);
...
#     ifdef DARWIN_DEBUG
        MK_GC_log_printf(
              "Adding section __DATA,%s at %p-%p (%lu bytes) from image %s\n",
               MK_GC_dyld_sections[i].sect, (void*)start, (void*)end,
               (unsigned long)sec->size, name);
#     endif
      MK_GC_add_roots_inner((ptr_t)start, (ptr_t)end, FALSE);
    }
    UNLOCK();
  }

  /* Sections constructed on demand.    */
  for (j = 0; (fmt = MK_GC_dyld_add_sect_fmts[j]) != NULL; j++) {
    /* Add our manufactured aligned BSS sections.       */
    for (i = 0; i <= L2_MAX_OFILE_ALIGNMENT; i++) {
      snprintf(secnam, sizeof(secnam), fmt, (unsigned)i);
      sec = MK_GC_GETSECTBYNAME(hdr, SEG_DATA, secnam);
      if (sec == NULL || sec->size == 0)
        continue;
      start = slide + sec->addr;
      end = start + sec->size;
#     ifdef DARWIN_DEBUG
        MK_GC_log_printf("Adding on-demand section __DATA,%s at"
                      " %p-%p (%lu bytes) from image %s\n",
                      secnam, (void*)start, (void*)end,
                      (unsigned long)sec->size, name);
#     endif
      MK_GC_add_roots((char*)start, (char*)end);
    }
  }
...
/* This should never be called by a thread holding the lock.    */
STATIC void MK_GC_dyld_image_remove(const struct MK_GC_MACH_HEADER *hdr,
                                 intptr_t slide)
{
  unsigned long start, end;
  unsigned i, j;
...
#   ifdef DARWIN_DEBUG
      MK_GC_log_printf(
            ...
            MK_GC_dyld_sections[i].sect, (void*)start, (void*)end,
            (unsigned long)sec->size, MK_GC_dyld_name_for_hdr(hdr));
#   endif
    MK_GC_remove_roots((char*)start, (char*)end);
  }
...
#     ifdef DARWIN_DEBUG
        MK_GC_log_printf("Removing on-demand section __DATA,%s at"
                      " %p-%p (%lu bytes) from image %s\n", secnam,
                      (void*)start, (void*)end, (unsigned long)sec->size,
                      MK_GC_dyld_name_for_hdr(hdr));
#     endif
      MK_GC_remove_roots((char*)start, (char*)end);
    }
  }
...
MK_GC_INNER void MK_GC_register_dynamic_libraries(void)
{
    /* Currently does nothing. The callbacks are setup by MK_GC_init_dyld()
...
# ifdef DARWIN_DEBUG
    MK_GC_log_printf("Registering dyld callbacks...\n");
# endif
  ...
  */
  ...
  _dyld_register_func_for_add_image(MK_GC_dyld_image_add);
  _dyld_register_func_for_remove_image(MK_GC_dyld_image_remove);
...
# else
    if (MK_GC_no_dls) return; /* skip main data segment registration */
    ...
    for (; p != NIL; p = p -> lf_prev) {
      for (q = p -> lf_ls; q != NIL; q = q -> ls_next) {
        if ((q -> ls_flags & PCR_IL_SegFlags_Traced_MASK)
          ...
          MK_GC_add_roots_inner((char *)(q -> ls_addr),
...
MK_GC_API void MK_GC_CALL MK_GC_register_has_static_roots_callback(
                                        MK_GC_has_static_roots_func callback)
{
    MK_GC_has_static_roots = callback;
}
