...
STATIC int MK_GC_nacl_num_gc_threads = 0;
STATIC __thread int MK_GC_nacl_thread_idx = -1;
STATIC int MK_GC_nacl_park_threads_now = 0;
STATIC pthread_t MK_GC_nacl_thread_parker = -1;

MK_GC_INNER __thread MK_GC_thread MK_GC_nacl_gc_thread_self = NULL;
...
# endif /* NSIG */
  ...
  void MK_GC_print_sig_mask(void)
  {
    sigset_t blocked;
    int i;
...
/* handler from a set.                                          */
STATIC void MK_GC_remove_allowed_signals(sigset_t *set)
{
...
STATIC volatile MK_AO_t MK_GC_stop_count = 0;
                        /* Incremented at the beginning of MK_GC_stop_world. */

STATIC volatile MK_AO_t MK_GC_world_is_stopped = FALSE;
...
#ifdef MK_GC_OSF1_THREADS
  STATIC MK_GC_bool MK_GC_retry_signals = TRUE;
#else
  STATIC MK_GC_bool MK_GC_retry_signals = FALSE;
#endif
...
#  if defined(MK_GC_HPUX_THREADS) || defined(MK_GC_OSF1_THREADS) \
      || defined(MK_GC_NETBSD_THREADS)
#    ifdef _SIGRTMIN
...
#endif /* MK_GC_EXPLICIT_SIGNALS_UNBLOCK */

STATIC sem_t MK_GC_suspend_ack_sem;

#ifdef MK_GC_NETBSD_THREADS
# define MK_GC_NETBSD_THREADS_WORKAROUND
  /* It seems to be necessary to wait until threads have restarted.     */
  /* But it is unclear why that is the case.                            */
  STATIC sem_t MK_GC_restart_ack_sem;
#endif
...
# ifdef SPARC
      me -> stop_info.stack_ptr = MK_GC_save_regs_in_stack();
# else
      me -> stop_info.stack_ptr = MK_GC_approx_sp();
# endif
# ifdef IA64
      me -> backing_store_ptr = MK_GC_save_regs_in_stack();
# endif

  /* Tell the thread that wants to stop the world that this     */
  /* thread has been stopped.  Note that sem_post() is          */
  /* the only async-signal-safe primitive in LinuxThreads.      */
  sem_post(&MK_GC_suspend_ack_sem);
  ...
  } while (MK_AO_load_acquire(&MK_GC_world_is_stopped)
           && MK_AO_load(&MK_GC_stop_count) == my_stop_count);
...
STATIC void MK_GC_restart_handler(int sig)
{
...
#           ifdef SPARC
                lo = (ptr_t)MK_GC_save_regs_in_stack();
#           else
                lo = MK_GC_approx_sp();
#           endif
            found_me = TRUE;
            IF_IA64(bs_hi = (ptr_t)MK_GC_save_regs_in_stack();)
        } else {
...
#       ifdef NACL
          ...
          MK_GC_push_all_stack((ptr_t)p -> stop_info.reg_storage,
              (ptr_t)(p -> stop_info.reg_storage + NACL_MK_GC_REG_STORAGE_SIZE));
          total_size += NACL_MK_GC_REG_STORAGE_SIZE * sizeof(ptr_t);
#       endif
#       ifdef IA64
#         ifdef DEBUG_THREADS
...
#       endif
  ...
  pthread_t MK_GC_stopping_thread;
...
/* were sent.                                                   */
STATIC int MK_GC_suspend_all(void)
{
...
#   ifdef DEBUG_THREADS
      MK_GC_stopping_thread = self;
      MK_GC_stopping_pid = getpid();
#   endif
    for (i = 0; i < THREAD_TABLE_SZ; i++) {
      for (p = MK_GC_threads[i]; p != 0; p = p -> next) {
        if (!THREAD_EQUAL(p -> id, self)) {
...
#   endif
    MK_GC_nacl_thread_parker = pthread_self();
    MK_GC_nacl_park_threads_now = 1;
#   ifdef DEBUG_THREADS
      MK_GC_stopping_thread = MK_GC_nacl_thread_parker;
      MK_GC_stopping_pid = getpid();
#   endif
    ...
    while (1) {
      ...
      for (i = 0; i < MAX_NACL_MK_GC_THREADS
                  && num_used < MK_GC_nacl_num_gc_threads; i++) {
        if (MK_GC_nacl_thread_used[i] == 1) {
          num_used++;
          if (MK_GC_nacl_thread_parked[i] == 1) {
            num_threads_parked++;
          }
        }
      }
      /* -1 for the current thread.     */
      if (num_threads_parked >= MK_GC_nacl_num_gc_threads - 1)
        break;
      ts.tv_sec = 0;
      ts.tv_nsec = NACL_PARK_WAIT_NANOSECONDS;
#     ifdef DEBUG_THREADS
        MK_GC_log_printf("Sleep waiting for %d threads to park...\n",
...
static struct sigaction old_sigsegv; /* JCB */
...
# else
    ...
    MK_AO_store_release(&MK_GC_world_is_stopped, TRUE);
    n_live_threads = MK_GC_suspend_all();
...
# endif
    ...
    { /* JCB */
      ...
      default_sigsegv.sa_handler = SIG_DFL;
      sigemptyset(&default_sigsegv.sa_mask);
      default_sigsegv.sa_flags = 0;

      if (sigaction(SIGSEGV, &default_sigsegv, &old_sigsegv))
	MK_GC_printf("\nsigaction failed!\n");
    }
}
...
#   define NACL_STORE_REGS() \
        do { \
          ...
          __asm__ __volatile__ ("mov %%esp, %0" \
                    : "=m" (MK_GC_nacl_gc_thread_self->stop_info.stack_ptr)); \
          BCOPY(MK_GC_nacl_gc_thread_self->stop_info.stack_ptr, \
                MK_GC_nacl_gc_thread_self->stop_info.reg_storage, \
                NACL_MK_GC_REG_STORAGE_SIZE * sizeof(ptr_t)); \
...
# endif
  ...
  MK_GC_API_OSCALL void nacl_pre_syscall_hook(void)
  {
    if (MK_GC_nacl_thread_idx != -1) {
      NACL_STORE_REGS();
      MK_GC_nacl_gc_thread_self->stop_info.stack_ptr = MK_GC_approx_sp();
      MK_GC_nacl_thread_parked[MK_GC_nacl_thread_idx] = 1;
    }
  }

  MK_GC_API_OSCALL void __nacl_suspend_thread_if_needed(void)
  {
    if (MK_GC_nacl_park_threads_now) {
      pthread_t self = pthread_self();

      /* Don't try to park the thread parker.   */
      if (MK_GC_nacl_thread_parker == self)
        return;

      /* This can happen when a thread is created outside of the GC     */
      /* system (wthread mostly).                                       */
      if (MK_GC_nacl_thread_idx < 0)
        return;

      /* If it was already 'parked', we're returning from a syscall,    */
      /* so don't bother storing registers again, the GC has a set.     */
      if (!MK_GC_nacl_thread_parked[MK_GC_nacl_thread_idx]) {
        NACL_STORE_REGS();
        MK_GC_nacl_gc_thread_self->stop_info.stack_ptr = MK_GC_approx_sp();
      }
      MK_GC_nacl_thread_parked[MK_GC_nacl_thread_idx] = 1;
      while (MK_GC_nacl_park_threads_now) {
        /* Just spin.   */
      }
      MK_GC_nacl_thread_parked[MK_GC_nacl_thread_idx] = 0;

      /* Clear out the reg storage for next suspend.    */
      BZERO(MK_GC_nacl_gc_thread_self->stop_info.reg_storage,
            NACL_MK_GC_REG_STORAGE_SIZE * sizeof(ptr_t));
    }
  }

  MK_GC_API_OSCALL void nacl_post_syscall_hook(void)
  {
  ...
  MK_GC_INNER void MK_GC_nacl_initialize_gc_thread(void)
  {
    ...
    MK_GC_ASSERT(MK_GC_nacl_num_gc_threads <= MAX_NACL_MK_GC_THREADS);
    for (i = 0; i < MAX_NACL_MK_GC_THREADS; i++) {
      if (MK_GC_nacl_thread_used[i] == 0) {
        MK_GC_nacl_thread_used[i] = 1;
        MK_GC_nacl_thread_idx = i;
        MK_GC_nacl_num_gc_threads++;
        break;
      }
    }
    pthread_mutex_unlock(&MK_GC_nacl_thread_alloc_lock);
  }

  MK_GC_INNER void MK_GC_nacl_shutdown_gc_thread(void)
  {
    pthread_mutex_lock(&MK_GC_nacl_thread_alloc_lock);
    MK_GC_ASSERT(MK_GC_nacl_thread_idx >= 0);
    MK_GC_ASSERT(MK_GC_nacl_thread_idx < MAX_NACL_MK_GC_THREADS);
    MK_GC_ASSERT(MK_GC_nacl_thread_used[MK_GC_nacl_thread_idx] != 0);
    MK_GC_nacl_thread_used[MK_GC_nacl_thread_idx] = 0;
    MK_GC_nacl_thread_idx = -1;
    MK_GC_nacl_num_gc_threads--;
    pthread_mutex_unlock(&MK_GC_nacl_thread_alloc_lock);
  }
#endif /* NACL */
...
#   ifdef MK_GC_NETBSD_THREADS_WORKAROUND
      if (sem_init(&MK_GC_restart_ack_sem, MK_GC_SEM_INIT_PSHARED, 0) != 0)
        ABORT("sem_init failed");
#   endif
