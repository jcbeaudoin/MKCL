...
#define MK_AO_malloc_large(sz) ((char*)0)
#define MK_AO_free_large(p) abort()
...
get_chunk(void)
{
  ...
  for (;;) {
    char *initial_ptr = (char *)MK_AO_load(&initial_heap_ptr);
    ...
    if (initial_ptr != my_chunk_ptr)
      {
        ...
        (void)MK_AO_compare_and_swap_acquire(&initial_heap_ptr,
                                    (MK_AO_t)initial_ptr, (MK_AO_t)my_chunk_ptr);
      }

    if (my_chunk_ptr - MK_AO_initial_heap > MK_AO_INITIAL_HEAP_SIZE - CHUNK_SIZE)
      break;
    if (MK_AO_compare_and_swap(&initial_heap_ptr, (MK_AO_t)my_chunk_ptr,
                            (MK_AO_t)(my_chunk_ptr + CHUNK_SIZE))) {
      return my_chunk_ptr;
    }
  }
...
static void add_chunk_as(void * chunk, unsigned log_sz)
{
  ...
  for (ofs = ALIGNMENT - sizeof(MK_AO_t); ofs <= limit; ofs += sz) {
    MK_AO_stack_push(&MK_AO_free_list[log_sz], (MK_AO_t *)((char *)chunk + ofs));
  }
}
...
MK_AO_malloc(size_t sz)
{
  ...
  log_sz = msb(sz + (sizeof(MK_AO_t) - 1));
