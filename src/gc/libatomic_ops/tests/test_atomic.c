...
#if defined(MK_AO_NO_PTHREADS) && defined(MK_AO_USE_PTHREAD_DEFS)
# include <stdio.h>
...
#if defined(MK_AO_HAVE_fetch_and_add1) && defined(MK_AO_HAVE_fetch_and_sub1)

MK_AO_t counter = 0;

void * add1sub1_thr(void * id)
{
  int me = (int)(MK_AO_PTRDIFF_T)id;

  int i;

  for (i = 0; i < NITERS; ++i)
    if ((me & 1) != 0) {
      (void)MK_AO_fetch_and_sub1(&counter);
    } else {
      (void)MK_AO_fetch_and_add1(&counter);
    }
  return 0;
}
...
#endif /* defined(MK_AO_HAVE_fetch_and_add1) && defined(MK_AO_HAVE_fetch_and_sub1) */

#if defined(MK_AO_HAVE_store_release_write) && defined(MK_AO_HAVE_load_acquire_read)
    ...
    if (me & 1)
      {
        ...
        my_counter1 = MK_AO_load(&counter1);
        MK_AO_store(&counter1, my_counter1 + 1);
        MK_AO_store_release_write(&counter2, my_counter1 + 1);
      }
    else
      {
        MK_AO_t my_counter1a, my_counter2a;
        MK_AO_t my_counter1b, my_counter2b;

        my_counter2a = MK_AO_load_acquire_read(&counter2);
        my_counter1a = MK_AO_load(&counter1);
        /* Redo this, to make sure that the second load of counter1 */
        /* is not viewed as a common subexpression.         */
        my_counter2b = MK_AO_load_acquire_read(&counter2);
        my_counter1b = MK_AO_load(&counter1);
...
#endif /* MK_AO_HAVE_store_release_write && MK_AO_HAVE_load_acquire_read */

#if defined(MK_AO_HAVE_test_and_set_acquire)

MK_AO_TS_t lock = MK_AO_TS_INITIALIZER;
...
void * test_and_set_thr(void * id)
{
  ...
  for (i = 0; i < NITERS/10; ++i)
    {
      while (MK_AO_test_and_set_acquire(&lock) != MK_AO_TS_CLEAR);
      ++locked_counter;
      if (locked_counter != 1)
        {
          fprintf(stderr, "Test and set failure 1, counter = %ld, id = %d\n",
                  locked_counter, (int)(MK_AO_PTRDIFF_T)id);
          abort();
        }
      locked_counter *= 2;
      locked_counter -= 1;
...
#endif /* defined(MK_AO_HAVE_test_and_set_acquire) */
...
#endif /* !MK_AO_NO_PTHREADS || !MK_AO_USE_PTHREAD_DEFS */
