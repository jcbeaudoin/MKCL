...
volatile MK_AO_t thread_created_cnt = 0;
volatile MK_AO_t thread_ended_cnt = 0;
...
#endif
{
    int thread_num = MK_AO_fetch_and_add(&thread_created_cnt, 1);
    MK_GC_word my_depth = (MK_GC_word)arg + 1;

    if (my_depth <= MAX_SUBTHREAD_DEPTH
            && thread_num < MAX_SUBTHREAD_COUNT
            && (thread_num % DECAY_DENOM) < DECAY_NUMER
            && (int)(thread_num - MK_AO_load(&thread_ended_cnt))
...
# endif
    ...
    MK_AO_fetch_and_add(&thread_ended_cnt, 1);
    return 0;
}
...
#     endif
  ...
  printf("subthread_create: created %d threads (%d ended)\n",
         (int)MK_AO_load(&thread_created_cnt), (int)MK_AO_load(&thread_ended_cnt));
  return 0;
}
...
#endif /* !MK_AO_HAVE_fetch_and_add */
