...
#ifndef MK_GC_TEST_IMPORT_API
# define MK_GC_TEST_IMPORT_API extern
#endif
...
struct treenode *root_nz[10] = { (void *)(MK_GC_word)1 };

static char *staticroot = 0;

MK_GC_TEST_IMPORT_API struct treenode * libsrl_mktree(int i);
MK_GC_TEST_IMPORT_API void * libsrl_init(void);
MK_GC_TEST_IMPORT_API struct treenode ** libsrl_getpelem(int i, int j);

MK_GC_TEST_IMPORT_API struct treenode ** libsrl_getpelem2(int i, int j);
...
# endif
  ...
  if (NULL == staticroot) {
    fprintf(stderr, "MK_GC_malloc returned NULL\n");
    return 2;
  }
