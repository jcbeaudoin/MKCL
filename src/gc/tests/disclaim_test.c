...
/* MK_GC_malloc_with_finalizer is not reclaimable before the finalizer     */
/* is called.                                                           */
...
void MK_GC_CALLBACK misc_sizes_dct(void *obj, void *cd)
{
...
{
    static const struct MK_GC_finalizer_closure fc = { misc_sizes_dct, NULL };
    int i;
    for (i = 1; i <= 20; ++i) { /* Up to 1 MiB. */
        void *p = MK_GC_finalized_malloc((size_t)1 << i, &fc);
        if (p == NULL) {
...
void MK_GC_CALLBACK pair_dct(void *obj, void *cd)
{
    pair_t p = obj;
    int checksum;
...
{
    pair_t p;
    static const struct MK_GC_finalizer_closure fc = { pair_dct, NULL };

    p = MK_GC_finalized_malloc(sizeof(struct pair_s), &fc);
    if (p == NULL) {
        fprintf(stderr, "Out of memory!\n");
        exit(3);
    }
...
int main(void)
{
    ...
    MK_GC_set_all_interior_pointers(0); /* for a stricter test */
    MK_GC_INIT();
    MK_GC_init_finalized_malloc();
