...
void MK_GC_CALLBACK testobj_finalize(void *obj, void *carg)
{
    ++*(int *)carg;
    my_assert(((testobj_t)obj)->i++ == 109);
}

static const struct MK_GC_finalizer_closure fclos = {
    testobj_finalize,
    &free_count
};
...
testobj_t testobj_new(int model)
{
    ...
    switch (model) {
        case 0:
            obj = MK_GC_MALLOC(sizeof(struct testobj_s));
            if (obj != NULL)
              MK_GC_register_finalizer_no_order(obj, testobj_finalize,
                                             &free_count, NULL, NULL);
            break;
        case 1:
            obj = MK_GC_finalized_malloc(sizeof(struct testobj_s), &fclos);
            break;
        case 2:
            obj = MK_GC_MALLOC(sizeof(struct testobj_s));
            break;
...
int main(int argc, char **argv)
{
    ...
    keep_arr = MK_GC_MALLOC(sizeof(void *)*KEEP_CNT);
