...
#ifndef MKCL_BYTECODE_H
#define MKCL_BYTECODE_H
 ...
 **********************************************************************/
/*
 * See mkcl/src/c/interpreter.d for a detailed explanation of all opcodes
 */
enum {
  OP_NOP,
  OP_QUOTE,
...
typedef int16_t mkcl_oparg;

/*
 * Note that in the small bytecode case, we have to recompose a signed
 ...
 */
  ...
  typedef signed char mkcl_opcode;
# define OPCODE_SIZE 1
# define OPARG_SIZE 2
# ifdef MKCL_WORDS_BIGENDIAN
#  define READ_OPARG(v)	((mkcl_word)v[0] << 8) + (unsigned char)v[1]
# else
#  define READ_OPARG(v) ((mkcl_word)v[1] << 8) + (unsigned char)v[0]
# endif
# define GET_OPARG(r,v) { r = READ_OPARG(v); v += 2; }
#else
  typedef int16_t mkcl_opcode;
# define OPCODE_SIZE 1
# define OPARG_SIZE 1
...
#define MKCL_THREADED_INTERPRETER
#endif

#ifdef MKCL_THREADED_INTERPRETER
#define BEGIN_SWITCH \
	THREAD_NEXT;
#define CASE(name) \
	LBL_##name:
...
#if !defined(MKCL_THREADED_INTERPRETER)
#define MKCL_OFFSET_TABLE
#else
#define MKCL_OFFSET_TABLE   \
